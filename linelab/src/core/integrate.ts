...
import type { FreezeWindow, LongitudinalAction } from "./controller.js";
import { initialMachine, stepSteering } from "./steering.js";
import type { LateralActivation, SteerMachine } from "./steering.js";
import type { ResolvedPositionAction, ResolvedTurnInAction } from "./types.js";
import { longitudinalEventKind, terminalEventKind, sortEvents } from "./events.js";
import type { RawPoint } from "./record.js";
...
type LateralAction = ResolvedTurnInAction | ResolvedPositionAction;

function lateralActions(scenario: ResolvedScenario): LateralAction[] {
  return scenario.rider.plan
    .filter((a): a is LateralAction => a.do === "turn_in" || a.do === "position")
...
export function integrate(
  ...
  let machine: SteerMachine = initialMachine(f0);
  ...
  const brakeEnded = new Set<string>(); // brake ids whose brake_end already fired
  let lastBrakeId: string | null = null; // most recent brake activation (brake_end attribution)
  ...
  for (let k = 0; k <= maxSteps && terminated === null; k++) {
    ...
    if (long.action !== null && long.action.do === "brake") lastBrakeId = long.action.id;
    if (
      lastBrakeId !== null &&
    ...
    const activations: LateralActivation[] = [];
    ...
    const steer = stepSteering(
      machine,
      activations,
      { s, d, f, psi: state.psi, v: state.v, phi: state.phi, t },
      road,
      roll_rate,
      prevAClip
    );
    machine = steer.machine;
    for (const ev of steer.events) events.push({ ...ev });
    ...
    const deriv = (st: Vec5, tau: number): Vec5 & { readonly a_clip: number } => {
    ...
    const advance = (st: Vec5, h: number, dv: Vec5): Vec5 => ({
      x: st.x + h * dv.x,
      y: st.y + h * dv.y,
      psi: st.psi + h * dv.psi,
      v: st.v + h * dv.v,
...
function activationTime(
  ...
  if (prev === undefined || s_now <= prev.s) return t_now;
  if (at_s >= s_now) return t_now;
  if (at_s <= prev.s) {
