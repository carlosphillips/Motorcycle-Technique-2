...
export function aLatMax(mu: number): number {
  return mu * G;
}

/** aLongMax(mu) = mu·G [m/s²] (02 §4). */
export function aLongMax(mu: number): number {
  return mu * G;
}

/** phiMax(mu) = atan(mu) [rad] ( 45.0° at mu = 1.0 ) (02 §4). */
export function phiMax(mu: number): number {
  return Math.atan(mu);
}

/** ellipseMag(aLong, aLat, mu) = sqrt((aLong/aLongMax)² + (aLat/aLatMax)²) (02 §4). */
export function ellipseMag(aLong: number, aLat_ms2: number, mu: number): number {
...
export function gripMargin(aLong: number, aLat_ms2: number, mu: number): number {
 ...
 */
export function aLongAvail(aLat_ms2: number, mu: number): number {
...
export function muUse(skill: number, mu: number): number {
  return skill * mu;
}

/** aLatReserve(muUse) = muUse·G [m/s²] — advisory reserve, never a ceiling (02 §4). */
export function aLatReserve(muUse_: number): number {
  return muUse_ * G;
}

/** phiReserve(muUse) = atan(muUse) [rad] ( 40.36° at skill 0.85, mu 1.0 ) (02 §4). */
export function phiReserve(muUse_: number): number {
...
export function tanhEnvelope(phi: number): number {
 ...
 */
export function suSustained(phi: number, a_cmd: number, mu: number): number {
...
export function suTransient(phi: number, a_cmd_rate: number): number {
 ...
 */
export function phiDotSu(phi: number, a_cmd: number, a_cmd_rate: number, mu: number): number {
 ...
 */
export function aNoReturn(phi: number, roll_rate: number): number {
 ...
 */
export function aWiden(phi: number, v: number, c: number, roll_rate: number): number | null {
  const t = tanhEnvelope(phi);
