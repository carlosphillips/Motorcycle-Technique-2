 ...
 */
export function governingCornerFor(
 ...
 */
export function dDotOf(road: RoadModel, s: number, psi: number, v: number): number {
 ...
 */
export function trackerTargetLean(
  f_target: number,
  road: RoadModel,
  s: number,
  d: number,
...
export function initialMachine(f_start: number): SteerMachine {
...
export function stepSteering(
  ...
  activations: readonly LateralActivation[],
  input: SteerStepInput,
  road: RoadModel,
  roll_rate: number,
  prev_a_clip: number
): SteerStepResult {
  const events: SteerEventDraft[] = [];
  let m = machine;
  ...
  for (const act of activations) {
    if (act.action.do === "turn_in") {
      ...
      const yields =
        m.kind !== "commit" || (m.corner !== null && input.s >= m.corner.s1 - 1e-9);
      if (!yields) continue;
      const target_f =
        act.action.f ?? (act.action.d !== undefined ? road.fOf(act.action.d, act.action.at_s) : 0);
  ...
  let target_lean: number;
  let lat_action_id: string | null;
  switch (m.kind) {
    case "commit":
