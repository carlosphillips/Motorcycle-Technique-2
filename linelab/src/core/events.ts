 ...
 */
export function terminalEventKind(reason: TerminatedReason): EventKind | null {
  switch (reason) {
    case "crash":
      return "crash";
    case "off_road":
 ...
 */
export function longitudinalEventKind(action: LongitudinalAction): EventKind {
  if (action.do === "brake") return "brake_start";
  return action.accel > 0 ? "roll_on" : "crack";
}

const KIND_ORDER: ReadonlyMap<EventKind, number> = new Map(
  EVENT_KINDS.map((k, i) => [k, i])
);
