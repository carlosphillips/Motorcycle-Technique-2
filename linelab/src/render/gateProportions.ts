...
export interface ProportionMetrics {
  readonly width_ratio: readonly { readonly corner_id: string; readonly value: number }[];
...
function twoSided(value: number, band: { min: number; max: number }): { severity: "warn" | "fail" } | null {
  if (value >= band.min && value <= band.max) return null;
  const span = band.max - band.min;
  const margin = span * WARN_MARGIN_FRAC;
  if (value >= band.min - margin && value <= band.max + margin) return { severity: "warn" };
  return { severity: "fail" };
}

function oneSidedMax(value: number, max: number): { severity: "warn" | "fail" } | null {
  if (value <= max) return null;
  const margin = max * WARN_MARGIN_FRAC;
  if (value <= max + margin) return { severity: "warn" };
  return { severity: "fail" };
}

const SEVERITY_RANK: Readonly<Record<GateVerdict, number>> = { pass: 0, warn: 1, fail: 2 };
 ...
 */
export function gateProportions(metrics: ProportionMetrics): ProportionGateResult {
  ...
  const verdict = findings.reduce<GateVerdict>((worst, f) => (SEVERITY_RANK[f.severity] > SEVERITY_RANK[worst] ? f.severity : worst), "pass");
 ...
 * window directly.
 */
export function computeProportionMetrics(
  scene: DrawnScene,
  corners: readonly { readonly id: string; readonly r: number }[],
  straightLenInWindowM: number
): ProportionMetrics {
