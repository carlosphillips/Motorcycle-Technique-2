...
const CLASS_EVENT: Readonly<Record<MarkClass, EventKind>> = Object.freeze({
  turn_point: "turn_in",
  apex: "apex",
  exit: "exit",
  release: "release"
...
const ALL_CLASSES: readonly MarkClass[] = ["turn_point", "apex", "exit", "release"];
 ...
 */
export function enabledClasses(spec: MarkSpec | undefined, role: FigureRole): ReadonlySet<MarkClass> {
  if (spec === "none") return new Set();
  if (spec === "all") return new Set(ALL_CLASSES);
  if (spec === undefined || spec === "auto") return new Set(role === "ideal" ? ALL_CLASSES : []);
  return new Set(spec);
}

function nearestPoint(line: LineResult, s: number): DrawnPoint {
