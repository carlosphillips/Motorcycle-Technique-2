...
import type { FigureLabel, LabelFeature } from "../plan/types.js";
import type { Event, EventKind } from "../core/types.js";
import type { DrawnLabel, DrawnPoint } from "./scene.js";

/** design/03 §8 feature vocabulary → recorded event kind, for the five event-sourced features. */
const FEATURE_EVENT: Partial<Record<LabelFeature, EventKind>> = {
  turn_point: "turn_in",
  apex: "apex",
  exit: "exit",
  release: "release",
...
function unknownId(at: string, message: string, reason: string, extra?: Record<string, unknown>): LinelabError {
  return { code: "UNKNOWN_ID", at, message, detail: { reason, ...extra } };
}

function nearestSampleAnchor(line: LineResult, s: number): { at: DrawnPoint; s: number } {
...
function candidateStations(line: LineResult, label: FigureLabel): readonly number[] {
  ...
  const matches: Event[] = line.trajectory.events
...
function resolveOne(lines: readonly LineResult[], label: FigureLabel, at: string): Result<DrawnLabel> {
  ...
  if (stations.length === 0) {
    return err(
      unknownId(at, `no "${label.feature}" event found on line "${label.line}"`, "anchor_no_match", {
  ...
  if (label.n !== undefined) {
    ...
    if (idx < 0 || idx >= stations.length) {
      return err(
        unknownId(at, `"${label.feature}"#${label.n} has no match on line "${label.line}" (${stations.length} found)`, "anchor_no_match", {
          feature: label.feature,
          n: label.n,
    ...
    return err(
      unknownId(at, `"${label.feature}" matches ${stations.length} events on line "${label.line}" — disambiguate with #n`, "anchor_ambiguous", {
        feature: label.feature,
        line: label.line,
...
export function resolveLabels(
  ...
  figureLabels: readonly FigureLabel[] | undefined
): Result<readonly DrawnLabel[]> {
  if (figureLabels === undefined || figureLabels.length === 0) return ok([]);
