...
export interface DrawnRoad {
   ...
   */
  readonly usable: { readonly lo: readonly DrawnPoint[]; readonly hi: readonly DrawnPoint[] } | null;
}
...
export interface DrawnScene {
  ...
  readonly legend: { readonly visible: boolean; readonly rows: readonly LegendRow[] };
...
export function withMarkers(scene: DrawnScene, markers: readonly DrawnMarker[]): DrawnScene {
  return { ...scene, markers };
}

/** Pure attach — a new `DrawnScene`, never a mutation of `scene` (labels.ts's `resolveLabels` output). */
export function withLabels(scene: DrawnScene, labels: readonly DrawnLabel[]): DrawnScene {
  return { ...scene, labels };
}

/** Pure attach — the figure's authored stage-11 placards (design/06 §3.1); `project()` never sees the FigureSpec. */
export function withPlacards(scene: DrawnScene, placards: readonly string[]): DrawnScene {
