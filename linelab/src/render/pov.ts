 ...
 */
export const POV_ROLL_MODES = ["lean", "level"] as const;
...
export interface PovFrame {
  ...
  readonly path: {
    ...
    readonly offFrame: { readonly at: Pt; readonly dx: number; readonly dy: number } | null;
  } | null;
...
interface Camera {
  ...
  /** the recorded lean, always — the HUD dial reads this in both roll modes. */
  readonly phiRad: number;
  /** what the FRAME is rotated by: `phiRad` under roll `lean`, 0 under `level`. */
...
function buildCamera(sample: Sample, look: PovLook, roll: PovRoll, W: number, H: number): Camera {
 ...
 * the vertex.
 */
function projectRuns(cam: Camera, verts: readonly { x: number; y: number; z?: number }[]): readonly (readonly Pt[])[] {
  const runs: Pt[][] = [];
  let current: Pt[] = [];
  for (const v of verts) {
    const p = project(cam, v.x, v.y, v.z ?? 0);
 ...
 */
function roadQuads(cam: Camera, road: ComposedRoad, fromS: number, toS: number): readonly (readonly Pt[])[] {
  const stations = stationsForward(fromS, Math.min(toS, road.total_len_m));
  const w = road.lane_width_m;
  const quads: { poly: readonly Pt[]; depth: number }[] = [];
  for (let i = 0; i + 1 < stations.length; i++) {
    ...
    quads.push({ poly: projected as Pt[], depth });
  }
 ...
 */
function pathOverlay(cam: Camera, line: LineResult, fromS: number): PovFrame["path"] {
  ...
  const inFrame = (p: Pt): boolean => p.x >= 0 && p.x <= cam.W && p.y >= 0 && p.y <= cam.H;
  if (runs.some((r) => r.some(inFrame))) return { runs, colour, offFrame: null };

  // nothing on screen: point at the nearest projected sample from the centre
  const all = runs.flat();
  const nearest = all.reduce((a, b) =>
    Math.hypot(b.x - cam.P0.x, b.y - cam.P0.y) < Math.hypot(a.x - cam.P0.x, a.y - cam.P0.y) ? b : a
  );
 ...
 */
function serializeLeanDial(frame: PovFrame): string {
  ...
  return (
    `<g${attrs({ "data-lean-dial": n(frame.phi_deg) })}>` +
 ...
 */
function serializeRiderAnchor(frame: PovFrame): string {
  const { width: W, height: H } = frame;
  const y = H - 34; // the HUD strip's top edge — the bars sit on it
  const ink = "#22262b";
  const side = (dir: -1 | 1): string => {
 ...
 */
export function renderPovForFigure(road: ComposedRoad, lines: readonly LineResult[], look: PovLook, roll: PovRoll = "lean"): string {
