...
import type { Sample, ResolvedHazard } from "../core/types.js";
...
// ---------------------------------------------------------------------------
// ViewSpec (design/06 §2.1, verbatim closed-set fields; ARCHITECTURE §6.5
// narrows the v0.1-legal surface — mode has one legal value, width_exag/
// straight_compress/taper_compress/fan are all typed-deferred).
...
export type ViewWindow = "auto" | "all" | { readonly from: StationRef; readonly to: StationRef };
...
function schemaErr(at: string, message: string, reason: string, extra?: Record<string, unknown>): LinelabError {
  return { code: "SCHEMA", at, message, detail: { reason, ...extra } };
}
function deferredErr(at: string, message: string, deferred: string, schema_ref: string): LinelabError {
  return { code: "SCHEMA", at, message, schema_ref, deferred, detail: { reason: "deferred" } };
}
function badRange(at: string, message: string, reason: string, extra?: Record<string, unknown>): LinelabError {
...
function isStationRef(v: unknown): v is StationRef {
  if (!isObject(v)) return false;
  if (typeof v["at_s"] === "number") return true;
...
interface ResolvedViewRequest {
  readonly window: ViewWindow;
  readonly orient: ViewOrient;
  readonly rays: ViewRays;
  readonly legend: ViewLegendMode;
  /** design/06 §3.1 stage 8b — draw where a runoff was pointing when it left. Off unless asked. */
...
function parseViewSpec(raw: unknown): Result<ResolvedViewRequest> {
  if (raw === undefined || raw === null) {
    return ok({ window: "auto", orient: "auto", rays: "auto", legend: "auto", consequence: false });
  }
  if (!isObject(raw)) return err(schemaErr("view", "view must be a JSON object", "type_mismatch"));
  ...
  for (const deferredKey of ["width_exag", "straight_compress", "taper_compress"] as const) {
    if (raw[deferredKey] !== undefined) {
      return err(
        deferredErr(`view.${deferredKey}`, `view.${deferredKey} is not shipped yet`, "projection (post-v0.1)", "view")
      );
    }
  }
  ...
  if (raw["orient"] !== undefined) {
    ...
    if (o === "auto") orient = "auto";
    else if (typeof o === "number" && ORIENT_NUMERIC.has(o)) orient = o as ViewOrient;
    else {
      ...
      return err(
        schemaErr(
          "view.orient",
          `view.orient must be "auto" or one of 0/90/180/270 — mirroring is not a view operation, handedness is set on the road (see "road ... hand=")`,
          "no_view_mirror"
        )
      );
    }
  ...
  let legend: ViewLegendMode = "auto";
  ...
  if (raw["consequence"] !== undefined) {
    ...
    if (c === "on" || c === true) consequence = true;
    else if (c === "off" || c === false) consequence = false;
    else return err(schemaErr("view.consequence", 'view.consequence must be "on" or "off"', "type_mismatch"));
  }
...
function endAnchorS(line: LineResult): number {
  let best: number | null = null;
  for (const e of line.trajectory.events) {
    if (e.kind === "exit") {
      if (best === null || e.s > best) best = e.s;
...
function autoWindow(lines: readonly LineResult[], roadLen: number): { from_s: number; to_s: number } {
  if (lines.length === 0) return { from_s: 0, to_s: roadLen };
...
function resolveWindow(
  ...
  road: ComposedRoad
): Result<{ from_s: number; to_s: number }> {
  if (req === "auto") return ok(autoWindow(lines, road.total_len_m));
  if (req === "all") return ok({ from_s: 0, to_s: road.total_len_m });
...
function resolveOrient(req: ViewOrient): 0 | 90 | 180 | 270 {
...
export function rotatePoint(p: DrawnPoint, cx: number, cy: number, deg: 0 | 90 | 180 | 270): DrawnPoint {
  if (deg === 0) return p;
...
function boundingBox(points: readonly DrawnPoint[]): { minX: number; minY: number; maxX: number; maxY: number } {
...
function paddedFrame(box: { minX: number; minY: number; maxX: number; maxY: number }): { width: number; height: number } {
...
function clippedSamples(samples: readonly Sample[], from_s: number, to_s: number): readonly Sample[] {
...
function defaultSightRay(line: LineResult, colour: string): DrawnSightRay | null {
  const firstTurnIn = line.trajectory.events.filter((e) => e.kind === "turn_in").sort((a, b) => a.s - b.s)[0];
  if (firstTurnIn === undefined) return null;
 ...
 * wash).
 */
function buildOcclusionWash(road: ComposedRoad, s_limit: number, from_s: number, to_s: number): DrawnPoint[] | null {
  const start = Math.max(s_limit, from_s);
  if (start >= to_s) return null;
  const stations: number[] = [];
  for (let s = start; s < to_s; s += EDGE_STEP_M) stations.push(s);
  stations.push(to_s);
...
function hazardDBand(road: ComposedRoad, h: ResolvedHazard, s: number): { lo: number; hi: number } {
  if (h.side === "center") {
 ...
 */
function buildDrawnHazard(road: ComposedRoad, h: ResolvedHazard): DrawnHazard {
  const s0 = h.at.at_s;
  const s1 = s0 + h.span_m;
  const stations: number[] = [];
  for (let s = s0; s < s1; s += EDGE_STEP_M) stations.push(s);
  stations.push(s1);
...
function buildDrawnHazards(road: ComposedRoad, lines: readonly LineResult[]): DrawnHazard[] {
  const hazards = lines[0]?.resolved_scenario.hazards ?? [];
  return hazards.map((h) => buildDrawnHazard(road, h));
}

function nearestSample(samples: readonly Sample[], s: number): Sample | undefined {
 ...
 */
function consequenceRay(from: DrawnPoint, heading_deg: number, occluders: readonly DrawnOccluder[]): readonly DrawnPoint[] {
  ...
  for (const o of occluders) {
    ...
    for (let i = 0; i < poly.length; i++) {
      ...
      if (t > 0.05 && t < best && u >= 0 && u <= 1) best = t;
    }
  }
 ...
 */
function edgeTangentDeg(road: ComposedRoad, s: number, dSign: 1 | -1): number {
...
function buildDrawnLine(
  ...
  from_s: number,
  to_s: number,
  occluders: readonly DrawnOccluder[],
  consequence: boolean
): DrawnLine {
  ...
  let sightRay: DrawnSightRay | null = null;
...
function buildDrawnRoad(road: ComposedRoad, from_s: number, to_s: number): DrawnRoad {
  const stations: number[] = [];
  for (let s = from_s; s < to_s; s += EDGE_STEP_M) stations.push(s);
  stations.push(to_s);
  const w = road.lane_width_m;
...
function buildDrawnOccluders(road: ComposedRoad, lines: readonly LineResult[]): DrawnOccluder[] {
 ...
 */
export function project(road: ComposedRoad, lines: readonly LineResult[], viewSpec: unknown): Result<DrawnScene> {
  ...
  const rawWash =
    rayLine?.sightRay !== null && rayLine?.sightRay !== undefined
      ? buildOcclusionWash(road, rayLine.sightRay.s_limit, from_s, to_s)
      : null;

  const allPoints: DrawnPoint[] = [
    ...drawnRoad.left,
    ...drawnRoad.right,
  ...
  const rotate = (p: DrawnPoint): DrawnPoint => rotatePoint(p, cx, cy, orient);
  const rotatedRoad: DrawnRoad = {
    ...drawnRoad,
    left: drawnRoad.left.map(rotate),
    right: drawnRoad.right.map(rotate),
    centre: drawnRoad.centre.map(rotate),
    usable:
      drawnRoad.usable === null
        ? null
  ...
  const rotatedLines: DrawnLine[] = drawnLines.map((l) => ({
    ...l,
  ...
  const rotatedOccluders: DrawnOccluder[] = occluders.map((o) => ({
    ...o,
    anchor: rotate(o.anchor),
    footprint: o.footprint.map(rotate)
  }));
  const rotatedHazards: DrawnHazard[] = hazards.map((h) => ({
    ...h,
