...
import { analyzeCorners, type CornerRow } from "../core/analyze.js";
...
import type { DoctrineCorner, DoctrineRecord } from "../plan/doctrine/types.js";
...
import {
  BEST_FAILING_MIN_CANDIDATES,
  BISECT_ITERS,
  COARSE_DS_M,
  KISS_TOL_F,
  N_PROBE,
  ROLLON_ACCEL_MS2,
  SOLVER_BRAKE_SLEW_MSS,
  TARGET_APEX_TABLE,
  exit_target,
  lean_frac
} from "./constants.js";
import {
  ...
  rollOnBracketAt,
  vTargetMs,
  type DerivedStations
} from "./stations.js";
import {
  constraintWithoutSolver,
  mergeAuthoredPlan,
  noSolutionConflict,
  type MergeDirectives
} from "./merge.js";
import {
  corridorExcessM,
  pickBestFailing,
...
import { assembleVerdict, type CornerCorrective } from "./verdict.js";
...
// ---------------------------------------------------------------------------
// Input surface. plan/types.ts's SolveSpec is the wire reconstruction; the
// authored-plan fragment of the §4.9 merge contract rides beside it (solve/ is
// the recorded authority on fields that file misses).
...
function outOfScope(at: string, message: string, reason: string): LinelabError {
...
const CONSTRAINT_BOUNDS: readonly ConstraintBound[] = ["f_min", "f_max", "v_max_kmh", "sight_margin_min_m"];
...
function resolveSpanAnchor(token: string, corners: readonly Corner[], at: string): Result<number> {
  if (token.startsWith("s:")) {
    const n = Number(token.slice(2));
    if (!Number.isFinite(n)) return err(schemaErr(at, `malformed station token "${token}"`, "anchor_malformed"));
    return ok(n);
  }
...
export function resolveConstraints(
  ...
  roadEnd: number
): Result<readonly ResolvedConstraint[] | null> {
  if (constraints === undefined || constraints.length === 0) return ok(null);
  ...
  for (let i = 0; i < constraints.length; i++) {
    ...
    seen.add(c.id);
    if (!(CONSTRAINT_BOUNDS as readonly string[]).includes(c.bound)) {
...
export function evalConstraints(
  ...
  return constraints.map((rc) => {
    ...
    let worst = span[0]!;
    let observe: (s: Sample) => number;
    let margin: (obs: number) => number;
    switch (rc.bound) {
      case "f_min":
        observe = (s) => s.f;
    ...
    for (const s of span) {
      if (margin(observe(s)) < margin(observe(worst))) worst = s;
    }
...
export function constraintsSatisfied(rows: readonly ConstraintRow[] | null): boolean {
  return rows === null || rows.every((r) => r.satisfied);
}

/** NO_SOLUTION/constraint_unmet naming the id, worst station, achieved vs required. */
export function constraintUnmet(rows: readonly ConstraintRow[], at: string): LinelabError {
...
function hazardBandMu(road: ComposedRoad, scenario: ResolvedScenario): (s: number, d: number) => number {
...
function wireRoadOf(road: SolveSpec["road"]): RoadSpec {
  if (typeof road === "string") {
    return (PRESET_NAMES as readonly string[]).includes(road) ? { preset: road } : { dsl: road };
  }
  return road;
}

function wireScenario(ctx: Pick<SolveCtx, "wireRoad" | "spec">, plan: readonly unknown[]): Record<string, unknown> {
  const spec = ctx.spec;
  return {
    spec: "linelab/1",
    id: "solve",
...
export function buildSolveContext(specIn: SolveInput): Result<SolveCtx> {
  const spec = specIn;
  ...
  if (acceptErr !== null) return err(acceptErr);
  const policy: AcceptPolicy = spec.accept ?? "clean";
  ...
  if (!directivesR.ok) return directivesR;
  ...
  const base = baseR.value as unknown as ResolvedScenario; // empty plan: structurally resolved
  ...
  if (!stationsR.ok) return stationsR;
  ...
  if (!constraintsR.ok) return constraintsR;
  ...
  if (constraints !== null) {
    for (const rc of constraints) {
      if (rc.s0 - 1e-9 <= corner.s1 && corner.s1 <= rc.s1 + 1e-9) {
        if (rc.bound === "f_max") exitTargetEff = Math.min(exitTargetEff, rc.value);
        if (rc.bound === "f_min") exitTargetEff = Math.max(exitTargetEff, rc.value);
      }
    }
  }
...
// ---------------------------------------------------------------------------
// Search-run machinery (engine measurements; verdict-free)
...
export function measureRun(ctx: SolveCtx, plan: readonly ResolvedPlanAction[], coarse: boolean): RunMeasure {
  const scenario: ResolvedScenario = { ...ctx.base, rider: { ...ctx.base.rider, plan } };
 ...
 */
function minFOver(m: RunMeasure, ctx: SolveCtx, s_ti: number): number {
  ...
  for (const s of m.traj.samples) {
    if (s.s < lo - 1e-9 || s.s > hi + 1e-9) continue;
    if (s.f < min) min = s.f;
  }
  return min;
}
...
function minFStation(m: RunMeasure, ctx: SolveCtx, s_ti: number): number | null {
  const lo = Math.min(s_ti, ctx.corner.s0);
  const hi = ctx.corner.s0 + 0.9 * (ctx.corner.s1 - ctx.corner.s0);
...
function deepestApex(m: RunMeasure, cornerId: string): { s: number; pct: number; f: number; lean_deg: number } | null {
  const row = m.rows.find((r) => r.id === cornerId);
  if (row === undefined || row.apexes.length === 0) return null;
  let best = row.apexes[0]!;
  for (const a of row.apexes) if (a.f < best.f) best = a;
 ...
 */
function apexLeanDeg(m: RunMeasure, ctx: SolveCtx): number {
 ...
 */
function exitF(m: RunMeasure, ctx: SolveCtx): number {
  if (m.traj.terminated.reason === "off_road" || m.traj.terminated.reason === "crash") {
    return Number.POSITIVE_INFINITY;
  }
  for (const s of m.traj.samples) {
    if (s.f > 1 + eps_f_detect) return Number.POSITIVE_INFINITY;
  }
  const row = m.rows.find((r) => r.id === ctx.corner.id);
...
export function containedRun(m: RunMeasure): boolean {
  if (m.traj.terminated.reason !== "road_end") return false;
  for (const s of m.traj.samples) {
    if (s.f > 1 + eps_f_detect || s.f < -EPS_INSIDE_CUT_F) return false;
  }
  return true;
}
 ...
 */
export function outwardCleanRun(m: RunMeasure): boolean {
  if (m.traj.terminated.reason !== "road_end") return false;
  for (const s of m.traj.samples) {
    if (s.f > 1 + eps_f_detect) return false;
  }
  return true;
}
...
function insideNormal(psi: number, hand: Corner["hand"]): { x: number; y: number } {
 ...
 */
function releaseStation(ctx: SolveCtx, s_ti: number, decel: number): number {
 ...
 */
export function predictVti(ctx: SolveCtx, s_ti: number, decel: number): number {
  const s_end = s_ti - ctx.stations.brake_gap_m;
...
function aimStation(ctx: SolveCtx): number {
  const c = ctx.corner;
 ...
 */
export function tangentLeanDeg(ctx: SolveCtx, s_ti: number, v_ti: number): TangentLean {
  const road = ctx.road;
  ...
  if (!(kappaAim > 0)) return { lean_deg: MIN_LEAN_DEG, capped: false };
  ...
  const clearance = (leanDeg: number): number => {
    let x = P0.x;
    let y = P0.y;
    let psi = h0;
    ...
    for (let k = 0; k < steps; k++) {
      ...
      const c = dist - r_i;
      if (c < min) min = c;
      if (dist > r_o + 3) break; // blown out of the corridor — no later kiss counts
    }
    return min;
  };

  const cHi = clearance(ctx.phi_reserve_deg);
  if (cHi > 0) return { lean_deg: ctx.phi_reserve_deg, capped: true };
  let lo = MIN_LEAN_DEG;
  let hi = ctx.phi_reserve_deg;
  if (clearance(lo) <= 0) return { lean_deg: MIN_LEAN_DEG, capped: false };
  for (let i = 0; i < 24 && hi - lo > 0.01; i++) {
    const mid = (lo + hi) / 2;
    const c = clearance(mid);
    if (Math.abs(c) < 0.005) return { lean_deg: mid, capped: false };
    if (c > 0) lo = mid;
    else hi = mid;
  }
...
interface KissResult {
  readonly lean_deg: number;
  readonly min_f: number;
  /** even phiReserve cannot reach the inside */
  readonly capped: boolean;
...
function kissRefine(
  ...
  for (let i = 0; i + 1 < rungs.length && budget > 0; i++) {
    ...
    const b = rungs[i + 1]!;
    const straddles =
      (a.min_f > KISS_TOL_F && b.min_f < 0) || (a.min_f < 0 && b.min_f > KISS_TOL_F);
  ...
  const capped =
    reserveRung !== undefined && reserveRung.min_f > KISS_TOL_F && best.min_f > KISS_TOL_F;
  return { ...best, capped };
}
...
function resolveAuthoredStation(a: PlanAction, corners: readonly Corner[], at: string): Result<number> {
  if (a.at_s !== undefined) return ok(a.at_s);
  if (a.at !== undefined) return resolveAnchor(a.at, corners, at);
...
interface PlanShape {
  ...
  readonly lean_deg: number;
  readonly roll_on_s: number;
  readonly roll_on_accel: number;
}
 ...
 */
function buildResolvedPlan(ctx: SolveCtx, shape: PlanShape): Result<readonly ResolvedPlanAction[]> {
  const sc = ctx.stations;
  const corner = ctx.corner;
  const d = ctx.directives;
  const actions: ResolvedPlanAction[] = [];
  ...
  for (const p of d.positions) {
    ...
    const over =
      p.over_m !== undefined && p.over_m !== "auto" ? p.over_m : Math.min(ctx.road.total_len_m, shape.s_ti) - s.value;
    const resolved: ResolvedPositionAction = {
      do: "position",
      id: p.id,
      at_s: s.value,
      ...(p.f !== undefined ? { f: p.f } : { d: p.d as number }),
      over_m: over
    };
...
function wirePlanOf(resolved: readonly ResolvedPlanAction[]): readonly Record<string, unknown>[] {
...
function narrowValidated(v: ValidatedScenario, at: string): Result<ResolvedScenario> {
  ...
  return ok(v as unknown as ResolvedScenario);
}

function lineSpecHash(resolved: ResolvedScenario, source: LineSource): Result<string> {
 ...
 * straight (02 §3.1).
 */
export function executeLine(input: ExecuteInput): Result<LineResult> {
  ...
  const merged = buildTrajectory(
    [...traj1.samples],
    sortEvents([...traj1.events, ...analysis.events]),
    traj1.terminated
  );

  const shot = correctiveShot({ trajectory: merged, resolved_scenario: resolved });
  const correctives: CornerCorrective[] = [];
  let final = merged;
  if (shot.ok) {
    ...
    if (detects.length > 0) {
      final = buildTrajectory(
        [...merged.samples],
        sortEvents([...merged.events, ...runWideDetectEvents(detects)]),
        merged.terminated
      );
  ...
  const doctrineCorners: DoctrineCorner[] = road.corners.map((c) => {
  ...
  if (!specHashR.ok) return specHashR;
  ...
  if (!verdictR.ok) return verdictR;
...
// ---------------------------------------------------------------------------
// The per-candidate full solve (§4.2 pipeline order)
...
export function fullSolveAtStation(
  ctx: SolveCtx,
  s_ti: number,
  opts: FullSolveOptions
  ...
  if (d.roll_on.kind === "pinned") {
    ...
    if (d.roll_on.kind === "magnitude") rollAccel = d.roll_on.accel;
  }
  ...
  const authoredLean =
    d.turn_in.kind === "pinned" && d.turn_in.action.target !== "tangent_inside"
      ? d.turn_in.action.target.lean_deg
      : null;

  const planFor = (decel: number, lean: number, rollOn: number): Result<readonly ResolvedPlanAction[]> =>
    buildResolvedPlan(ctx, { s_ti, decel, lean_deg: lean, roll_on_s: rollOn, roll_on_accel: rollAccel });

  const runShape = (decel: number, lean: number, rollOn: number): Result<RunMeasure> => {
  ...
  if (!probeRunR.ok) return probeRunR;
  ...
  let probeInfeasible: LinelabError | null = null;
  if (seedProbe.capped && authoredLean === null && probeMinF > KISS_TOL_F) {
    probeInfeasible = noSolution(
      "turn_in_infeasible_late",
      "solve.probe",
      `turn-in at s=${s_ti.toFixed(2)} is too late: the apex never comes inside (min f ${probeMinF.toFixed(3)} even at phiReserve)`,
      { s_ti, min_f: probeMinF }
    );
  } else if (probeMinF < -EPS_INSIDE_CUT_F || (probeApex !== null && probeApex.s < ctx.corner.s0 - 1e-6)) {
    probeInfeasible = noSolution(
      "turn_in_infeasible_early",
      "solve.probe",
      `turn-in at s=${s_ti.toFixed(2)} is too early: the line cuts inside`,
      { s_ti, min_f: probeMinF, ...(probeApex !== null ? { apex_s: probeApex.s } : {}) }
    );
  }
  if (probeInfeasible !== null && opts.short_circuit_probe) return err(probeInfeasible);

  // --- 2a. bisect decel against the emergent apex-lean target ----------------
  let decel = decelLo;
  if (d.decel.kind !== "pinned" && authoredLean === null) {
    const g = (dec: number): Result<number> => {
  ...
  if (d.roll_on.kind !== "pinned") {
    ...
    const g = (onset: number): Result<number> => {
      const r = runShape(decel, lean, onset);
      if (!r.ok) return r;
      const m = r.value;
      for (const sm of m.traj.samples) {
        if (sm.f < -EPS_INSIDE_CUT_F) return ok(Number.NEGATIVE_INFINITY);
      }
    ...
    const gHiR = g(rollHi);
    if (!gHiR.ok) return gHiR;
    let best: { onset: number; gap: number } | null = null;
    const consider = (
      prev: { onset: number; gap: number } | null,
      onset: number,
      value: number
    ): { onset: number; gap: number } | null => {
      if (Number.isFinite(value) && value <= target) {
        const gap = target - value;
        if (prev === null || gap < prev.gap) return { onset, gap };
      }
      return prev;
    };
    ...
    if (gLoR.value <= target && Number.isFinite(gLoR.value)) {
      ...
      for (let i = 0; i < BISECT_ITERS && hi - lo > 0.02; i++) {
        ...
        if (Math.abs(gm.value - target) < 0.005) break;
        if (gm.value > target) lo = mid;
        else hi = mid;
      }
  ...
  if (!finalPlanR.ok) return finalPlanR;
  ...
  if (!validated.ok) {
    ...
    if (hit !== undefined) return err(noSolutionConflict(atPath, hit, validated.error.message));
    return err({
      code: "INTERNAL",
      at: "solve.selfVerify",
  ...
  const source: LineSource = { kind: "solve", solveSpec: ctx.spec };
  ...
  const fineApexPct =
    fineApex !== undefined && fineApex.apexes.length > 0
 ...
 */
export function solve(spec: SolveInput): Result<LineResult> {
  ...
  if (typeof spec.turn_in === "number") explicit = spec.turn_in;
  ...
  if (explicit === null) return autoSolve(ctx);
...
// ---------------------------------------------------------------------------
// Coarse-sweep per-candidate evaluation (design/04 §3.1 — one cheap engine run
// per candidate at COARSE_DS_M; suggest.ts hosts the sweep/rank/top-4 loop)
...
export function coarseCandidate(ctx: SolveCtx, s_ti: number): CoarseCandidate {
  const dead = (error: LinelabError): CoarseCandidate => ({
    s_ti,
    error,
    contained: false,
    apex_pct: null,
  ...
  if (d.decel.kind === "pinned") {
    ...
    if (!bracket.ok) return dead(bracket.error);
    decelLo = bracket.value.lo;
    decelHi = bracket.value.hi;
  }
  let rollMid: number;
  ...
  if (!planR.ok) return dead(planR.error);
  ...
  const constraint_rows =
    ctx.constraints !== null ? evalConstraints(m.traj.samples, ctx.constraints) : null;
  return {
    s_ti,
    error: null,
