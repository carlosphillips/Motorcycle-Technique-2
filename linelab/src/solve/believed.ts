...
import { noSolution, solve, type SolveInput } from "./solve.js";
...
import type { LineResult, MisjudgeDivergenceKind, MisjudgmentBlock } from "./types.js";
...
function sameSegment(a: Segment, b: Segment): boolean {
  ...
  if (a.type === "straight" && b.type === "straight") return a.len_m === b.len_m;
...
export function divergenceOf(actual: readonly Segment[], believed: readonly Segment[]): Divergence | null {
  let boundary = 0;
  ...
  for (let i = 0; i < n; i++) {
    ...
    const isRadius =
      a.type === "arc" && b.type === "arc" && a.hand === b.hand && a.angle_deg === b.angle_deg && a.r_m !== b.r_m;
    return {
      s_div: boundary,
  ...
  if (actual.length === believed.length) return null; // identical after normalization
...
function kappaGap(
  actual: { readonly kappa_road: (s: number) => number; readonly total_len_m: number },
  believed: { readonly kappa_road: (s: number) => number; readonly total_len_m: number },
  sDiv: number
): { readonly max_abs_1pm: number; readonly at_s: number } {
...
// ---------------------------------------------------------------------------
// underread/overread sugar (03 §7.4): rewrite ONE corner's segment on the
// actual road — constant `arc r=r_believed`, or a rescaled angle_deg — leaving
// every other segment byte-identical. The mistake-kind grammar/defaults live
// in plan/mistakes.ts (WP-05); WP-12's compiler calls this rewrite.
...
export function believedRoadFromSugar(
  actualRoad: SolveInput["road"],
  sugar: BeliefSugar
): Result<RoadSpec> {
  ...
  if (hasR && hasSweep) {
    return err(
      schemaErr("mistake.params", "exactly one of r_believed | sweep_believed_deg (one belief, D23)", "misjudge_param_required")
    );
  }
  if (!hasR && !hasSweep) {
    ...
    return err(
      schemaErr(
        "mistake.params",
        "exactly one of r_believed | sweep_believed_deg is required on an arc corner",
        "misjudge_param_required"
      )
    );
  }
  ...
  if (hasR) {
    rewritten = { type: "arc", r_m: sugar.r_believed as number, angle_deg: corner.angle_deg, hand: corner.hand };
  } else if (seg.type === "arc") {
    rewritten = { type: "arc", r_m: seg.r_m, angle_deg: sugar.sweep_believed_deg as number, hand: seg.hand };
  } else if (seg.type === "taper") {
    rewritten = { type: "taper", r1_m: seg.r1_m, r2_m: seg.r2_m, angle_deg: sugar.sweep_believed_deg as number, hand: seg.hand };
  } else {
 ...
 */
export function solveBelieved(spec: SolveInput, opts?: SolveBelievedOptions): Result<LineResult> {
  ...
  if (spec.accept === "best_failing") {
    return err(
      schemaErr("accept", "best_failing would relax the believed-world clean bar", "accept_policy_incompatible_with_misjudge")
    );
  }
  ...
  if (!actualNorm.ok) return actualNorm;
  const believedNorm = normalizeRoadSpec(believedWire);
  if (!believedNorm.ok) return believedNorm;

  // lane geometry must match (v1: the rider misjudges curvature, not lane width)
  if (
    actualNorm.value.lane_width_m !== believedNorm.value.lane_width_m ||
    ...
    return err(
      schemaErr("believed_road", "believed and actual lane geometry differ", "believed_lane_geometry_differs", {
  ...
  if (divergence.s_div <= 0) {
    return err(
      schemaErr(
        "believed_road",
        "the worlds differ from the start — the plan's stations must mean the same asphalt in both worlds up to the misread",
        "believed_no_shared_prefix",
        { s_div: divergence.s_div }
      )
    );
  }
  ...
  const believedSpec: SolveInput = { ...rideSpec, road: believedWire };
  const believedComposed = compose(believedWire);
  if (!believedComposed.ok) return believedComposed;
  const believedR =
    spec.vis === "cautious"
    ...
    if (believedR.error.code === "NO_SOLUTION") {
      return err(
        noSolution(
          ...
          "solveBelieved",
          "the believed-world solve did not produce a clean line — a bad plan in a wrong world is two perturbations",
          { inner: believedR.error.detail ?? {}, inner_message: believedR.error.message }
        )
      );
    }
    return believedR;
  ...
  if (!believedLine.verdict.ok) {
    return err(
      noSolution("believed_world_not_clean", "solveBelieved", "the believed-world line does not verify clean", {
  ...
  if (!actualComposed.ok) return actualComposed;
  ...
  if (!canonBelieved.ok) return canonBelieved;

  const divergedCorner =
    actualComposed.value.corners.find((c) => c.s0 <= divergence.s_div + 1e-6 && divergence.s_div < c.s1 + 1e-6) ??
...
export function solveMisjudgeSugar(
  spec: SolveInput,
  sugar: BeliefSugar
): Result<LineResult> {
  const believedRoadR = believedRoadFromSugar(spec.road, sugar);
  if (!believedRoadR.ok) return believedRoadR;
  ...
  if (sugar.r_believed !== undefined) params["r_believed"] = sugar.r_believed;
  if (sugar.sweep_believed_deg !== undefined) params["sweep_believed_deg"] = sugar.sweep_believed_deg;
  return solveBelieved(
    { ...spec, believed_road: road },
    { sugar: { kind: sugar.kind, params, corner_id: cornerId } }
  );
}
