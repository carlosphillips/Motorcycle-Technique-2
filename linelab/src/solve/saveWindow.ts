...
export const SAVE_WINDOW_PLACARD =
 ...
 */
export const SAVE_WINDOW_STATUS_SENTENCES: Readonly<Record<SaveWindowStatus, string>> = Object.freeze({
  resolved:
    "the reserve-lean save window opened and closed once over this corner; `tau_close_s` is the last instant from which the probe still returned inside the corridor.",
  open_at_end:
    "the reserve-lean save window is still open at the scanned horizon — `tau_close_s` is the horizon, not an observed closure, so no countdown to it is drawn.",
  never_open:
    "no reserve-lean save existed at any legal start instant on this corner: the window never opened, and no closing instant is reported.",
  intermittent:
    "the reserve-lean save window opened and closed N times over this corner; linelab will not report a single closing instant.",
  not_applicable:
    "this corner recorded no run-wide detect, so the reserve-lean probe was never attempted and the analysis is inert here."
});

/** Display precision (04 §4b.5): every human-facing string clamps to HORIZON_DISPLAY_DP. */
export function horizonDisplay(x: number): string {
  return (x === 0 ? 0 : x).toFixed(HORIZON_DISPLAY_DP);
}

// ---------------------------------------------------------------------------
// Output shapes (04 §4b.7, field names verbatim)
...
// ---------------------------------------------------------------------------
// Input: the structural subset of LineResult (05 §7) the analysis reads —
// declared here exactly as solve/corrective.ts declares CorrectiveShotInput.
// The full LineResult satisfies it; the analysis is a pure CONSUMER of the
// finished line document and adds zero engine runs to any figure path.
...
export interface SaveWindowInput {
  ...
  readonly verdict: { readonly corners: readonly SaveWindowCornerRow[] };
}
...
function internal(at: string, message: string, reason: string, extra: Record<string, unknown> = {}): LinelabError {
...
function clipAtStar(doc: CfShadowDocument, s_star: number | null): CfShadowDocument {
  if (s_star === null) return doc;
  ...
  if (cut >= doc.samples.length) return doc;
...
interface CornerContext {
  ...
  readonly hand: Hand;
  /** the main line's recorded run_wide_detect for this corner, or null */
  readonly detect: { readonly s: number; readonly t: number } | null;
}

function cornerContext(
  line: SaveWindowInput,
  cornerId: string
): Result<CornerContext & { readonly row: SaveWindowCornerRow }> {
  ...
  const ev = line.trajectory.events.find(
    (e) => e.kind === "run_wide_detect" && e.corner_id === cornerId
  );
  return ok({
    road: composed.value,
    spec,
...
function saveAtInner(
  line: SaveWindowInput,
  ctx: CornerContext,
  tau_s: number
): Result<SaveAtResult, CfRefusal | LinelabError> {
  ...
  if (at === null) {
    return err(
      internal("saveAt", "launch instant outside the recorded line", "save_launch_unresolvable", {
        tau_s
      })
    );
  }
  const turn_in_before = line.trajectory.events.some(
    (e) => e.kind === "turn_in" && e.t <= tau_s + 1e-12
  );
  ...
  const cf = counterfactual(
    ...
    SAVEAT_BINDING.rider,
    SAVEAT_BINDING.predicate
  );
  if (!cf.ok) return cf;
...
function policyOf(line: SaveWindowInput, hand: Hand): SaveWindowPolicy {
  const scenario = line.resolved_scenario;
  const profile = effectiveProfile(scenario);
  return Object.freeze({
    target_phi_deg: radToDeg(
      handSign(hand) * phiReserve(muUse(profile.skill, scenario.config.mu))
    ),
    roll_rate_dps: profile.roll_rate_dps,
    a_cmd_ms2: 0 as const,
    basis: "phiReserve(skill·mu)" as const
  });
}
...
function windowBase(line: SaveWindowInput, cornerId: string, hand: Hand, scan_ds_m: number): WindowBase {
  return {
...
function seal(w: SaveWindow): SaveWindow {
...
function computeWindow(
  ...
  cornerId: string,
  scan_ds_m: number
): Result<SaveWindow, CfRefusal | LinelabError> {
  ...
  if (ctx.detect === null) {
    ...
    return err(
      internal(
        "saveWindow",
        "corner carries a corrective block but no run_wide_detect event",
        "savewin_detect_event_missing",
        { corner_id: cornerId }
      )
    );
  }
  ...
  if (turnIn === undefined) {
    return err(
      internal(
        "saveWindow",
        "corner carries a detect but the line records no turn_in event",
        "savewin_turn_in_missing",
        { corner_id: cornerId }
      )
    );
  }
  ...
  let v_max = 0;
  for (const p of inDomain) v_max = Math.max(v_max, p.v);
  for (const t of [lo, hi]) {
    const at = recordedStateAt(traj, t, ctx.road);
    if (at !== null) v_max = Math.max(v_max, at.sample.v);
  }
  if (!(v_max > 0)) {
  ...
  if (step_s > HORIZON_TAU_QUANTUM_S + 1e-12) {
    return err({
      ...
      at: "scan_ds_m",
      message:
        `scan step ${scan_ds_m} m is ${step_s.toFixed(3)} s at the domain's fastest station ` +
  ...
  for (const t of [t_detect, t_shot, t_freeze_end]) {
    if (t >= lo - 1e-12 && t <= hi + 1e-12) taus.push(t);
  }
  taus.sort((a, b) => a - b);
  const grid: number[] = [];
  for (const t of taus) {
    if (grid.length === 0 || t - grid[grid.length - 1]! > 1e-9) grid.push(t);
  }
  ...
  for (let i = 0; i + 1 < verdicts.length; i++) {
    if (verdicts[i] !== verdicts[i + 1]) transition_count++;
  }
  ...
  for (let i = 0; i < verdicts.length; i++) {
    if (verdicts[i] && (i === 0 || !verdicts[i - 1])) open_count++;
  }
  ...
  while (k >= 0 && verdicts[k] !== true) k--; // band's trailing (last-true) index
  ...
  if (close === null) {
    return err(
      internal("saveWindow", "tau_close_s fell outside the recorded line", "savewin_close_unresolvable", {
        tau_close_s
      })
    );
  }
...
export function saveWindow(
  ...
  cornerId?: undefined,
  opts?: SaveWindowOptions
): Result<readonly SaveWindow[], CfRefusal | LinelabError>;
export function saveWindow(
  line: SaveWindowInput,
  cornerId?: string,
  opts?: SaveWindowOptions
): Result<SaveWindow | readonly SaveWindow[], CfRefusal | LinelabError> {
  ...
  if (cornerId !== undefined) return computeWindow(line, cornerId, scan_ds_m);
  const out: SaveWindow[] = [];
...
export function saveWindowScalarLines(w: SaveWindow): readonly string[] {
  const rows: string[] = [];
  if (w.tau_close_s !== undefined) rows.push(`tau_close_s ${horizonDisplay(w.tau_close_s)} s`);
  if (w.s_close_m !== undefined) rows.push(`s_close_m ${horizonDisplay(w.s_close_m)} m`);
  if (w.s_star_m !== undefined) rows.push(`s_star_m ${horizonDisplay(w.s_star_m)} m`);
  if (w.reaction_budget_s !== undefined && w.react_profile_s !== undefined) {
    rows.push(
      `reaction budget ${horizonDisplay(w.reaction_budget_s)} s vs react ${horizonDisplay(w.react_profile_s)} s`
    );
  }
  return rows;
}
 ...
 * verbatim.
 */
export function saveWindowSummary(w: SaveWindow): string {
  const sentence = SAVE_WINDOW_STATUS_SENTENCES[w.status].replace(
    "N times",
    `${w.transition_count} times`
  );
  const head = `${w.line_id} · ${w.corner_id} · ${w.status} — ${sentence}`;
  const disclosure =
    `rider ${w.rider} · predicate ${w.predicate} · policy target_phi ${horizonDisplay(w.policy.target_phi_deg)}° ` +
...
export function saveWindowSummaryAll(ws: readonly SaveWindow[]): string {
