...
import type { FigureRole, MistakeScope, MistakeSpec } from "../plan/types.js";
import {
  EXECUTION_MISTAKE_KINDS,
  MISJUDGMENT_MISTAKE_KINDS,
  MISTAKE_KIND_DEFS,
  MISTAKE_KINDS,
  RETIRED_MISTAKE_NAME,
  printMistakeToken,
  type ExecutionMistakeKind,
  type MisjudgmentMistakeKind,
  type MistakeKind
} from "../plan/mistakes.js";
...
import { solveMisjudgeSugar, type BeliefSugar } from "./believed.js";
import { buildLineResult, sealVerdict } from "./envelope.js";
import type { DiagnosisBlock, LineResult, LineSource, Verdict } from "./types.js";

// ---------------------------------------------------------------------------
// Compile context + result (design/03 §7's signature; the full first-class
// LineResult rides beside the six named members so callers — run.ts, the
// `mistake` verb — never re-run the engine to obtain the line itself)
...
// ---------------------------------------------------------------------------
// Param coercion — token params arrive as strings (plan/mistakes.ts leaves
// numeric coercion to the compiler, since param meaning is per-kind); JSON
// params may already be numbers. Unknown names are rejected (D8 — nothing is
// accepted-and-ignored).

type RawParams = Readonly<Record<string, number | string>> | undefined;

interface CoercedParams {
  readonly nums: Readonly<Record<string, number>>;
  readonly strs: Readonly<Record<string, string>>;
}

/** Params whose values stay strings (corner-id references). */
const STRING_PARAMS: readonly string[] = ["of"];

function coerceParams(kind: MistakeKind, params: RawParams): Result<CoercedParams> {
  ...
  for (const [name, value] of Object.entries(params ?? {})) {
    if (!known.includes(name)) {
      return err(
        schemaErr(`mistake.params.${name}`, `unknown param "${name}" for kind "${kind}" (known: ${known.join(", ")})`, "unknown_mistake_param", { kind, param: name })
      );
    }
...
function defaultOf(kind: MistakeKind, name: string): number {
...
function stripToProbeSpec(spec: SolveInput, cornerId: string): SolveInput {
  const {
    vis: _vis,
    vis_hold_f: _vh,
    vis_margin: _vm,
...
function buildCornerCtx(spec: SolveInput, cornerId: string): Result<SolveCtx> {
...
function minFOverCorner(m: RunMeasure, corner: Corner, s_ti: number): number {
  const lo = Math.min(s_ti, corner.s0);
  const hi = corner.s0 + 0.9 * (corner.s1 - corner.s0);
  let min = Number.POSITIVE_INFINITY;
  for (const sm of m.traj.samples) {
    if (sm.s < lo - 1e-9 || sm.s > hi + 1e-9) continue;
    if (sm.f < min) min = sm.f;
  }
  return min;
}

/** the run is still live at station s (not terminated before it). */
function liveAt(traj: Trajectory, s: number): boolean {
...
function probeKissLean(
  ...
  const measure = (leanDeg: number): number => {
    const ti: ResolvedPlanAction = {
      do: "turn_in",
      id: (plan[tiIndex] as ResolvedPlanAction & { id: string }).id,
      at_s: s_ti,
  ...
  let best: { lean: number; f: number } | null = null;
  let shots = 0;

  const fHi = measure(hi);
  shots += 1;
  if (fHi >= 0 && fHi <= KISS_TOL_F) return hi;
  if (fHi > KISS_TOL_F) return hi; // even full reserve cannot reach the inside
  // fHi < 0: phiReserve cuts inside — bisect down toward the kiss band
  while (shots < N_PROBE) {
    const mid = (lo + hi) / 2;
    const f = measure(mid);
    shots += 1;
    if (f >= 0 && f <= KISS_TOL_F) return mid;
    if (f < 0) hi = mid;
    else {
      lo = mid;
      if (best === null || f < best.f) best = { lean: mid, f };
    }
  }
...
function containedMeasure(m: RunMeasure): boolean {
  if (m.traj.terminated.reason !== "road_end") return false;
  for (const sm of m.traj.samples) {
    if (sm.f > 1 + eps_f_detect) return false;
  }
  return true;
}
...
function probeEasedLean(
  ...
  const measure = (leanDeg: number): boolean => {
    const ti: ResolvedPlanAction = {
      do: "turn_in",
      id: (plan[tiIndex] as ResolvedPlanAction & { id: string }).id,
      at_s: s_ti,
  ...
  for (let k = 1; k <= N_PROBE; k++) {
    ...
    if (measure(lean)) return lean;
    if (lean >= ctx.phi_reserve_deg) break;
  }
 ...
 */
export function facetLadder(kiss: number, eased: number, facets: number): readonly number[] {
  const nBites = Math.ceil(facets / 2);
  const rung = (k: number): number =>
    nBites === 1 ? eased : kiss + ((eased - kiss) * k) / (nBites - 1);
  const out: number[] = [];
...
function facetActionsFor(
  ladder: readonly number[],
  baseId: string,
  corner: Corner,
  s_ti: number,
  spanArcF: number
): readonly ResolvedPlanAction[] {
 ...
 */
function reachableFacetActions(
  ctx: SolveCtx,
  plan: readonly ResolvedPlanAction[],
  tiIndex: number,
  ladder: readonly number[],
  ...
  for (let k = 0; k < N_PROBE; k++) {
    ...
    if (fired >= ladder.length) return actions;
    actions = facetActionsFor(
      ladder,
      baseId,
      corner,
      s_ti,
      FACET_SPAN_ARC_F * Math.pow(FACET_COMPRESS_F, k + 1)
    );
  }
...
function turnInActionIdFor(events: readonly Event[], cornerId: string): string | null {
...
function cornerAt(corners: readonly Corner[], s: number): Corner | null {
  let match: Corner | null = null;
  for (const c of corners) {
    if (c.s0 <= s + 1e-12) match = c;
  }
  return match;
}
...
function resolveScope(
  scope: MistakeScope | undefined,
  corners: readonly Corner[],
  defaultCorner: Corner
): Result<readonly Corner[]> {
  if (scope === undefined) return ok([defaultCorner]);
  if (scope === "all_corners") return ok(corners);
  if (scope.length === 0) {
...
function withDiagnosis(line: LineResult, diagnosis: DiagnosisBlock | null): Result<LineResult> {
  if (diagnosis === null) return ok(line);
  const patched: Verdict = { ...line.verdict, diagnosis };
...
function execSpecOf(spec: SolveInput): SolveInput {
  const {
    vis: _vis,
    vis_hold_f: _vh,
    vis_margin: _vm,
...
export function compileMistake(
  kind: string,
  params: RawParams,
  ctx: MistakeCtx
): Result<CompiledMistake> {
  ...
  if (!(MISTAKE_KINDS as readonly string[]).includes(kind)) {
    return err({
      code: "UNKNOWN_ID",
      at: "mistake.kind",
  ...
  const k = kind as MistakeKind;

  // -- one control OR one belief, never both (D23) ----------------------------
  const isExecution = (EXECUTION_MISTAKE_KINDS as readonly string[]).includes(k);
  if (isExecution && ctx.spec.believed_road !== undefined) {
    return err(
      schemaErr("mistake", "one control or one belief, never both (D23)", "misjudge_with_execution_mistake")
    );
  }
  ...
  if (!isExecution && ctx.spec.believed_road !== undefined) {
    return err(
      schemaErr("mistake", "a sugar misjudgment beside believed_road is two beliefs — one belief only (D23)", "misjudge_double_belief")
    );
  }
  ...
  if (!isExecution) {
    return compileMisjudgment(k as MisjudgmentMistakeKind, coerced.value, ctx);
  }
  return compileExecution(k as ExecutionMistakeKind, coerced.value, ctx);
}
...
function compileMisjudgment(
  kind: MisjudgmentMistakeKind,
  params: CoercedParams,
  ctx: MistakeCtx
): Result<CompiledMistake> {
  const sugar: BeliefSugar = {
    kind,
  ...
  const cornerId =
    line0.source.kind === "misjudge" && line0.source.sugar !== null
...
function mapToStrings(params: Record<string, number | string>): Record<string, string> {
...
function relabel(
  line: LineResult,
  fields: { line_id: string; role: FigureRole; label: string }
): LineResult {
...
// ---------------------------------------------------------------------------
// Execution kinds — one contiguous replacement in exactly one channel

interface Perturbed {
  readonly plan: readonly ResolvedPlanAction[];
  readonly applied: readonly string[];
  /** rider rate cap the perturbation compiles to (slow_steer only) */
  readonly roll_rate_cap_dps?: number;
  /** entry-speed override (overspeed only) */
  readonly entry_kmh?: number;
  readonly diagnosisSeed: DiagnosisSeed;
}

type DiagnosisSeed =
  | { readonly kind: "plan_gap"; readonly at_s: number; readonly corner_id: string; readonly detail: Record<string, unknown> }
  | { readonly kind: "roll_rate_limited"; readonly at_s: number; readonly corner_id: string; readonly detail: Record<string, unknown> }
  | { readonly kind: "overspeed_entry"; readonly corner_id: string; readonly detail: Record<string, unknown> }
  | { readonly kind: "stand_up"; readonly cut_at_s: number; readonly corner_id: string; readonly detail: Record<string, unknown> };

function compileExecution(
  kind: ExecutionMistakeKind,
  params: CoercedParams,
  ctx: MistakeCtx
): Result<CompiledMistake> {
  ...
  if (!perturbedR.ok) return perturbedR;
  ...
  const resolvedParams: Record<string, number | string> = { ...params.nums, ...params.strs };
  ...
  if (!teachingCtxR.ok) return teachingCtxR;
...
function perturbPlan(
  ...
  ctx: MistakeCtx,
  corners: readonly Corner[],
  basePlan: readonly ResolvedPlanAction[],
...
function perturbChop(
  ...
  // target corner)
  const scopeIds =
    ctx.scope === undefined ? null : ctx.scope === "all_corners" ? corners.map((c) => c.id) : ctx.scope;
  let rollOn: ResolvedPlanAction | null = null;
  for (const a of basePlan) {
    if (a.do !== "throttle" || !(a.accel > 0)) continue;
    if (scopeIds !== null) {
      const c = cornerAt(corners, a.at_s);
      if (c === null || !scopeIds.includes(c.id)) continue;
    }
    if (rollOn === null || a.at_s > rollOn.at_s) rollOn = a;
  }
  if (rollOn === null) {
    return err(
      badRange("mistake", "chop needs a solved roll-on (a drive throttle action) to cut", "mistake_no_roll_on", {
  ...
  if (cutAt >= roadEnd) {
    return err(
      badRange("mistake.params.offset_m", `the cut at s=${cutAt.toFixed(1)} lies past road end (${roadEnd.toFixed(1)})`, "mistake_cut_past_road_end", { cut_at_s: cutAt, road_end_m: roadEnd })
    );
  }

  const cut: ResolvedPlanAction = {
    do: "throttle",
    id: `${rollOn.id}_chop`,
    at_s: cutAt,
    accel: 0,
...
function perturbTurnIn(
  kind: "premature" | "premature_contained" | "fifty_pence",
  params: CoercedParams,
  ctx: MistakeCtx,
  corners: readonly Corner[],
  basePlan: readonly ResolvedPlanAction[],
  ...
  if (!scopedR.ok) return scopedR;
  const scoped = scopedR.value;

  let plan: ResolvedPlanAction[] = [...basePlan];
  ...
  for (const corner of scoped) {
    ...
    if (actionId === null) {
      ...
      if (ctx.scope !== undefined && ctx.scope !== "all_corners") {
        return err(
          badRange("mistake.scope", `the base line has no turn_in for corner "${corner.id}"`, "mistake_no_turn_in_for_corner", { corner_id: corner.id })
        );
      }
      continue;
    }
    const tiIndex = plan.findIndex((a) => a.do === "turn_in" && a.id === actionId);
    if (tiIndex < 0) continue; // superseded by an earlier facet replacement
    ...
    if (tiGood.at_s <= EARLY_FLOOR_M) {
      return err(
        badRange("mistake.params.early_by_m", `the solved turn-in already sits at road start (s=${tiGood.at_s.toFixed(1)}) — no earlier placement exists`, "mistake_early_before_road_start", { early_by_m: early, s_ti_good: tiGood.at_s })
      );
    }
    const s_ti = Math.max(EARLY_FLOOR_M, tiGood.at_s - early);

    const cctxR = buildCornerCtx(ctx.spec, corner.id);
    if (!cctxR.ok) return cctxR;
    ...
    if (!liveAt(pre.traj, s_ti)) break;

    if (kind === "premature") {
      const lean =
        params.nums["lean_deg"] ??
        probeKissLean(cctx, plan, tiIndex, s_ti);
      plan = plan.map((a, i) =>
        i === tiIndex
          ? ({ do: "turn_in", id: tiGood.id, at_s: s_ti, target: { lean_deg: lean }, hand: corner.hand } as ResolvedPlanAction)
          : a
      );
      ...
      plan = plan.map((a, i) =>
        i === tiIndex
          ? ({ do: "turn_in", id: tiGood.id, at_s: s_ti, target: { lean_deg: eased }, hand: corner.hand } as ResolvedPlanAction)
          : a
      );
    } else {
  ...
  if (applied.length === 0) {
    return err(
      badRange("mistake", "no scoped corner received the perturbation (the base line never turned in before terminating)", "mistake_nothing_applied")
    );
  }
...
function buildDiagnosis(seed: DiagnosisSeed, traj: Trajectory): DiagnosisBlock {
  switch (seed.kind) {
    case "plan_gap":
    ...
    case "stand_up": {
      ...
      for (const sm of traj.samples) {
        if (sm.s < seed.cut_at_s - 1e-9) continue;
