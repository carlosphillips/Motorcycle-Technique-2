...
// ---------------------------------------------------------------------------
// The typed refusal (design/04 §4.1a: detail.sub_reason = "road_too_short",
...
export function roadTooShort(
  quantity: RoadTooShortQuantity,
  corner_id: string,
  required_m: number,
  available_m: number
): LinelabError {
...
// ---------------------------------------------------------------------------
// Reference lengths (§4.1a, formulas verbatim):
//
//   L_arc(n)  = s1(n) − s0(n)
//   L_app(n)  = s0(n) − max(0, s1(n−1))
//   L_exit(n) = min(road_end, s0(n+1)) − s1(n)
//   gap(n)    = s0(n+1) − s1(n)            (= L_app(n+1); null on the last)
...
export function refLengths(road: RoadModel, index: number): CornerRefLengths {
...
// ---------------------------------------------------------------------------
// Derived stations (per corner; refuses turn_in_sweep degeneration)
...
function sweepCandidates(lo: number, hi: number): readonly number[] {
  const span = hi - lo;
  const evenSpacing = span / (N_SWEEP - 1);
  const count =
    evenSpacing >= SWEEP_STEP_MIN_M
      ? N_SWEEP
      : Math.max(2, Math.floor(span / SWEEP_STEP_MIN_M) + 1);
  const out: number[] = [];
  for (let i = 0; i < count; i++) out.push(lo + (span * i) / (count - 1));
  return out;
}
 ...
 */
export function deriveStations(road: RoadModel, index: number): Result<DerivedStations> {
  const corner = road.corners[index];
  if (corner === undefined) {
    return err({
      code: "INTERNAL",
 ...
 */
export function vTargetMs(corner: Corner, skill: number, mu: number): number {
 ...
 */
export function decelBracketAt(
  d: DerivedStations,
  s_ti: number,
  v_entry_ms: number,
  v_target_ms: number
 ...
 */
export function rollOnBracketAt(d: DerivedStations, s_ti: number): Result<RollOnBracket> {
