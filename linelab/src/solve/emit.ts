 ...
 */
const FRACTION_KEYS: ReadonlySet<string> = new Set([
  "f",
  "grip",
  "grip_min",
  "n_long",
...
const PCT_KEYS: ReadonlySet<string> = new Set(["pct"]);
 ...
 */
const F_VALUED_BOUNDS: ReadonlySet<string> = new Set(["f_min", "f_max"]);

/** The keys of a ConstraintRow (and its `worst`) that carry the bound's quantity. */
const BOUND_VALUED_KEYS: ReadonlySet<string> = new Set(["value", "margin"]);
 ...
 */
export function emissionDpFor(key: string): number {
  if (PCT_KEYS.has(key)) return 1;
  if (FRACTION_KEYS.has(key) || key.endsWith("_f") || key.startsWith("f_")) return 3;
  return 2;
}

function roundScalar(x: number, dp: number): number {
 ...
 */
function walk(value: unknown, key: string, fValued: boolean): unknown {
  ...
  const rowFValued =
    fValued || (typeof bound === "string" && F_VALUED_BOUNDS.has(bound));
 ...
 */
export function roundEmission<T>(value: T): T {
