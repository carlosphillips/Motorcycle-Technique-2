...
import type { Corner, ResolvedPlanAction, Sample } from "../core/types.js";
...
export function wireRoadSpecOf(road: SolveSpec["road"]): RoadSpec {
...
export function composeSpecRoad(road: SolveSpec["road"]): Result<ComposedRoad> {
  return compose(wireRoadSpecOf(road));
}

/** A resolved plan re-spelled in wire (validate() input) form. */
export function wirePlanFromResolved(
  resolved: readonly ResolvedPlanAction[]
...
export function buildWireScenario(
  fields: WireScenarioFields,
...
// ---------------------------------------------------------------------------
// Execution with verdict extensions (holds / misjudgment): the sibling modules
// run everything through solve.ts's ONE executeLine pipeline, then patch the
// verdict members the pipeline has no input surface for and RE-SEAL through
// envelope.sealVerdict — rounding is idempotent and result_hash is recomputed
// over the patched rounded verdict, so the hash law (ARCHITECTURE §6.3) holds
// on the patched verdict exactly as on a directly-assembled one.
...
export function executeSolvedPlan(input: ExecuteExtInput): Result<LineResult> {
  const wire = buildWireScenario(
    { spec: input.spec, wireRoad: input.wireRoad, ...(input.entry_kmh !== undefined ? { entry_kmh: input.entry_kmh } : {}) },
    input.plan
  );
  ...
  if (input.holds === undefined && input.misjudgment === undefined) return ok(line);
...
export function patchAndReseal(
  line: LineResult,
  holds: readonly SightHold[] | undefined,
  misjudgment: MisjudgmentBlock | undefined
): Result<LineResult> {
...
// ---------------------------------------------------------------------------
// Chain context: one SolveCtx per selected corner (buildSolveContext with the
// corner pinned — deriveStations already computes the §4.1a chain arithmetic,
// L_app(n) = gap(n−1), through refLengths' prev-corner clamp).
...
function stripToChainSpec(spec: SolveInput, cornerId: string): SolveInput {
...
export function buildChainContext(spec: SolveInput): Result<ChainCtx> {
  ...
  if (spec.plan !== undefined && spec.plan.length > 0) {
    return err(
      noSolutionConflict(
        "plan",
        spec.plan[0]!.id,
        "the chained/double-apex/vis solvers build every candidate plan from scratch in v0.1 — the §4.9 merge is single-corner scope"
      )
    );
  }
  ...
  if (spec.corner !== undefined && spec.corner.includes("..")) {
    const [a, b] = spec.corner.split("..") as [string, string];
    ...
    indices = [];
    for (let i = ia; i <= ib; i++) indices.push(i);
  } else {
...
function speedAt(m: RunMeasure, s: number): number {
  let best: Sample | null = null;
  for (const sm of m.traj.samples) {
    if (best === null || Math.abs(sm.s - s) < Math.abs(best.s - s)) best = sm;
  }
  return best !== null ? best.v : 0;
}

/** contained (both sides) through station sEnd, and still alive past it. */
function containedThrough(m: RunMeasure, sEnd: number): boolean {
  for (const sm of m.traj.samples) {
    if (sm.s > sEnd + 1e-9) break;
    if (sm.f > 1 + eps_f_detect || sm.f < -EPS_INSIDE_F) return false;
  }
...
function exitFOf(m: RunMeasure, corner: Corner): number {
  const row = m.rows.find((r) => r.id === corner.id);
  if (row !== undefined) return row.exit.f;
  let best: Sample | null = null;
  for (const sm of m.traj.samples) {
    if (best === null || Math.abs(sm.s - corner.s1) < Math.abs(best.s - corner.s1)) best = sm;
  }
  return best !== null ? best.f : Number.POSITIVE_INFINITY;
}

// ---------------------------------------------------------------------------
// Per-corner turn-in + lean search (bounded engine probes — 04 §5.1 discipline)

interface CornerPick {
  readonly s_ti: number;
  readonly lean_deg: number;
  readonly exit_f: number;
...
function pickCorner(
  chain: ChainCtx,
  ctxIndex: number,
  planSoFar: readonly ResolvedPlanAction[],
  exitTargetF: number | null
): CornerPick | null {
  ...
  if (ctxIndex === 0) {
    ...
    const N = 8;
    candidates = [];
    for (let i = 0; i < N; i++) candidates.push(hi - ((hi - lo) * i) / (N - 1));
  }
  ...
  interface Probed {
    ...
    readonly exitF: number;
    readonly apexPct: number | null;
    readonly m: RunMeasure;
  }

  const scored: { pick: CornerPick; adequate: boolean }[] = [];
  ...
  for (const s_ti of candidates) {
    ...
    if (scored.some((s) => s.adequate)) break;
    if (scored.length >= CHAIN_CAND_KEEP && !isLast) break;
    const v_ti = Math.max(speedAt(measure0, s_ti), v_valid_min_ms * 0.5);

    const probe = (lean: number): Probed => {
      ...
      const extra: ResolvedPlanAction[] = [ti];
      if (isLast) {
      ...
      const plan: ResolvedPlanAction[] = [...planSoFar, ...extra].sort((a, b) => a.at_s - b.at_s);
      const m = measureRun(ctx, plan, true);
      const row = m.rows.find((r) => r.id === corner.id);
      const deepest =
        row !== undefined && row.apexes.length > 0
    ...
    const adequacy = (r: Probed): boolean => {
      if (!(r.minF >= 0 && r.minF <= DEPTH_BAR_F)) return false;
      if (isLast) return r.apexPct !== null && r.apexPct >= 45 && r.apexPct <= 90;
      return true;
    };
    let best: { lean: number; r: Probed; adequate: boolean } | null = null;
    for (let p = 0; p < N_PROBE; p++) {
      ...
      if (r.contained && r.minF >= -EPS_INSIDE_F) {
        ...
        if (adequate) break; // smallest adequate lean — stop the ladder
      }
      if (lean >= leanCap) break;
    }
  ...
  if (isLast) return pool[0]!.pick; // latest adequate candidate wins
  let best = pool[0]!.pick;
  for (const { pick } of pool) {
    if (Math.abs(pick.exit_f - exitTargetF) < Math.abs(best.exit_f - exitTargetF)) best = pick;
  }
  return best;
}

// ---------------------------------------------------------------------------
// The ascending decel scan (04 §5): gentlest fully-contained decel wins;
// slowing is the chain's one answer to a tight flip; the floor refusal names
// the first infeasible link.

interface ChainAttempt {
  readonly plan: readonly ResolvedPlanAction[];
  /** index (into chain.indices) of the first corner with no contained candidate; null = all placed */
  readonly failed_at: number | null;
  /** the failing corner's approach speed (m/s) — the flip diagnosis reads it */
  readonly v_at_failure: number | null;
}

function attemptChain(chain: ChainCtx, decel: number): ChainAttempt {
  ...
  for (let k = 0; k < chain.ctxs.length; k++) {
    ...
    if (k === 0 && decel > 0) {
      ...
      const brake: ResolvedPlanAction = {
        do: "brake",
        id: "b_chain",
      ...
      const crack: ResolvedPlanAction = {
        do: "throttle",
        id: "k_chain",
  ...
  if (bracketR.ok) {
    ...
    const g = (onset: number): number => {
      const ro: ResolvedPlanAction = {
        do: "throttle",
      ...
      const p: ResolvedPlanAction[] = [...plan, ro].sort((a, b) => a.at_s - b.at_s);
      const m = measureRun(last, p, true);
      if (!containedThrough(m, chain.road.total_len_m - 1e-6)) return Number.POSITIVE_INFINITY;
    ...
    for (let i = 0; i < 8; i++) {
      ...
      if (!Number.isFinite(f) || f > exit_target) a = mid; // too much drive — later onset
      else b = mid; // under target — earlier onset drives more
      if (b - a < 0.25) break;
    }
    ...
    const onset =
      bestOnset ?? Math.min(last.corner.s0 + 0.58 * (last.corner.s1 - last.corner.s0), last.corner.s1 - 1);
...
function lastTurnInStation(plan: readonly ResolvedPlanAction[], corner: Corner): number {
  for (let i = plan.length - 1; i >= 0; i--) {
    const a = plan[i]!;
    if (a.do === "turn_in" && a.id === `ti_${corner.id}`) return a.at_s;
  }
  return corner.s0;
}
...
function decelLadder(chain: ChainCtx): readonly number[] {
  const ctx0 = chain.ctxs[0]!;
  ...
  const out: number[] = [0, 0.6, 1.2, 1.8];
  for (let i = 0; i < 4; i++) out.push(DECEL_LO + ((DECEL_HI - DECEL_LO) * i) / 3);
  ...
  if (dFloor > DECEL_HI) {
    for (let i = 1; i <= 3; i++) out.push(DECEL_HI + ((dFloor - DECEL_HI) * i) / 3);
  }
  return out;
}

/** d_flip(v) = v·(φ_n + φ_{n+1})/roll_rate — the D27 flip budget, metres. */
export function dFlipM(
  v_ms: number,
  r_prev_m: number,
  r_next_m: number,
 ...
 */
export function chainedSolve(spec: SolveInput): Result<LineResult> {
  ...
  const failures: { attempt: ChainAttempt; decel: number }[] = [];
  const noteFailure = (attempt: ChainAttempt, decel: number): void => {
    const cur = failures[0];
    if (cur === undefined || (attempt.failed_at ?? -1) >= (cur.attempt.failed_at ?? -1)) {
      failures[0] = { attempt, decel };
    }
  };
  let bestFailing: LineResult | null = null;
  let bestContained: LineResult | null = null;
  let constraintViolation: readonly ConstraintRow[] | null = null;
  ...
  for (const decel of decelLadder(chain)) {
    ...
    if (line.verdict.ok) return ok(line); // gentlest CLEAN chain wins
    ...
    if (bestFailing === null) bestFailing = line;
  ...
  if (bestContained !== null) return ok(bestContained);
  if (chain.policy === "best_failing" && bestFailing !== null) return ok(bestFailing);
  ...
  if (lastFailure !== null && lastFailure.attempt.failed_at !== null && lastFailure.attempt.failed_at > 0) {
    ...
    if (prev.hand !== cur.hand) {
      ...
      return err(
        noSolution(
          ...
          "chainedSolve",
          `the ${prev.id}->${cur.id} hand flip cannot complete above the model-validity floor`,
          {
            link: `${prev.id}->${cur.id}`,
    ...
    return err(
      noSolution("empty_band", "chainedSolve", `no contained turn-in candidate exists for ${cur.id} at any scanned decel`, {
        corner_id: cur.id
      })
    );
  }
  return err(
    noSolution("non_clean_band", "chainedSolve", "chain candidates existed but none verified contained at full resolution", {})
  );
}
