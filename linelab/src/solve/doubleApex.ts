...
function windowOf(road: ComposedRoad, indices: readonly number[]): DaWindow {
...
function candidateWindows(road: ComposedRoad): readonly DaWindow[] {
  const out: DaWindow[] = [];
  let run: number[] = [];
  const flush = (): void => {
    if (run.length > 0) out.push(windowOf(road, run));
    run = [];
  };
  for (let i = 0; i < road.corners.length; i++) {
    ...
    if (run.length === 0) {
      ...
      if (prev.linked_next && prev.hand === c.hand) run.push(i);
      else {
        flush();
        run = [i];
      }
...
function qualifyWindow(w: DaWindow): Result<DaWindow> {
  if (w.sweep_deg < DA_SWEEP_MIN_DEG) {
    return err(
      noSolution("no_double_apex_geometry", "solveDoubleApex", "the targeted window does not reach the double-apex sweep qualification", {
...
function resolveWindow(road: ComposedRoad, corner: string | undefined): Result<DaWindow> {
  if (corner !== undefined) {
    if (corner.includes("..")) {
      const [a, b] = corner.split("..") as [string, string];
      ...
      for (let k = 0; k + 1 < indices.length; k++) {
        ...
        if (!cA.linked_next || cA.hand !== cB.hand) {
          ...
          return err(
            noSolution("no_double_apex_geometry", "solveDoubleApex", `corners ${cA.id} and ${cB.id} are not a same-hand linked pair`, {
              best_window_sweep_deg: w.sweep_deg,
              required_sweep_deg: DA_SWEEP_MIN_DEG
            })
          );
  ...
  let best: DaWindow | null = null;
  for (const w of windows) {
    if (best === null || w.sweep_deg > best.sweep_deg) best = w;
  }
  if (best === null || best.sweep_deg < DA_SWEEP_MIN_DEG) {
    return err(
      noSolution("no_double_apex_geometry", "solveDoubleApex", "no window reaches the double-apex sweep qualification", {
...
// ---------------------------------------------------------------------------
// Cumulative-sweep percent across the window (pct of swept angle, 04 §4.6)
...
export function sweepScale(w: DaWindow): SweepScale {
  ...
  const bounds: { s0: number; s1: number; a0: number; a1: number }[] = [];
  let acc = 0;
  ...
  const total = acc;
  const pctAt = (s: number): number => {
    if (s <= w.s0) return 0;
    if (s >= w.s1) return 100;
    for (const b of bounds) {
      ...
      if (s < b.s0) return (100 * b.a0) / total;
    }
    return 100;
  };
  const sAt = (pct: number): number => {
...
// ---------------------------------------------------------------------------
// The touch predicate (over the RECORDED apex list + the sample series)
...
export function touchesOf(
  apexes: readonly { readonly s: number; readonly f: number }[],
  ...
  const maxFBetween = (sA: number, sB: number): number => {
    let max = Number.NEGATIVE_INFINITY;
    for (const sm of samples) {
      if (sm.s <= sA || sm.s >= sB) continue;
      if (sm.f > max) max = sm.f;
    }
    return max;
  };
  ...
  for (const cand of candidates) {
    ...
    if (prominence < DA_PROMINENCE_NOISE) {
      ...
      if (cand.f < prev.f) merged[merged.length - 1] = cand;
      continue;
    }
  ...
  for (const t of merged) {
    ...
    if (prev !== undefined && t.pct - prev.pct < DA_TOUCH_SEP_PCT) {
      if (t.f < prev.f) touches[touches.length - 1] = t;
      continue;
    }
    touches.push(t);
  }
...
function windowApexes(m: RunMeasure, w: DaWindow): readonly { s: number; f: number }[] {
  const out: { s: number; f: number }[] = [];
  for (const c of w.corners) {
    const row = m.rows.find((r) => r.id === c.id);
    if (row === undefined) continue;
    for (const a of row.apexes) out.push({ s: a.s, f: a.f });
  }
...
interface DaShape {
  ...
  readonly ti1: number;
  readonly lean1: number;
  readonly ti2: number;
  readonly lean2: number;
  readonly rollOn: number | null;
}

function daPlan(shape: DaShape, w: DaWindow, brakeStart: number, brakeGap: number, entryV: number, firstHand: Corner["hand"]): readonly ResolvedPlanAction[] {
...
// ---------------------------------------------------------------------------
// solveDoubleApex (ARCHITECTURE §5)

interface DaCandidate {
  readonly shape: DaShape;
  readonly rank: number;
  readonly report: TouchReport;
  readonly contained: boolean;
  readonly worst_f: number;
  readonly searchRank: number;
  /** §4.6 filter step 3: constraints (§4.5) joined as always — coarse rows */
...
export function solveDoubleApex(spec: SolveInput): Result<LineResult> {
  ...
  const ctxFirst: SolveCtx = chain.ctxs[0]!;
  const ctxLast: SolveCtx = chain.ctxs[chain.ctxs.length - 1]!;
  ...
  // grids
  const decels: number[] = [];
  for (let i = 0; i < N_DA_DECEL; i++) decels.push(decelLoEff + ((DECEL_HI - decelLoEff) * i) / Math.max(1, N_DA_DECEL - 1));
  const ti1s: number[] = [];
  for (let i = 0; i < DA_GRID_N; i++) ti1s.push(sweepLo + ((sweepHi - sweepLo) * i) / (DA_GRID_N - 1));
  ...
  const ti2s: number[] = [];
  for (let i = 0; i < DA_GRID_N; i++) ti2s.push(ti2Lo + ((ti2Hi - ti2Lo) * i) / (DA_GRID_N - 1));

  const speedNear = (m: RunMeasure, s: number): number => {
  ...
  const leanSeed = (ctx: SolveCtx, v: number): number => {
    const c = ctx.corner;
  ...
  for (const decel of decels) {
    for (const ti1 of ti1s) {
      ...
      for (let p = 0; p < 4; p++) {
        ...
        for (const sm of m.traj.samples) {
          if (sm.s < w.s0 - 1e-9 || sm.s > scale.sAt(50)) continue;
      ...
      const preRun = measureRun(
        ctxFirst,
        daPlan({ decel, ti1, lean1, ti2: w.s1 + 1, lean2: 0, rollOn: null }, w, brakeStart, brakeGap, entryV, w.corners[0]!.hand)
          .filter((a) => a.id !== "ti_da2" && a.id !== "k_da2"),
        true
      );
      for (const ti2 of ti2s) {
        if (ti2 <= ti1 + 2) continue;
        searchRank += 1;
        const seed2 = leanSeed(ctxLast, speedNear(preRun, ti2));
        // lean #2 mini-ladder: first contained two-touch wins, else best effort
        let chosen: { shape: DaShape; m: RunMeasure } | null = null;
        for (let p = 0; p < 3; p++) {
          const lean2 = seed2 + 4 * p; // seed … seed+8
          const shape: DaShape = { decel, ti1, lean1, ti2, lean2, rollOn: null };
        ...
        const constraintRows =
          ctxFirst.constraints !== null ? evalConstraints(m.traj.samples, ctxFirst.constraints) : null;
        const constraintOk = constraintsSatisfied(constraintRows);
        const rank =
          report.two_touch
            ? Math.max(
                Math.abs(report.touches[0]!.pct - DA_APEX1_PCT),
                Math.abs(report.touches[1]!.pct - DA_APEX2_PCT)
              )
  ...
  const bestFailingOrder = coarse.filter((c) => c.constraint_ok).sort((a, b) => {
    const cls = (c: DaCandidate): number =>
      c.contained && c.report.two_touch ? 0 : c.contained && c.report.touches.length >= 1 ? 1 : 2;
    if (cls(a) !== cls(b)) return cls(a) - cls(b);
    if (a.worst_f !== b.worst_f) return a.worst_f - b.worst_f;
    return a.searchRank - b.searchRank;
  });

  const executeShape = (shape: DaShape): Result<LineResult> => {
    // fine: exit roll-on bisected against exit_target over [ti2+2, s1]
    let rollOn: number | null = null;
    {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        let breach = false;
        for (const sm of m.traj.samples) {
          if (sm.f > 1 + eps_f_detect) breach = true;
        }
        ...
        if (Number.isFinite(exitF) && exitF <= exit_target) {
          ...
          if (best === null || gap < best.gap) best = { onset: mid, gap };
        }
        if (!Number.isFinite(exitF) || exitF > exit_target) a = mid;
        else b = mid;
        if (b - a < 0.25) break;
      }
      rollOn = best !== null ? best.onset : null;
    }
    const daSpec: SolveInput = { ...spec, style: "double_apex", corner: `${w.corners[0]!.id}..${w.corners[w.corners.length - 1]!.id}` };
  ...
  let fineViolation: readonly ConstraintRow[] | null = null;
  let disagreement: { shape: DaShape; fine_outcome: string; fine_touches: number } | null = null;
  for (const cand of passing.slice(0, N_DA_FINE)) {
    ...
    if (line.verdict.outcome === "contained" && report.two_touch) return ok(line);
  ...
  if (disagreement !== null) {
    return err(
      noSolution("coarse_fine_disagreement", "solveDoubleApex", "a coarse two-touch candidate did not hold at full resolution", {
  ...
  return err(
    noSolution("no_two_touch_line", "solveDoubleApex", "the scan exhausted with no contained two-touch candidate at this entry", {
