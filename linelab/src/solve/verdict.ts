...
// ---------------------------------------------------------------------------
// Input shape — the seam between the run pipeline (WP-12) and this assembly.
// The trajectory is the FINAL merged record: analyzer events (apex/exit,
// sight_min, hazard_visible) and corrective events (run_wide_detect,
// correction) are already in `trajectory.events` (one event source, 05 §5).
...
function detectEvents(traj: Trajectory): readonly Event[] {
...
function attributeCorner(corners: readonly Corner[], s: number): Corner | null {
...
export function physicsOutcome(
  ...
  for (const d of detectEvents(traj)) {
    if (d.corner_id === undefined) {
      return err({
        ...
        at: "physicsOutcome",
        message:
          "run_wide_detect event carries no corner_id — 04 §4a.2 attributes every detect to a corner",
        detail: { reason: "detect_missing_corner_id" }
      });
    }
    ranWide.add(d.corner_id);
  }
  for (const c of correctives) ranWide.add(c.corner_id);
  ...
  for (const cornerId of ranWide) {
    ...
    if (block === undefined || !block.feasible) return ok("runoff");
  }
  ...
  if (traj.terminated.reason === "off_road") {
    ...
    if (terminal === null || !ranWide.has(terminal.id)) return ok("runoff");
  }

  if (ranWide.size > 0) return ok("wide");
  if (traj.terminated.reason === "stopped") return ok("stopped");
  return ok("contained");
}
...
function headlineFor(
  ...
  runoffCornerId: string | null,
  wideCornerId: string | null
): string {
  switch (outcome) {
    case "crash":
      return "crash";
...
function turnInRows(
  ...
  const rows: TurnInRow[] = [];
  for (const e of traj.events) {
    if (e.kind !== "turn_in" || e.corner_id !== cornerId) continue;
    const action = plan.find((a) => a.do === "turn_in" && a.id === e.action_id);
    const release = traj.events.find(
      (r) => r.kind === "release" && r.action_id === e.action_id
    );
    rows.push({
      s: e.s,
      lean_commit_deg:
        action !== undefined && action.do === "turn_in" ? action.target.lean_deg : 0,
      hand:
        action !== undefined && action.do === "turn_in"
          ? action.hand
 ...
 */
function validityDwell(samples: readonly Sample[]): ValidityBlock | null {
  let dwell = 0;
  for (let i = 0; i + 1 < samples.length; i++) {
    if (samples[i]!.below_validity) dwell += samples[i + 1]!.t - samples[i]!.t;
  }
...
export function assembleVerdict(input: VerdictInput): Result<Verdict> {
  ...
  const ranWideCorners = new Set<string>();
  for (const e of detectEvents(traj)) {
    if (e.corner_id !== undefined) ranWideCorners.add(e.corner_id);
  }
  for (const c of correctives) ranWideCorners.add(c.corner_id);

  const crashCorner =
    traj.terminated.reason === "crash"
      ? attributeCorner(input.road_corners, traj.terminated.s)
      : null;

  const corners: CornerVerdict[] = input.corner_rows.map((row) => {
    const corrective = byCorner.get(row.id) ?? null;
    const crashed = crashCorner !== null && crashCorner.id === row.id;
    const base: CornerVerdict = {
      id: row.id,
      hand: row.hand,
