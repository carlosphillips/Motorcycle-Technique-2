...
export type DecelDirective =
  ...
  | { readonly kind: "pinned"; readonly value: number; readonly action: BrakeAction };

export type RollOnDirective =
  | { readonly kind: "bisect" }
  | { readonly kind: "pinned"; readonly action: ThrottleAction }
  | { readonly kind: "magnitude"; readonly accel: number; readonly action: ThrottleAction };

export type TurnInDirective =
  | { readonly kind: "auto" }
  | { readonly kind: "pinned"; readonly action: TurnInAction };
...
function hasOnset(a: PlanAction): boolean {
 ...
 */
export function mergeAuthoredPlan(
  authored: readonly PlanAction[] | undefined,
  at = "plan"
): Result<MergeDirectives> {
  if (authored === undefined || authored.length === 0) return ok(EMPTY);

  let decel: DecelDirective = { kind: "bisect" };
  let roll_on: RollOnDirective = { kind: "bisect" };
  let turn_in: TurnInDirective = { kind: "auto" };
  const positions: PositionAction[] = [];
  const cracks: ThrottleAction[] = [];
    ...
    switch (action.do) {
      case "brake": {
        if (decel.kind === "pinned") {
          return err(
            noSolutionConflict(here, action.id, "a second authored brake leaves no decel control to search")
          );
        }
  ...
  const nothing_to_search =
    decel.kind === "pinned" && roll_on.kind === "pinned" && turn_in.kind === "pinned";
...
export function noSolutionConflict(at: string, action_id: string, why: string): LinelabError {
  return {
    code: "NO_SOLUTION",
    at,
...
export function constraintWithoutSolver(at: string): LinelabError {
  return {
    code: "INEFFECTUAL",
    at,
