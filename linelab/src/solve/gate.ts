...
import { MISTAKE_PIN_TABLE, type MistakePinRow } from "../plan/mistakes.js";
import type { MistakeScope } from "../plan/types.js";
...
// ---------------------------------------------------------------------------
// Options: rule 1's explicit declarations, keyed by line_id (produced by
// run.ts's expectDeclarationsOf over the loaded FigureSpec JSON).
...
function pinRowFor(kind: string, scope: MistakeScope | undefined): MistakePinRow | null {
  const wantChained = scope === "all_corners";
  let base: MistakePinRow | null = null;
  for (const row of MISTAKE_PIN_TABLE) {
    if (row.kind !== kind) continue;
    if (wantChained && row.scope === "all_corners") return row;
    if (row.scope === undefined) base = row;
  }
  return base;
}
...
function deriveExpectation(
  entry: LineEntry,
  envelope: FigureResult,
  declared: ExpectBlock | undefined
): GateExpectation {
...
function failedCheckIds(verdict: Verdict): readonly string[] {
  const out = new Set<string>();
  for (const c of verdict.doctrine.checks) {
    if (c.verdict === "fail") out.add(c.id);
  }
  return [...out].sort();
}
...
function evaluateLine(
  ...
  if (expectation.outcome !== null && !expectation.outcome.includes(v.outcome)) {
    misses.push(
      `outcome "${v.outcome}" outside the expected set {${expectation.outcome.join(", ")}}`
    );
  }
  ...
  return {
    ...
    refused: false,
    expectation,
    observed: { outcome: v.outcome as Outcome, quality: v.quality, failed_checks: failed },
    met: misses.length === 0,
    misses
  };
}
...
export function gateFigure(envelope: FigureResult, opts?: GateOptions): GateReport {
  const lines: GateLineReport[] = envelope.lines.map((entry) => {
