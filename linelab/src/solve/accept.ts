...
export const ACCEPT_POLICIES = ["clean", "best_failing"] as const;

export function validateAcceptPolicy(value: unknown, at: string): LinelabError | null {
  if (value === undefined) return null;
  if (value === "clean" || value === "best_failing") return null;
  return {
    code: "SCHEMA",
    at,
 ...
 */
export function corridorExcessM(samples: readonly Sample[], road: RoadModel): number {
  let worst = 0;
...
export function apexDistancePct(verdict: Verdict, corner_id: string): number {
  const row = verdict.corners.find((c) => c.id === corner_id);
  if (row === undefined || row.apexes.length === 0) return Number.POSITIVE_INFINITY;
...
export function compareCandidates(a: RankedCandidate, b: RankedCandidate): number {
  ...
  if (a.corridor_excess_m !== b.corridor_excess_m) return a.corridor_excess_m - b.corridor_excess_m;
  if (a.apex_distance_pct !== b.apex_distance_pct) return a.apex_distance_pct - b.apex_distance_pct;
 ...
 * search-time outcome).
 */
export function pickBestFailing(pool: readonly RankedCandidate[]): LineResult | LinelabError {
  if (pool.length === 0) {
    return {
      code: "NO_SOLUTION",
      at: "solve.accept",
...
export function acceptOnMistakeLine(at: string): LinelabError {
  return {
    code: "INEFFECTUAL",
    at,
