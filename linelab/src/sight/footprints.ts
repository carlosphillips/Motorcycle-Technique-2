...
import type { Hand, OccluderKind, ResolvedOccluder } from "../core/types.js";
 ...
 */
function anchorHand(road: ComposedRoad, at_s: number): Hand {
...
function bandFootprint(
  road: ComposedRoad,
  occ: ResolvedOccluder,
  kind: "hedge" | "wall" | "bank"
): OpaqueFootprint {
  ...
  for (let s = s0; s < s1; s += FOOTPRINT_STEP_M) stations.push(s);
  stations.push(s1);
 ...
 */
function vehicleCentreD(road: ComposedRoad, occ: ResolvedOccluder, width_m: number): number {
...
function vehicleFootprint(road: ComposedRoad, occ: ResolvedOccluder): OpaqueFootprint {
...
export function footprintOf(road: ComposedRoad, occ: ResolvedOccluder): OpaqueFootprint {
