...
import type { Event, ResolvedOccluder, Sample, SightTrend, Trajectory } from "../core/types.js";
...
function binarySearchFirstGE(samples: readonly Sample[], target: number, from: number): number {
  let lo = from;
  let hi = samples.length - 1;
  if (samples[hi]!.s < target) return -1; // beyond the last sample — caller clamps
  while (lo < hi) {
    const mid = (lo + hi) >> 1;
    if (samples[mid]!.s >= target) hi = mid;
    else lo = mid + 1;
  }
  return lo;
}
 ...
 */
function rebaseSightRide(samples: readonly Sample[]): readonly number[] {
...
function segmentUnblocked(eye: Vec2, target: Vec2, footprints: readonly OpaqueFootprint[]): boolean {
  for (const fp of footprints) {
    const poly = fp.polygon;
    const n = poly.length;
    for (let i = 0; i < n; i++) {
      if (segmentsCross(eye, target, poly[i]!, poly[(i + 1) % n]!)) return false;
    }
  }
  return true;
}
...
function isOnRoadVehicle(occ: ResolvedOccluder): boolean {
  return occ.kind === "vehicle" && (occ.lane !== undefined || occ.f !== undefined);
}

function hazardVisibleEvents(
  ...
  occluders.forEach((occ, idx) => {
    if (!isOnRoadVehicle(occ)) return;
    const own = all[idx]!;
    const others = all.filter((_, j) => j !== idx);
    for (const sample of samples) {
      const eye: Vec2 = { x: sample.x, y: sample.y };
...
function sightMinEvent(samples: readonly Sample[], sightRide: readonly number[]): Event | null {
...
export function analyzeSight(
  ...
  if (minEvent !== null) newEvents.push(minEvent);
...
function nearestIndexAtOrBefore(samples: readonly Sample[], targetS: number, upTo: number): number {
  if (targetS <= samples[0]!.s) return 0;
  let lo = 0;
  let hi = upTo;
  while (lo < hi) {
    const mid = (lo + hi + 1) >> 1;
    if (samples[mid]!.s <= targetS) lo = mid;
    else hi = mid - 1;
  }
  const before = lo;
 ...
 */
export function sightTrendAt(samples: readonly Sample[], index: number): SightTrend {
