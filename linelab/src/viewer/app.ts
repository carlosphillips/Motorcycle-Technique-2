...
import { hudAt, type HudModel } from "./hud.js";
 ...
 */
export function toggleSaveWindow(app: AppState): AppState {
  if (app.saveWindow !== null) return Object.freeze({ ...app, saveWindow: null });
 ...
 */
export function domainFor(app: AppState): ReturnType<typeof scenarioDomain> {
  return scenarioDomain(app.session.lines, app.stepper.axis);
}

function terminalBadge(app: AppState): string {
  const line = lineOf(app.session, app.session.focus);
  if (line === null) return "";
  const end = line.trajectory.terminated;
  const endValue = app.stepper.axis === "s" ? end.s : end.t;
  if (app.stepper.value < endValue) return "";
  return end.reason === "off_road" ? `off_road — ${OFF_ROAD_PLACARD}` : end.reason;
}

function legendOf(app: AppState): readonly LegendEntry[] {
  return app.session.lines.map((l) =>
    Object.freeze({
      line_id: l.line_id,
      role: l.role,
      label: l.label,
 ...
 */
export function frameOf(app: AppState): AppFrame {
  ...
  const saveRows =
    overlay === null || instant === null ? [] : saveWindowHudRows(overlay, instant.sample.t);
...
export function scrub(app: AppState, value: number): AppState {
  return Object.freeze({ ...app, stepper: scrubTo(app.stepper, value, domainFor(app)) });
}

export function togglePlay(app: AppState): AppState {
  return Object.freeze({ ...app, stepper: app.stepper.playing ? pause(app.stepper) : play(app.stepper) });
}

export function setSpeed(app: AppState, speed: number): AppState {
  return Object.freeze({ ...app, stepper: withSpeed(app.stepper, speed) });
}

export function tick(app: AppState, wallDeltaS: number): AppState {
  const line = lineOf(app.session, app.session.focus);
  return Object.freeze({
    ...app,
...
export function jumpToBookmark(app: AppState, token: string): AppState {
  const line = lineOf(app.session, app.session.focus);
  if (line === null) return app;
  const parsed = parseBookmark(token, line);
  if (!parsed.ok) return app;
...
export function hudHtml(rows: readonly HudRow[]): string {
  const cells = rows
    .map(
      (r) =>
        `<tr data-group="${r.group}"${r.badge === undefined ? "" : ` data-badge="${r.badge}"`}>` +
...
export function bookmarkOptionsHtml(bookmarks: readonly Bookmark[]): string {
  return bookmarks
    .map((b) => `<option value="${escapeHtml(printBookmark(b))}">${escapeHtml(b.label)} · t=${b.t.toFixed(2)} s</option>`)
    .join("");
}

export function legendHtml(entries: readonly LegendEntry[]): string {
  return entries
    .map(
      (e) =>
        `<option value="${escapeHtml(e.line_id)}"${e.focused ? " selected" : ""}>` +
 ...
 */
export function boot(host: ViewerHost, payloadText: string, engineSemver?: string): Result<AppHandle> {
  let spec: unknown;
  try {
  ...
  const paint = (): void => {
    ...
    host.byId("readout")?.setText(
      `${f.cursor.axis} = ${f.cursor.value.toFixed(2)}` +
  ...
  host.byId("bookmarks")?.on("change", () => {
    ...
    if (token !== "") apply(jumpToBookmark(current, token));
  });
  ...
  const cancel = host.every(1000 * FRAME_STEP_S, () => {
    ...
    last = now;
    if (current.stepper.playing) apply(tick(current, delta));
  });
