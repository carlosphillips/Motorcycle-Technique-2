...
import type { FigureResult, LineEntry, LineRefusal, LineResult } from "../solve/types.js";
 ...
 */
export function loadSession(spec: unknown, opts?: LoadOptions): Result<ViewerSession> {
...
export function sessionOf(figure: FigureResult): Result<ViewerSession> {
  const road = figure.road as ComposedRoad;
  ...
  const entries: readonly LineEntry[] = figure.lines;
  const lines = entries.filter((l): l is LineResult => !isLineRefusal(l));
  const refusals = entries.filter((l): l is LineRefusal => isLineRefusal(l));
...
export function withFocus(session: ViewerSession, lineId: string): ViewerSession {
  return Object.freeze({ ...session, focus: lineId });
}

export function lineOf(session: ViewerSession, lineId: string): LineResult | null {
  return session.lines.find((l) => l.line_id === lineId) ?? null;
}

export function focusedLine(session: ViewerSession): LineResult | null {
 ...
 * coincidence.
 */
export function stateInputFor(session: ViewerSession, line: LineResult): StateAtInput {
  return {
