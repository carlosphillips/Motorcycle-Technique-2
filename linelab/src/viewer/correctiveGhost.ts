...
import {
  ...
  correctiveShot,
  wideVsRunoff,
  type CorrectiveShotInput
} from "../solve/corrective.js";
import type { CfShadowDocument } from "../solve/counterfactual.js";
...
function inputOf(line: LineResult): CorrectiveShotInput {
 ...
 * thing under test.
 */
function clipAtStar(samples: readonly Sample[], s_star: number): readonly Sample[] {
  for (let i = 0; i < samples.length; i++) {
    if (samples[i]!.s >= s_star - 1e-9) return samples.slice(0, i + 1);
  }
  return samples;
}
 ...
 */
export function correctiveGhostOverlay(
  line: LineResult,
  scene?: DrawnScene
): Result<CorrectiveGhostOverlay | null> {
  const shot = correctiveShot(inputOf(line));
  if (!shot.ok) return shot as Result<CorrectiveGhostOverlay | null>;
  const { corrective, shadow } = shot.value;
  // toggle inert: no corner ran wide, or the shot never became integrable
  if (corrective === null || shadow === null) return ok(null);
  ...
  const path: OverlayPoint[] = clipped.map((p) => {
 ...
 */
export function correctiveGhostSvg(overlay: CorrectiveGhostOverlay | null): string {
  if (overlay === null || overlay.path.length === 0) return "";
  const num = (n: number): string => String(Number(n.toFixed(4)));
  let out =
    `<g data-overlay="corrective-ghost" data-line="${overlay.line_id}" ` +
