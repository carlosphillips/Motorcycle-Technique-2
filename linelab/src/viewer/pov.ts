...
import { renderPov, renderPovForFigure, POV_LOOK_MODES, type PovLook } from "../render/pov.js";
 ...
 */
export function parsePovLook(look: string | undefined): Result<PovLook> {
  if (look === undefined) return ok("heading");
  if ((POV_LOOK_MODES as readonly string[]).includes(look)) return ok(look as PovLook);
  return err({
    code: "SCHEMA",
    at: "look",
 ...
 */
export function renderPovView(
  road: ComposedRoad,
  line: LineResult,
  instant: InstantState | null,
  look: PovLook
): string {
  if (instant === null) return renderPovForFigure(road, [line], look);
