...
import { stateAt, type StateAtQuery } from "../core/stateAt.js";
...
function radiusText(kappa: number): string {
...
function profileRow(label: string, text: string, value: HudRow["value"]): HudRow {
  return Object.freeze({ group: "lean" as const, label, text, value, origin: "profile" as const, path: null });
}

function motionRows(i: InstantState): readonly HudRow[] {
  return [
...
function leanRows(i: InstantState, line: LineResult): readonly HudRow[] {
  ...
  if (i.derived.stand_up_dps !== 0) {
    rows.push(
      row("lean", "stand up", `${fixed(i.derived.stand_up_dps, 1)} °/s`, i.derived.stand_up_dps, "derived.stand_up_dps", "stand_up")
    );
  }
  ...
  rows.push(
    row(
      "lean",
      "a_noreturn",
      anr === null ? "brake ceiling at lean: —" : `brake ceiling at lean: ${fixed(anr, 1)} m/s²`,
      anr,
      "derived.a_noreturn_ms2"
    )
  );
  ...
  rows.push(
    row("lean", "a_widen", aw === null ? "—" : `${fixed(aw, 1)} m/s²`, aw, "derived.a_widen_ms2")
  );
  return rows;
}

function gripRows(i: InstantState): readonly HudRow[] {
  return [
...
function controlsRows(i: InstantState): readonly HudRow[] {
  ...
  const rows: HudRow[] = [
    ...
    row(
      "controls",
      "cmd_a → a_long",
      `${fixed(i.sample.cmd_a, 2)} → ${fixed(i.sample.a_long, 2)} m/s²`,
      i.sample.a_long,
      "sample.a_long",
      i.sample.clipped ? "clip" : undefined
    ),
    ...
    row(
      "controls",
      "action",
      i.sample.action_id === null ? "—" : `${i.sample.action_id}${action === null ? "" : ` (${action.do})`}`,
      i.sample.action_id,
      "sample.action_id"
    ),
...
function sightRows(i: InstantState): readonly HudRow[] {
...
function verdictRows(i: InstantState, line: LineResult): readonly HudRow[] {
  const rows: HudRow[] = [
 ...
 */
export function hudRowsOf(instant: InstantState, line: LineResult): readonly HudRow[] {
  return Object.freeze([
    ...motionRows(instant),
 ...
 */
export function hudAt(session: ViewerSession, lineId: string, query: StateAtQuery): Result<HudModel> {
 ...
 */
export function instantValueAt(instant: InstantState, path: string): unknown {
  let cursor: unknown = instant;
