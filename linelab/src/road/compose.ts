...
import type { Corner, CornerType } from "../core/types.js";
import type { ComposedRoad, RoadSpec, Segment, SegmentsRoadSpec, StationPoint } from "./types.js";
...
import type { CorridorParams } from "./corridor.js";
 ...
 */
export function normalizeRoadSpec(spec: RoadSpec): Result<NormalizedRoadSpec> {
...
function poseInSegment(g: SegGeom, ell: number): Pose {
...
function sweepBelowRUturn(seg: Segment): number {
  if (seg.type === "straight") return 0;
  if (seg.type === "arc") return seg.r_m <= R_UTURN_MAX ? seg.angle_deg : 0;
  const { r1_m: r1, r2_m: r2, angle_deg } = seg;
  if (r1 === r2) return r1 <= R_UTURN_MAX ? angle_deg : 0;
 ...
 */
export function cornerIdAtIndex(index: number): string {
  return `c${index + 1}`;
}

/** The corner ids a segment list mints, in order — `compose()`'s own rule, without composing. */
export function cornerIdsOf(segments: readonly Segment[]): readonly string[] {
  const out: string[] = [];
...
export function compose(
  ...
  for (const seg of segments) {
    ...
    const g: SegGeom = { seg, s0: s, len, x0: x, y0: y, psi0: psi };
    geoms.push(g);
  ...
  const poseAt = (station: number): Pose => {
  ...
  for (const g of geoms) {
    ...
    if (below >= SWEEP_UTURN_MIN) {
      return err({
        ...
        at: id,
        message:
          `corner ${id} accumulates ${below.toFixed(1)}° of sweep at r ≤ ${R_UTURN_MAX} m ` +
  ...
  const finalCorners: Corner[] = corners.map((c, i) => {
    const next = corners[i + 1];
    if (next === undefined) return Object.freeze({ ...c }); // last: gap absent, linked false
  ...
  }
  {
    ...
    stations.push(
      Object.freeze({ s: total_len_m, x: p.x, y: p.y, psi: p.psi, kappa: p.kappa })
    );
  }

  // -- corridor closures -----------------------------------------------------
  const corridor: CorridorParams = {
    lane_width_m,
    bike_margin_m,
    use_full_width,
  ...
  const worldAt = (station: number, d: number): { x: number; y: number } => {
    const p = poseAt(station);
    return { x: p.x + d * Math.sin(p.psi), y: p.y - d * Math.cos(p.psi) };
  };

  const project = (px: number, py: number): { s: number; d: number } => {
  ...
  const model: ComposedRoad = Object.freeze({
    lane_width_m,
    bike_margin_m,
    use_full_width,
    total_len_m,
    ...
    psi_road: (st: number) => poseAt(st).psi,
    kappa_road: (st: number) => poseAt(st).kappa,
    dOf: (f: number, st: number) => corridorDOf(corridor, f, st),
    fOf: (d: number, st: number) => corridorFOf(corridor, d, st),
