 ...
 * corner id.
 */
export function parseAnchorRef(ref: string, at: string): Result<CornerAnchor> {
  ...
  if (kind === "apex") {
    return err(
      schemaErr(
        at,
        `"${ref}": there is no apex anchor — the apex is measured, never authored (D7)`,
        "no_apex_anchor"
      )
    );
  }
  ...
  if (EMBEDDED_OFFSET_RE.test(idPart)) {
    return err(
      schemaErr(
        at,
        `"${ref}": station offset belongs in the <offset>x<span> token, not inside the anchor`,
        "anchor_embedded_offset"
      )
    );
  }
  return ok({ kind: kind as AnchorKind, corner_id: idPart });
}
 ...
 */
export function parseAnchorToken(token: string, at: string): Result<WireAnchor> {
...
function baseStation(kind: AnchorKind, corner: Corner): number {
...
export function resolveAnchor(
  ...
  if (typeof anchor !== "object" || anchor === null) {
    return err(
      schemaErr(at, `anchor must be an object — {at_s: <m>} or {ref: "entry|exit|mid:<cornerId>"}`, "anchor_malformed")
    );
  }
  if ("at_s" in anchor) return ok(anchor.at_s);
