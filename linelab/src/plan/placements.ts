...
import {
  OCCLUDER_BAND_DEFAULTS,
  VEHICLE_DEFAULTS,
  GRAVEL_DEFAULTS
} from "../road/constants.js";
import type { Occluder, Hazard, WireAnchor } from "./types.js";
...
function outOfScope(at: string, message: string, reason: string, detail?: Record<string, unknown>): LinelabError {
...
const BAND_KINDS = ["hedge", "wall", "bank"] as const;

export function resolveOccluder(
  raw: Occluder,
  corners: readonly Corner[],
  id: string,
  at: string
): Result<ResolvedOccluder> {
  if (raw.height_m !== undefined) {
    return err(
      outOfScope(at, "vertical geometry (height) is not modelled", "vertical_geometry_not_modelled")
    );
  }
  const at_s = resolveAnchor(raw.at, corners, `${at}.at`);
  if (!at_s.ok) return at_s;

  if (raw.kind === "vehicle") {
    if (raw.speed_kmh !== undefined) {
      return err(
        outOfScope(at, "vehicles are optical-only in v1 — motion fields are refused", "moving_hazards_not_modelled")
      );
    }
    ...
    if (forms !== 1) {
      return err(
        schemaErr(
          at,
          "a vehicle placement takes exactly one of lane, f, or side (D27)",
  ...
  const bandDefaults = OCCLUDER_BAND_DEFAULTS[raw.kind as (typeof BAND_KINDS)[number]];
  return ok({
    id,
    kind: raw.kind,
    side: raw.side,
 ...
 */
export function checkFullWidthOncoming(
  useFullWidth: boolean,
  placedInOncomingLane: boolean,
  at: string
): LinelabError | undefined {
  if (useFullWidth && placedInOncomingLane) {
    return outOfScope(
      at,
      "full_width and an oncoming-lane placement cannot both hold",
      "full_width_with_oncoming_traffic"
    );
  }
  return undefined;
}
...
function parseKeyVals(tokens: readonly string[], at: string): Result<Record<string, string>> {
...
const OCCLUDER_KIND_TOKENS = ["hedge", "wall", "bank", "vehicle"] as const;
const OCCLUDER_SIDE_TOKENS = ["inside", "outside", "left", "right"] as const;
const HAZARD_SIDE_TOKENS = ["inside", "outside", "left", "right", "center"] as const;
const VEHICLE_LATERAL_TOKENS = ["own", "oncoming", "inside", "outside", "left", "right"] as const;
 ...
 */
export function parseOccluderOrHazardToken(
  token: string,
  at: string
): Result<{ readonly occluder?: Occluder; readonly hazard?: Hazard }> {
  ...
  if (!isGravel && !(OCCLUDER_KIND_TOKENS as readonly string[]).includes(kind)) {
  ...
  if (side === undefined || !(validSides as readonly string[]).includes(side)) {
    return err(
      schemaErr(at, `"${kind}" needs a side ∈ ${validSides.join("|")} (got "${side}")`, "placement_side_missing")
    );
  }
  ...
  if (m === null) {
    return err(
      schemaErr(at, `"${kind}" needs an <offset>x<span> token (got "${spanTok ?? ""}")`, "placement_offset_span_missing")
    );
  }
  ...
  const anchorWithOffset: WireAnchor =
    "at_s" in anchor.value ? { at_s: anchor.value.at_s + offset } : { ref: anchor.value.ref, offset_m: offset };

  if (isGravel) {
    const hazard: Hazard = {
      kind: "gravel",
      side: side as (typeof HAZARD_SIDE_TOKENS)[number],
      at: anchorWithOffset,
      span_m: span,
  ...
  const occluder: Occluder = {
    kind: kind as "hedge" | "wall" | "bank",
    side: side as (typeof OCCLUDER_SIDE_TOKENS)[number],
    at: anchorWithOffset,
    span_m: span,
...
function parseVehicleToken(tokens: readonly string[], at: string): Result<{ readonly occluder: Occluder }> {
  ...
  if (fMatch === null && !(VEHICLE_LATERAL_TOKENS as readonly string[]).includes(lateral)) {
