...
export const MISJUDGMENT_MISTAKE_KINDS = ["underread", "overread"] as const;
export type MisjudgmentMistakeKind = (typeof MISJUDGMENT_MISTAKE_KINDS)[number];

export const MISTAKE_KINDS = [...EXECUTION_MISTAKE_KINDS, ...MISJUDGMENT_MISTAKE_KINDS] as const;
...
// ---------------------------------------------------------------------------
// Per-kind param + default surface (design/03 §7.1/§7.4, ALL TUNING)
...
export const MISTAKE_KIND_DEFS: Readonly<Record<MistakeKind, MistakeKindDef>> = Object.freeze({
  premature: Object.freeze({
    ...
    perturbation:
      "solved turn_in replaced by one placed early_by_m earlier, target = the committed (largest inside-kissing) lean, derived by engine probe",
...
// ---------------------------------------------------------------------------
// The ONE pin table (design/03 §7.1) — rows {kind, scope?, admissible_outcomes,
// fixture, fixture_pin, expect_fail?, book_figure, teaches}. Pins are EMPIRICAL
// (rule 4): a pin that stops holding is an engine bug or mis-tune, never patched
// by editing the pin.
...
export const MISTAKE_PIN_TABLE: readonly MistakePinRow[] = Object.freeze([
  Object.freeze({
    kind: "premature",
    admissible_outcomes: Object.freeze(["wide", "runoff"] as const),
    fixture: "F-ORACLE-90",
    fixture_pin: "runoff",
    book_figure: "fig 8.1",
  ...
  Object.freeze({
    kind: "premature_contained",
    admissible_outcomes: Object.freeze(["contained"] as const),
    fixture: "F-ORACLE-90",
  ...
  Object.freeze({
    kind: "overspeed",
    admissible_outcomes: Object.freeze(["wide", "runoff", "crash"] as const),
    fixture: "F-ORACLE-DR",
    fixture_pin: "runoff",
    book_figure: "fig 8.4",
...
// ---------------------------------------------------------------------------
// Mistake spec (JSON) — design/03 §7.2: {kind, params?, scope?}
...
function printCompactToken(kind: string, params?: Record<string, string>, scope?: string): string {
  let s = kind;
  if (params !== undefined && Object.keys(params).length > 0) {
    s += ":" + Object.entries(params).map(([k, v]) => `${k}=${v}`).join(",");
  }
  if (scope !== undefined) s += `@${scope}`;
  return s;
}
 ...
 */
function rewriteLegacySpelling(token: string): string | undefined {
  ...
  for (const part of parts.slice(1)) {
    ...
    else kv.push(`${m[1]}=${m[2]}`);
  }
  return printCompactToken(kind, kv.length > 0 ? Object.fromEntries(kv.map((p) => p.split("=") as [string, string])) : undefined, scope);
}
 ...
 * meaning is per-kind).
 */
export function parseMistakeToken(token: string, at: string): Result<ResolvedMistakeSpec> {
  if (/\s/.test(token)) {
    const rewrite = rewriteLegacySpelling(token);
    return err(
      schemaErr(
        at,
        `the legacy space-separated mistake spelling is rejected` + (rewrite !== undefined ? ` — use "${rewrite}"` : ""),
        "mistake_token_legacy_spelling",
        rewrite !== undefined ? { rewrite } : undefined
      )
    );
  }
  ...
  if (kind === undefined || !(MISTAKE_KINDS as readonly string[]).includes(kind)) {
    return err({
      code: "UNKNOWN_ID",
      at,
...
export function printMistakeToken(spec: ResolvedMistakeSpec): string {
