...
export const CHECK_LEXICON: Readonly<Record<CheckId, CheckPhrasing>> = Object.freeze({
  late_apex: {
 ...
 */
export function riderMessage(id: CheckId, metrics: Readonly<Record<string, unknown>> | undefined): string | null {
  ...
  if (id === "late_apex") {
    ...
    const bar = num("bar");
    if (pct === null || bar === null) return null;
  ...
  if (id === "single_input") {
    ...
    if (count === null || allowed === null) return null;
    if (count === 0) return "no steering input was commanded inside this corner at all";
    if (count === 1 && allowed >= 1) return "one committed steering input, which is what the corner asks for";
...
export function checkLexiconRows(): readonly (CheckPhrasing & { readonly id: CheckId })[] {
