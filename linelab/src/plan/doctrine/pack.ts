 ...
 * of these arrives.
 */
export const STRUCK_CHECK_IDS: Readonly<Record<string, string>> = Object.freeze({
 ...
 */
export const KNOWN_BOOK_CITES: readonly string[] = Object.freeze([
 ...
 * exists to reject.
 */
export const TUNING_MARKED_NAMES: readonly string[] = Object.freeze([
 ...
 */
export function scanPackProvenance(json: unknown, atPrefix = "pack"): Result<number> {
  let count = 0;
  const walk = (
    node: unknown,
    path: string,
    ...
    if (src !== undefined) {
      ...
      if (typeof src !== "string" || (src !== "TUNING" && !src.startsWith("book:"))) {
        return schemaErr(
          `${path}.source`,
          `${inCheck}source must be "TUNING" or "book:<cite>" — got ${JSON.stringify(src)}`,
          "source_unresolved",
          { check_id: owner, name: parentKey, source: src }
        );
      }
      if (src.startsWith("book:")) {
        ...
        if (!KNOWN_BOOK_CITES.includes(cite)) {
          return schemaErr(
            `${path}.source`,
            `${inCheck}book citation does not resolve in the committed book_text/ extraction: ${cite}`,
            "source_unresolved",
            { check_id: owner, name: parentKey, source: src, cite }
          );
        }
        if (parentKey !== null && TUNING_MARKED_NAMES.includes(parentKey)) {
          return schemaErr(
            `${path}.source`,
            `${inCheck}${parentKey} is marked TUNING in the design of record and may never carry a book: source`,
            "source_unresolved",
            { check_id: owner, name: parentKey, source: src, cite }
          );
        }
...
export function loadRubricPack(json: unknown): Result<RubricPack> {
  if (!isRecord(json)) {
  ...
  if (json["pack"] !== "linelab-rubric/1") {
    return err(
      schemaErr("rubric.pack", 'pack wire-format must be "linelab-rubric/1"', "bad_pack_literal")
    );
  }
  ...
  if (typeof rcv !== "number" || !Number.isInteger(rcv)) {
    return err(
      schemaErr("rubric.requires_checks_version", "requires_checks_version must be an integer", "type_mismatch")
    );
  }
  if (rcv !== CONFIG_CHECKS_VERSION_DEFAULT) {
    return err(
      schemaErr(
        "rubric.requires_checks_version",
        `pack requires checks_version ${rcv} but this engine implements checks_version ${CONFIG_CHECKS_VERSION_DEFAULT}`,
        "checks_version_mismatch",
        { pack_requires: rcv, engine_implements: CONFIG_CHECKS_VERSION_DEFAULT }
      )
    );
  }
  ...
  for (let i = 0; i < checksJson.length; i++) {
    ...
    if (!(CHECK_IDS as readonly string[]).includes(id)) {
      ...
      return err(
        unknownIdErr(`${rowPath}.id`, `unknown check id: ${id} (no evaluator in checks_version ${CONFIG_CHECKS_VERSION_DEFAULT})`, "unknown_check_id", { id })
      );
    }
    ...
    if (metric !== CHECK_METRIC[checkId]) {
      return err(
        schemaErr(
          `${rowPath}.metric`,
          `check ${id} binds metric ${String(metric)}; checks_version ${CONFIG_CHECKS_VERSION_DEFAULT} computes it from ${CHECK_METRIC[checkId]}`,
          "metric_binding_mismatch",
          { id, bound: metric, expected: CHECK_METRIC[checkId] }
        )
      );
    }
    const scope = row["scope"];
    if (typeof scope !== "string" || !(CHECK_SCOPES as readonly string[]).includes(scope)) {
    ...
    if (scope !== CHECK_SCOPE[checkId]) {
      return err(
        schemaErr(
          `${rowPath}.scope`,
          `check ${id} is ${CHECK_SCOPE[checkId]}-scope in checks_version ${CONFIG_CHECKS_VERSION_DEFAULT}`,
          "scope_mismatch",
          { id, bound: scope, expected: CHECK_SCOPE[checkId] }
        )
      );
    }
    ...
    for (const k of Object.keys(applicability)) {
      if (!(APPLICABILITY_KEYS as readonly string[]).includes(k)) {
        return err(
          schemaErr(`${rowPath}.applicability.${k}`, `unknown applicability key: ${k}`, "unknown_applicability_key", { key: k })
        );
      }
    }
    ...
    for (const [tName, tEntry] of Object.entries(thresholds)) {
      ...
      if (typeof value !== "number" || !Number.isFinite(value)) {
        ...
        return err(
          schemaErr(
            `${rowPath}.thresholds.${tName}.value`,
            `threshold ${tName} must bind a finite scalar, not an expression`,
            "pack_defines_rider",
            { name: tName }
          )
        );
    ...
    for (const tName of CHECK_THRESHOLDS[checkId]) {
      if (!(tName in thresholds)) {
        return err(
          schemaErr(
            `${rowPath}.thresholds`,
            `check ${id} must bind threshold ${tName} — checks_version ${CONFIG_CHECKS_VERSION_DEFAULT} arithmetic consumes it`,
            "thresholds_incomplete",
            { id, missing: tName }
          )
        );
      }
    ...
    for (const [bName, bVerdict] of Object.entries(bands)) {
      if (bVerdict !== "pass" && bVerdict !== "warn" && bVerdict !== "fail") {
        return err(
          schemaErr(`${rowPath}.bands.${bName}`, `band verdict must be pass|warn|fail, got ${String(bVerdict)}`, "bad_band_verdict")
        );
      }
    }
    for (const token of CHECK_BANDS[checkId]) {
      if (!(token in bands)) {
        return err(
          schemaErr(`${rowPath}.bands`, `bands must map every ${id} band token; missing "${token}"`, "bands_incomplete", { id, missing: token })
        );
      }
    }
    ...
    checks.push({
      ...
      scope: scope as RubricCheck["scope"],
      severity: severity as RubricCheck["severity"],
      applicability: applicability as RubricCheck["applicability"],
      thresholds: thresholds as RubricCheck["thresholds"],
      bands: bands as RubricCheck["bands"],
      teaches,
      book_ref: bookRef
    });
  }
  ...
  for (const [oldId, newId] of Object.entries(renamesJson)) {
    if (typeof newId !== "string" || !checkIds.has(newId)) {
      return err(
        unknownIdErr(`rubric.renames.${oldId}`, `rename successor ${String(newId)} is not a shipped check id`, "unknown_check", { old_id: oldId, new_id: newId })
      );
    }
  }
  const renames = renamesJson as Record<string, string>;
  ...
  if (annexJson === undefined || reserveJson === undefined) {
    return err(
      schemaErr(
        "rubric.annex.reserve_checks",
        "a pack without the reserve annex cannot be asked for a standing; it is refused, never defaulted",
        "reserve_checks_missing"
      )
    );
  }
  ...
  if (reserveJson.length === 0) {
    return err(
      schemaErr("rubric.annex.reserve_checks", "an empty reserve annex would make the top rung vacuous; it is rejected rather than shipped", "reserve_checks_empty")
    );
  }
  for (const member of reserveJson as string[]) {
    const successor = renames[member];
    if (successor !== undefined) {
      return err(
        unknownIdErr(
          "rubric.annex.reserve_checks",
          `${member} was renamed to ${successor} in checks_version ${CONFIG_CHECKS_VERSION_DEFAULT}`,
          "renamed_check",
          { id: member, successor }
        )
      );
    ...
    if (!checkIds.has(member)) {
      return err(
        unknownIdErr(
          "rubric.annex.reserve_checks",
          `pack ${name}/${version} declares reserve check ${member}, which is not in its check id set`,
          "unknown_reserve_check",
          { pack: `${name}/${version}`, id: member }
        )
      );
    }
  }
  ...
  const pack: RubricPack = {
    ...
    checks,
    renames,
    annex: { reserve_checks: reserveJson as readonly string[] }
  };
...
export function rubricString(pack: RubricPack): string {
 ...
 */
export function loadShippedRubricPack(name: string): Result<RubricPack> {
  if (name !== CONFIG_RUBRIC_DEFAULT) {
    return err(
      unknownIdErr("config.rubric", `unknown rubric: ${name} (v0.1 ships exactly "${CONFIG_RUBRIC_DEFAULT}")`, "unknown_rubric", { name })
    );
  }
 ...
 */
export function validateContinuationPackData(json: unknown): Result<number> {
  if (!isRecord(json)) {
  ...
  if (!Array.isArray(ladder) || ladder.length !== K_MEMBERS) {
    return err(
      schemaErr(
        "prior.ladder",
        `ladder must carry exactly K_MEMBERS = ${K_MEMBERS} rungs, got ${
          Array.isArray(ladder) ? ladder.length : String(typeof ladder)
        }`,
        "ladder_cardinality_mismatch",
        { expected: K_MEMBERS, got: Array.isArray(ladder) ? ladder.length : null }
      )
    );
  }
  ...
  if (typeof kMax !== "number" || typeof kStep !== "number") {
    return err(
      schemaErr(
        "prior.envelope",
        "envelope must bind kappa_max_1pm and kappa_step_max_1pm scalar values",
        "type_mismatch"
      )
    );
  }
  if (kStep < kMax) {
    return err(
      schemaErr(
        "prior.envelope.kappa_step_max_1pm",
        `kappa_step_max_1pm (${kStep}) must be >= kappa_max_1pm (${kMax}) — the coupled constants move together (design/03 §7a.3)`,
        "kappa_step_below_kappa_max",
        { kappa_step_max_1pm: kStep, kappa_max_1pm: kMax }
      )
    );
  }
  return prov;
 ...
 */
export function resolveCheckId(pack: RubricPack, id: string): Result<string> {
  if (pack.checks.some((c) => c.id === id)) return ok(id);
  const successor = pack.renames[id];
  if (successor !== undefined) {
    return err(
      unknownIdErr(
        id,
        `${id} was renamed to ${successor} in checks_version ${pack.requires_checks_version}`,
        "renamed_check",
        { id, successor }
      )
    );
