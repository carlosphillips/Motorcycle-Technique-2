...
import { FLAG_MAPPINGS, type FlagMapping } from "../args.js";
import { deferredFor, DEFERRED_TABLE } from "../deferred.js";
import {
  SWEEP_DEFAULT_METRICS,
  SWEEP_MAX_CELLS,
  SWEEP_METRICS,
  SWEEP_ROOTS,
  type SweepMetric,
...
// ---------------------------------------------------------------------------
// The wrapper shape (design/08 §5.1.1, verbatim)
...
export const ALL_SECTIONS = [...SHIPPED_SECTIONS, "continuations"] as const;
...
function field(
  ...
  effect: string,
  schema_ref: string,
  extra?: Partial<Omit<SchemaField, "name" | "type" | "effect" | "schema_ref">>
): SchemaField {
  return { name, type, effect, schema_ref, ...extra };
}

function scenarioSection(): SchemaSection {
  return {
    name: "scenario",
    prose:
      "A wire Scenario ({spec:\"linelab/1\", id, road, occluders?, hazards?, rider, config?, expect_fail?, meta?}) is the explicit-plan input validate() accepts (design/03 §6). rider.start.speed_kmh is required, no default.",
    fields: [
...
function planSection(): SchemaSection {
  return {
    name: "plan",
    prose:
      "design/03 §6.1 plan actions: brake, turn_in, throttle, position — each anchored via {at_s} or {ref, offset_m?} (the D32 anchor grammar).",
    fields: [
...
function roadDslSection(): SchemaSection {
  const grammar: SchemaGrammarRule[] = [
  ...
  const presetFields: SchemaField[] = PRESET_NAMES.map((name) => {
  ...
  return {
    name: "road-dsl",
    prose:
      "One line, `|`-separated segments; `lane` appears exactly once, first. Presets print their full expansion at the default (book-ink) hand.",
    fields: presetFields,
    grammar
  };
}

function occludersSection(): SchemaSection {
  return {
    name: "occluders",
...
function hazardsSection(): SchemaSection {
  return {
    name: "hazards",
...
function mistakesSection(): SchemaSection {
  const kinds: SchemaKind[] = MISTAKE_KINDS.map((kind) => {
    ...
    return {
      ...
      params: def.params.map((p) =>
        field(p.name, "number|string", p.note, "mistakes", {
  ...
  return {
    name: "mistakes",
    prose:
      "design/03 §7.1's pin table, printed verbatim from the single machine-readable source plan/mistakes.ts — the schema, the compiler, and the oracle can never disagree.",
    kinds,
    grammar: [
...
function solveSection(): SchemaSection {
  return {
    name: "solve",
 ...
 */
function sweepSection(): SchemaSection {
  const holdFixed: Readonly<Record<SweepRoot, string>> = {
  ...
  const metricSourcing: Readonly<Record<SweepMetric, string>> = {
  ...
  return {
    name: "sweep",
    prose:
      "design/08 §4.3: `sweep` addresses the whole composed input through a CLOSED root set (D34) — never bare array indices — and reports one row per grid cell. 1–2 --params; grids larger than sweep_max_cells = " +
    ...
    fields: [
      ...SWEEP_ROOTS.map((root) =>
        field(`${root}.`, "sweep-root", holdFixed[root], "sweep")
      ),
      ...SWEEP_METRICS.map((m) =>
        field(m, "metric", metricSourcing[m], "sweep", {
...
function sceneSection(): SchemaSection {
  return {
    name: "scene",
    prose:
      "design/04 §7's scene-text grammar (D30 sugar over FigureSpec JSON): top-level keys road/lines/occluders/hazards/marks/labels/view/note; `lines:`/`labels:` entries indented. lowerScene is the pure, total lowering.",
    grammar: [
...
function figureSection(): SchemaSection {
  return {
    name: "figure",
    prose:
      "design/03 §8/D30 — the canonical FigureSpec JSON: {road, occluders?, hazards?, lines: [{name, role, spec}], labels?, marks?, view?, note?}. Scene text lowers onto this exact shape.",
    fields: [
...
function viewSection(): SchemaSection {
  return {
    name: "view",
    prose:
...
function envelopeSection(): SchemaSection {
  return {
    name: "envelope",
    prose:
      "design/05 — output shapes. Sample field order (= trace CSV column order), the closed event-kind set, the closed outcome/phase sets, and the closed 8-code error vocabulary, each printed verbatim from their one owning declaration.",
    fields: [
...
function rubricSection(pack: import("../../plan/doctrine/types.js").RubricPack): SchemaSection {
  return {
    name: "rubric",
    ...
    fields: (CHECK_IDS as readonly CheckId[]).map((id) => {
...
function cliSection(): SchemaSection {
  return {
    name: "cli",
    prose:
      "The cross-surface spelling table (design/08 §4.1): one row per wire field, bijective with the runtime flag parser (cli/args.ts FLAG_TABLE — one source, so this section and the parser can never disagree).",
    flags: FLAG_MAPPINGS
  };
}
...
function allSections(): Record<ShippedSection, SchemaSection> {
...
export interface SchemaDocError {
  ...
  readonly detail: { readonly reason: string; readonly sections?: readonly string[] };
}
...
export function buildSchemaDoc(
  section?: string
): { ok: true; value: SchemaDoc } | { ok: false; error: SchemaDocError } {
  const packR = loadShippedRubricPack(CONFIG_RUBRIC_DEFAULT);
  ...
  if ((SHIPPED_SECTIONS as readonly string[]).includes(section)) {
    const one = { [section]: sections[section as ShippedSection] };
    return {
      ok: true,
