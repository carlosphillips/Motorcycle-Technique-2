...
import type { Hand, RiderProfileName } from "../core/types.js";
...
import { parseMistakeToken, type ResolvedMistakeSpec } from "../plan/mistakes.js";
...
// ---------------------------------------------------------------------------
// The zero-file composition draft (a superset of SolveInput's flat fields,
// design/08 §4.1's zero-file flag set).
...
// ---------------------------------------------------------------------------
// The flag table — `FlagMapping = {field, scene_key, flag, sugar?}`
// (design/08 §5.1.1). `scene_key: ""` = the field has no scene-text spelling
// (plan-channel authoring and CLI-only knobs — recorded as a deviation, since
// design/04 §7's scene grammar (plan/scene.ts RIDE_KEYS) genuinely does not
// carry these keys).
...
function parseNum(value: string, at: string): Result<number> {
  const n = Number(value);
  if (!Number.isFinite(n)) return err(schemaErr(at, `expected a number, got "${value}"`, "cli_number_malformed"));
  return ok(n);
}

/** `<road DSL> | preset <name> [hand=L|R] | <bare preset name>` (design/08 §4.1). */
function parseRoadRefFlag(value: string, at: string): Result<RoadSpec> {
  const tokens = value.trim().split(/\s+/).filter((t) => t.length > 0);
  const first = tokens[0];
  if (first === undefined) return err(schemaErr(at, "empty road reference", "road_ref_missing"));
  if (first === "preset") {
    const name = tokens[1];
    if (name === undefined) return err(schemaErr(at, '"preset" needs a name', "road_ref_preset_name_missing"));
    const hand = tokens[2];
    if (hand !== undefined) {
      const m = /^hand=([LR])$/.exec(hand);
      if (m === null) return err(schemaErr(at, `malformed road option "${hand}"`, "road_ref_option_malformed"));
      return ok({ preset: name, hand: m[1] as Hand });
    }
...
function parseConstraintFlag(value: string, at: string): Result<Constraint> {
  const TOKEN_RE = /^(f|v_kmh|sight_margin_m)(>=|<=)([^@]+)@(.+)$/;
  const m = TOKEN_RE.exec(value.trim());
  if (m === null) return err(schemaErr(at, `malformed constraint token "${value}"`, "constraint_token_malformed"));
  ...
  else return err(schemaErr(at, `"${field}${op}" has no bound in the closed set`, "constraint_bound_unknown"));
  if (!(CONSTRAINT_BOUNDS as readonly string[]).includes(bound)) {
...
function mintActionId(prefix: string): string {
  planActionCounter += 1;
...
export const FLAG_TABLE: readonly FlagSpec[] = [
  {
  ...
  },
  {
    ...
    apply: (d, v, at) => { const n = parseNum(v, at); if (!n.ok) return n; d.bike_margin_m = n.value; return ok(undefined); }
  },
  {
    ...
    apply: (d, v, at) => {
      ...
      if (!n.ok) return n;
      if (!(n.value > 0)) return err(badRange(at, "mu must be positive", "mu_nonpositive"));
      d.mu = n.value;
      return ok(undefined);
    }
  },
  ...
  },
  {
    ...
    apply: (d, v, at) => {
      const n = parseNum(v, at);
      if (!n.ok) return n;
      if (!(n.value > 0)) return err(badRange(at, "entry must be positive", "entry_kmh_nonpositive"));
      d.entry_kmh = n.value;
      return ok(undefined);
    }
  },
  {
    ...
    apply: (d, v, at) => { const n = parseNum(v, at); if (!n.ok) return n; d.start_f = n.value; return ok(undefined); }
  },
  {
    field: "profile", scene_key: "", flag: "--profile", arity: "value", group: "Rider & start",
    apply: (d, v, at) => {
      if (!(RIDER_PROFILE_NAMES as readonly string[]).includes(v)) {
        return err(schemaErr(at, `profile must be one of ${RIDER_PROFILE_NAMES.join(", ")}`, "profile_unknown"));
      }
      d.profile = v as RiderProfileName;
      return ok(undefined);
    }
  },
  {
    field: "roll_rate_cap_dps", scene_key: "", flag: "--roll-rate-cap", arity: "value", group: "Rider & start",
    apply: (d, v, at) => { const n = parseNum(v, at); if (!n.ok) return n; d.roll_rate_cap_dps = n.value; return ok(undefined); }
  },
  {
    field: "turn_in", scene_key: "turnIn", flag: "--turn-in", arity: "value", group: "Plan channels",
    apply: (d, v, at) => {
      if (v === "auto") { d.turn_in = "auto"; return ok(undefined); }
      const n = parseNum(v, at);
      if (!n.ok) return n;
      d.turn_in = n.value;
      return ok(undefined);
  ...
  },
  {
    ...
    apply: (d, v, at) => {
      if (v === "auto") return ok(undefined); // "auto" == no authored pin (the solver's default)
      const n = parseNum(v, at);
      if (!n.ok) return n;
      if (!(n.value > 0)) return err(badRange(at, "brake decel must be positive", "decel_nonpositive"));
  ...
  },
  {
    ...
    apply: (d, v, at) => {
      ...
      if (idx < 0) return err(schemaErr(at, "--brake-slew needs a preceding --brake", "brake_slew_without_brake"));
      const b = plan[idx]!;
  ...
  },
  {
    ...
    apply: (d, v, at) => {
      const n = parseNum(v, at);
      if (!n.ok) return n;
      if (n.value < 0) return err(badRange(at, "throttle accel must be >= 0", "accel_negative"));
  ...
  },
  {
    ...
    apply: (d, v, at) => {
      ...
      if (idx < 0) return err(schemaErr(at, "--throttle-slew needs a preceding --throttle", "throttle_slew_without_throttle"));
      const th = plan[idx]!;
  ...
  },
  {
    ...
    apply: (d, v, at) => {
      ...
      const plan = d.plan ?? [];
      const idx = plan.findIndex((a) => a.do === "throttle");
      if (idx < 0) return err(schemaErr(at, "--throttle-freeze needs a preceding --throttle", "throttle_freeze_without_throttle"));
      const th = plan[idx]!;
  ...
  },
  {
    ...
    apply: (d, v, at) => {
      ...
      for (const tok of v.split(",").map((t) => t.trim()).filter((t) => t.length > 0)) {
        ...
        if (eq <= 0) return err(schemaErr(at, `malformed --position token "${tok}"`, "position_token_malformed"));
      ...
      const over_m: number | "auto" | undefined = overRaw === undefined ? undefined : overRaw === "auto" ? "auto" : Number(overRaw);
      d.plan = [
        ...(d.plan ?? []),
        {
          do: "position",
  ...
  },
  {
    ...
    apply: (d, v, at) => {
      if (!(VIS_MODES as readonly string[]).includes(v)) {
  ...
  },
  {
    ...
    apply: (d, v, at) => { const n = parseNum(v, at); if (!n.ok) return n; d.vis_hold_f = n.value; return ok(undefined); }
  },
  {
    field: "vis_margin", scene_key: "visMargin", flag: "--vis-margin", arity: "value", group: "Solver intent",
    apply: (d, v, at) => { const n = parseNum(v, at); if (!n.ok) return n; d.vis_margin = n.value; return ok(undefined); }
  },
  {
...
export const FLAG_MAPPINGS: readonly FlagMapping[] = FLAG_TABLE.map((f) => ({
  field: f.field,
...
for (const f of FLAG_TABLE) {
  ...
  if (f.sugar !== undefined) FLAG_BY_NAME.set(f.sugar, f);
}
...
// ---------------------------------------------------------------------------
// General verb-shared flags (not zero-file composition fields): --gate,
// --out, --trace, --suggest, --check, --views, --mode, --as, --all,
// --no-cache, --line, --pretty, --quiet, --standing, --on, --corner,
// --s/--t (state, design/08 §3), --scan-ds (save-window, §4.1), the sweep
// path grammar §4.3 (--param/--param2/--range/--range2/--metric/--format,
// parsed here so a later sweep-verb WP need not touch this file), --port
// (serve, §3).
 ...
 */
export const CLI_VIEWS = ["topdown", "controls", "pov"] as const;
 ...
 * parse.
 */
export const VERB_SCOPED_FLAGS: readonly { readonly flag: string; readonly verbs: readonly string[] }[] = [
 ...
 */
export function ineffectualFlagFor(verb: string, parsed: ParsedInvocation): LinelabError | null {
  ...
  const legacyReason: Readonly<Record<string, string>> = {
    "--standing": "standing_without_check",
    "--scan-ds": "scan_ds_without_save_window"
  };
  for (const row of VERB_SCOPED_FLAGS) {
    if (named[row.flag] === undefined) continue;
    if (row.verbs.includes(verb)) continue;
    return {
      code: "INEFFECTUAL",
      at: row.flag,
 ...
 */
export function parseZeroFileFlags(argv: readonly string[]): Result<ParsedInvocation> {
  ...
  let mode: "true" | "diagram" | undefined;
  ...
  let scanDs: number | undefined;
  let param: string | undefined;
  let param2: string | undefined;
  let range: string | undefined;
  let range2: string | undefined;
  let metric: string | undefined;
  let format: "tsv" | "json" | undefined;
  let port: number | undefined;
  let lock: "station" | "time" | undefined;
  ...
  for (const tok of argv) {
    if (DEFERRED_FLAG_NAMES.has(tok)) {
      ...
      if (deferred !== undefined) return err(deferredError("cli", tok, deferred));
    }
  }
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (tok === "--gate") { gate = true; continue; }
    if (tok === "--suggest") { suggest = true; continue; }
    if (tok === "--check") { check = true; continue; }
    if (tok === "--all") { all = true; continue; }
    if (tok === "--no-cache") { noCache = true; continue; }
    if (tok === "--pretty") { pretty = true; continue; }
    if (tok === "--quiet") { quiet = true; continue; }
    if (tok === "--standing") { standing = true; continue; }
    if (VALUE_ONLY_FLAGS.has(tok)) {
      const v = argv[++i];
      if (v === undefined) return err(schemaErr(tok, `${tok} needs a value`, "flag_value_missing"));
      if (tok === "--out") out = v;
      else if (tok === "--trace") trace = v;
      else if (tok === "--views") {
        ...
        const bad = named.find((x) => !(CLI_VIEWS as readonly string[]).includes(x));
        if (bad !== undefined) {
          return err(
            schemaErr(tok, `unknown view "${bad}" — the closed set is ${CLI_VIEWS.join(" | ")}`, "view_unknown", {
              views: CLI_VIEWS
            })
          );
        }
        ...
        if (v !== "true" && v !== "diagram") return err(schemaErr(tok, '--mode must be "true" or "diagram"', "mode_unknown"));
        mode = v;
      } else if (tok === "--as") as = v;
      else if (tok === "--line") line = v;
      else if (tok === "--on") on = v;
      else if (tok === "--corner") corner = v;
      else if (tok === "--s") {
        ...
        if (!n.ok) return n;
        scanDs = n.value;
      } else if (tok === "--param") param = v;
      else if (tok === "--param2") param2 = v;
      else if (tok === "--range") range = v;
      else if (tok === "--range2") range2 = v;
      else if (tok === "--metric") metric = v;
      else if (tok === "--format") {
        if (v !== "tsv" && v !== "json") return err(schemaErr(tok, '--format must be "tsv" or "json"', "format_unknown"));
        format = v;
      } else if (tok === "--port") {
        const n = parseNum(v, tok);
        if (!n.ok) return n;
        ...
        if (v !== "station" && v !== "time") return err(schemaErr(tok, '--lock must be "station" or "time"', "lock_unknown"));
        lock = v;
      }
      continue;
    }
...
export function draftToComposedInput(draft: ComposeDraft): Record<string, unknown> {
  const mistake: MistakeSpec | undefined =
    draft.mistakes.length === 1
      ? {
...
function withRoadOptions(draft: ComposeDraft): RoadSpec {
 ...
 */
export function mergeDraftOverLoaded(loaded: unknown, draft: ComposeDraft): Record<string, unknown> {
  const composed = draftToComposedInput(draft);
  if (loaded === undefined || loaded === null || typeof loaded !== "object" || Array.isArray(loaded)) {
    return composed;
  }
  const base = loaded as Record<string, unknown>;
  // Scenario file: flags land inside rider.start / rider / config / road.
  if ("rider" in base) {
    const rider = (base["rider"] as Record<string, unknown> | undefined) ?? {};
    const start = (rider["start"] as Record<string, unknown> | undefined) ?? {};
    const config = (base["config"] as Record<string, unknown> | undefined) ?? {};
    ...
    const loadedRoad = (base["road"] as Record<string, unknown> | undefined) ?? {};
    const roadOverride =
      draft.road !== undefined
