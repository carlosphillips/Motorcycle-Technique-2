...
import type { RoadModel, Sample, Trajectory } from "../core/types.js";
...
// ---------------------------------------------------------------------------
// The golden roster (design/09 §3.2; ARCHITECTURE §7 golden gate map)

export type RosterInput =
  /** a wire Scenario (explicit plan) or a composed solver input — both through run() */
  | { readonly kind: "run"; readonly input: Record<string, unknown> }
  ...
  | {
      ...
      readonly mistake: { readonly kind: string; readonly params?: Record<string, unknown> };
    };
...
function scenario(id: string, dsl: string, entry_kmh: number, f: number | undefined, plan: readonly Record<string, unknown>[]): Record<string, unknown> {
  return {
    spec: "linelab/1",
    id,
    road: { dsl },
...
export const BLESS_ROSTER: readonly RosterEntry[] = [
  {
    id: "C30",
    blessed_line: "solved",
...
export const BLESSED_BLOCK_FIXTURES: readonly string[] = ["C30", "C30-chop", "C30-trailbrake", "C30-DR"];

// ---------------------------------------------------------------------------
// Golden record shapes (raw f64 — the bless tap)
...
export interface GoldenLineRecord {
  ...
  readonly terminated: { readonly reason: string; readonly s: number; readonly t: number };
  ...
  readonly checks: readonly { readonly id: string; readonly verdict: string }[];
}
...
// ---------------------------------------------------------------------------
// Tolerance labels (read from THE one table, test/fixtures/tolerances.json)
...
interface ToleranceCategoryRow {
  readonly category: string;
  readonly tol?: number;
}

export function tolLabelsFrom(tolerancesJson: unknown): TolLabels {
  const cats = (tolerancesJson as { categories?: readonly ToleranceCategoryRow[] }).categories ?? [];
  const label = (name: string, fallback: number): string => {
...
function trajStats(samples: readonly Sample[]): { phi_max_deg: number; grip_min: number; v_min_ms: number } {
  let phi = 0;
  ...
  for (const p of samples) {
    ...
    if (a > phi) phi = a;
    if (p.grip < grip) grip = p.grip;
    if (p.v < v) v = p.v;
  }
  return { phi_max_deg: phi, grip_min: grip, v_min_ms: v };
}

function rawAnalysis(line: LineResult, road: RoadModel): ReturnType<typeof analyzeCorners> {
...
function eventsOf(traj: Trajectory): GoldenEventRecord[] {
...
function lineRecord(line: LineResult, road: RoadModel): GoldenLineRecord {
...
export function blessedRowsFor(fixture: string, line: LineResult, road: RoadModel, tol: TolLabels): BlessedRow[] {
  const rows: BlessedRow[] = [];
  const push = (quantity: string, value: number | string | undefined, unit: string, t: string): void => {
  ...
  const apex =
    c1 !== undefined && c1.apexes.length > 0
      ? c1.apexes.reduce((a, b) => (b.f < a.f ? b : a))
      : undefined;
  let leanCommit = 0;
  for (const p of samples) leanCommit = Math.max(leanCommit, Math.abs(p.cmd_lean));
...
// ---------------------------------------------------------------------------
// Roster integration (pure library calls; refusals are typed, never swallowed)
...
export type ComputeResult =
  | { readonly ok: true; readonly records: readonly GoldenRecord[] }
  | { readonly ok: false; readonly failure: RosterFailure };

function composeDsl(dsl: string): RoadModel {
  const r = compose({ dsl });
  if (!r.ok) throw new Error(`roster road failed to compose: ${JSON.stringify(r.error)}`);
  return r.value;
}

function computeOne(entry: RosterEntry, tol: TolLabels): GoldenRecord | RosterFailure {
  if (entry.input.kind === "run") {
    const r = run(entry.input.input, { engine_semver: ENGINE_SEMVER, figure_id: entry.id });
    if (!r.ok) return { fixture: entry.id, error: r.error };
    const refusal = r.value.lines.find((l) => isLineRefusal(l));
    if (refusal !== undefined) return { fixture: entry.id, error: (refusal as { error: unknown }).error };
    const lines = r.value.lines.filter((l): l is LineResult => !isLineRefusal(l));
    ...
    return {
      ...
      blessed:
        blessedLine !== undefined && BLESSED_BLOCK_FIXTURES.includes(entry.id)
  ...
  if (!baseR.ok) return { fixture: entry.id, error: baseR.error };
  ...
  if (!compiled.ok) return { fixture: entry.id, error: compiled.error };
...
export function computeGoldenRecords(tol: TolLabels): ComputeResult {
  const records: GoldenRecord[] = [];
  for (const entry of BLESS_ROSTER) {
    const r = computeOne(entry, tol);
    if ("error" in r) return { ok: false, failure: r };
    records.push(r);
  }
...
export function formatBlessedBlock(records: readonly GoldenRecord[], engineSemver: string, dateIso: string): string {
  const lines: string[] = [
...
export function spliceBlessedBlock(docText: string, block: string): { ok: true; value: string } | { ok: false; error: string } {
...
export function parseBlessedHeader(docText: string): { engine: string; date: string } | null {
...
// ---------------------------------------------------------------------------
// FigureSpec stamp regeneration (design/09 §3.3f)
...
function restampFigureSpec(raw: Record<string, unknown>): { ok: true; value: Record<string, unknown> } | { ok: false; reason: string } {
  const r = run(raw, { engine_semver: ENGINE_SEMVER });
  if (!r.ok) return { ok: false, reason: `run refused: ${JSON.stringify(r.error)}` };
  const byName = new Map<string, LineResult>();
  for (const l of r.value.lines) if (!isLineRefusal(l)) byName.set(l.line_id, l);
  const linesRaw = raw["lines"];
  if (!Array.isArray(linesRaw)) return { ok: false, reason: "no lines[]" };
  const stampedLines = linesRaw.map((entry) => {
    if (typeof entry !== "object" || entry === null) return entry;
    const e = entry as Record<string, unknown>;
    const line = typeof e["name"] === "string" ? byName.get(e["name"]) : undefined;
    if (line === undefined) return e;
...
function parseArgs(argv: readonly string[], defaultRoot: string): BlessArgs {
  let writeDocs = false;
  let root = defaultRoot;
  let design: string | undefined;
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i]!;
    if (a === "--write-docs") writeDocs = true;
    else if (a === "--root") {
      const v = argv[++i];
      if (v === undefined) usage("--root needs a directory");
      root = resolve(v);
    } else if (a === "--design") {
      const v = argv[++i];
      if (v === undefined) usage("--design needs a directory");
      design = resolve(v);
    } else usage(`unknown flag "${a}" (known: --write-docs, --root <dir>, --design <dir>)`);
  }
...
function runAnalyticGate(root: string): { green: boolean; exit: number | null; tail: string } {
  const vitestMjs = join(root, "node_modules", "vitest", "vitest.mjs");
  if (!existsSync(vitestMjs)) return { green: false, exit: null, tail: `vitest not found at ${vitestMjs}` };
  const r = spawnSync(
    process.execPath,
    [vitestMjs, "run", "test/analytic/an.test.ts", "test/analytic/bounds.test.ts"],
...
function main(): void {
  ...
  if (!gate.green) {
    process.stdout.write(
      JSON.stringify(
        { ok: false, refusal: "analytic_gate_red", vitest_exit: gate.exit, detail: "A-AN-* + D-BOUNDS must be green in this same invocation (design/09 §3.2a step 3; A-BLESS-REFUSES)", tail: gate.tail },
        null,
        2
      ) + "\n"
    );
  ...
  if (!computed.ok) {
    process.stdout.write(
      JSON.stringify({ ok: false, refusal: "golden_roster_refused", fixture: computed.failure.fixture, error: computed.failure.error }, null, 2) + "\n"
    );
    process.exit(3);
  }
  ...
  if (args.writeDocs) {
    ...
    if (spliced.value !== doc) writes.push({ path: docPath, content: spliced.value });
    wroteDocs = true;
  }
  ...
  if (existsSync(figuresDir)) {
    for (const file of readdirSync(figuresDir).filter((f) => f.endsWith(".json")).sort()) {
      ...
      if (typeof raw !== "object" || raw === null || !Array.isArray((raw as Record<string, unknown>)["lines"])) {
        stamped.push({ file, stamped: false, reason: "not a FigureSpec (no lines[])" });
        continue;
      }
      const r = restampFigureSpec(raw as Record<string, unknown>);
      if (!r.ok) {
