...
export function runVerb(input: RunVerbInput): VerbOutcome {
  ...
  if (parsed.value.draft.mistakes.length > 1) {
    ...
    return errOutcome(
      schemaErr("--mistake", "run's zero-file path accepts at most one --mistake in this build — author additional mistake lines via `figure`/scene text", "run_multi_mistake_unsupported")
    );
  }
  ...
  const opts: RunOptions = { engine_semver: input.engineSemver, ...(input.figureId !== undefined ? { figure_id: input.figureId } : {}) };
  const result = run(composed, opts);
  if (!result.ok) return errOutcome(result.error);
  const envelope = result.value;

  let exit: VerbOutcome["exit"] = EXIT.OK;
  if (parsed.value.gate) {
    const declared = loaded !== undefined ? expectDeclarationsOf(loaded) : { ok: true as const, value: {} };
    const report = gateFigure(envelope, declared.ok ? { expect: declared.value } : {});
    if (!report.pass) exit = EXIT.DEVIATION;
  }
