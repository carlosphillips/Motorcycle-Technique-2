...
export function controlsView(input: ControlsViewInput): VerbOutcome {
  ...
  if (typeof topdown.stdout === "object" && topdown.stdout !== null && (topdown.stdout as { ok?: boolean }).ok === false) {
    return topdown;
  }
  const topValue = (topdown.stdout as { value: Record<string, unknown> }).value;
  return okOutcome(
    {
      ...topValue,
