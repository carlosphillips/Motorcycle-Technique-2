...
import type { InstantState, ResolvedHazard, ResolvedOccluder, ResolvedScenario, RoadModel } from "../../core/types.js";
import type { ComposedRoad } from "../../road/types.js";
import { LINELAB_SPEC, type LineEntry, type LineResult } from "../../solve/types.js";
...
/** design/07 §3.7 lock mode — closed 2-value set, `station` default. */
type Lock = "station" | "time";

/** one member's metric snapshot at a lock position — every value is `stateAt`. */
...
interface CompareSample {
  /** the lock coordinate: station `s` under station lock, elapsed `t` under time lock */
  readonly at: number;
  /** per input index → that line's OWN stateAt at `at` (C-COMPARE) */
  readonly per_input: Readonly<Record<string, MetricCell>>;
}
...
interface VerdictDelta {
  ...
  readonly sight_margin_min_m: number | null;
}

interface ComparePair {
  readonly line_id: string;
  /** input indices carrying this line_id */
  readonly inputs: readonly number[];
  /** per-line verdict deltas (§3.5) */
  readonly verdict: readonly VerdictDelta[];
  /** diff domain = intersection of the paired lines' station ranges (§3.5) */
  readonly span: { readonly from_s: number; readonly to_s: number; readonly clipped: boolean };
  /** station-aligned metric diff (sight, speed, lean, grip) at shared lock positions */
  readonly samples: readonly CompareSample[];
  /** true iff the shared grid was uniformly subsampled to COMPARE_MAX_SAMPLES */
  readonly truncated: boolean;
}

interface WorldEntry {
  readonly present_in: readonly number[];
...
interface CompareResult {
  ...
  readonly lock: Lock;
  /** the shared road hash (equal across every input — the road contract) */
  readonly road_hash: string;
  readonly inputs: readonly { readonly index: number; readonly figure_id: string; readonly lines: readonly string[] }[];
  /** occluder/hazard entries NOT shared by every input (empty ⇒ same world) */
  readonly world_delta: {
    readonly differs: boolean;
    readonly occluders: readonly WorldEntry[];
    readonly hazards: readonly WorldEntry[];
  };
  readonly pairs: readonly ComparePair[];
  /** line_ids present in only one input — reported, never silently dropped (§3.5) */
  readonly unpaired: { readonly a: readonly string[]; readonly b: readonly string[] };
  /** written overlay SVG path, or null (no `--out`, or the overlay could not render) */
...
// ---------------------------------------------------------------------------
// Recompute one input (D6). Never trusts a shipped trajectory.

/** the recomputed slice compare consumes — a FigureResult in all but name. */
interface RecomputedInput {
  ...
  /** non-refusal lines only (refusals carry no trajectory to diff) */
...
function recomputeSpec(input: unknown, engineSemver: string): Result<RecomputedInput> {
  ...
  return {
    ...
    value: {
      ...
      lines: fr.value.lines.filter((l): l is LineResult => !isLineRefusal(l))
    }
  };
}

/** re-run every line of an envelope from its `resolved_scenario` (A-RESOLVED-RERUN). */
function recomputeEnvelope(json: Record<string, unknown>, engineSemver: string): Result<RecomputedInput> {
  const figure_id = typeof json["figure_id"] === "string" ? (json["figure_id"] as string) : "compare";
  const linesRaw = (json["lines"] as unknown[]) ?? [];
  const opts = { engine_semver: engineSemver };
  const lines: LineResult[] = [];
  let road: RoadModel | undefined;
  let occluders: readonly ResolvedOccluder[] = [];
  let hazards: readonly ResolvedHazard[] = [];
  for (let i = 0; i < linesRaw.length; i++) {
    const entry = linesRaw[i] as LineEntry;
    if (isLineRefusal(entry)) continue; // a refused input line has no trajectory to recompute
    const line = entry as LineResult;
    const rs = line.resolved_scenario as ResolvedScenario;
    ...
    if (!fr.ok) return { ok: false, error: { ...fr.error, at: `lines[${line.line_id}]${fr.error.at ? `.${fr.error.at}` : ""}` } };
    const recomputed = fr.value.lines.find((l): l is LineResult => !isLineRefusal(l));
...
function recomputeInput(text: string, engineSemver: string): Result<RecomputedInput> {
...
function roadKey(road: RoadModel): RoadKey {
  const segments = (road as ComposedRoad).segments;
  return {
...
function roadHash(road: RoadModel): string {
  return fnv1a(canonOf(roadKey(road)));
}

/** first index at which two segment lists differ (by canonical JSON), or null. */
function firstDifferingSegment(a: readonly unknown[], b: readonly unknown[]): number | null {
  const n = Math.max(a.length, b.length);
  for (let i = 0; i < n; i++) {
    if (canonOf(a[i]) !== canonOf(b[i])) return i;
  }
  return null;
}
...
function collectWorld(fr: RecomputedInput): { occ: Map<string, ResolvedOccluder>; haz: Map<string, ResolvedHazard> } {
  const occ = new Map<string, ResolvedOccluder>();
  const haz = new Map<string, ResolvedHazard>();
  const addOcc = (o: ResolvedOccluder): void => { occ.set(canonOf(o), o); };
  const addHaz = (h: ResolvedHazard): void => { haz.set(canonOf(h), h); };
  for (const o of fr.occluders) addOcc(o);
  for (const h of fr.hazards) addHaz(h);
  ...
  for (const l of fr.lines) {
    ...
    for (const o of l.resolved_scenario.occluders) addOcc(o);
    for (const h of l.resolved_scenario.hazards) addHaz(h);
  }
  return { occ, haz };
}

function worldDelta(recomputed: readonly RecomputedInput[]): CompareResult["world_delta"] {
  const n = recomputed.length;
  const worlds = recomputed.map(collectWorld);
  const diffEntries = <T>(pick: (w: { occ: Map<string, ResolvedOccluder>; haz: Map<string, ResolvedHazard> }) => Map<string, T>): WorldEntry[] => {
    const keys = new Set<string>();
    for (const w of worlds) for (const k of pick(w).keys()) keys.add(k);
    ...
    for (const k of [...keys].sort()) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (m.has(k)) { present.push(i); entry = m.get(k); }
      }
      if (present.length < n) out.push({ present_in: present, entry });
    }
    return out;
  };
...
function stateInputFor(line: LineResult, road: RoadModel): StateAtInput {
  return { trajectory: line.trajectory, road, plan: line.resolved_scenario.rider.plan, sightTrendAt };
}

function metricsOf(st: InstantState): MetricCell {
  return {
    s: r2(st.sample.s),
    t: r2(st.sample.t),
...
function subsample(xs: readonly number[], cap: number): { grid: readonly number[]; truncated: boolean } {
  if (xs.length <= cap) return { grid: xs, truncated: false };
  const out: number[] = [];
...
function buildPair(line_id: string, members: readonly Member[], lock: Lock): ComparePair {
  const key: "s" | "t" = lock === "station" ? "s" : "t";
  const EPS = 1e-6;
  const endsOf = (which: "s" | "t"): { first: number[]; last: number[] } => {
    const first: number[] = [];
    const last: number[] = [];
  ...
  for (const q of grid) {
    const query: StateAtQuery = key === "s" ? { s: q } : { t: q };
    const per_input: Record<string, MetricCell> = {};
    let good = true;
    for (const m of members) {
      const st = stateAt(stateInputFor(m.line, m.road), query);
      if (!st.ok) { good = false; break; }
      per_input[String(m.input)] = metricsOf(st.value);
    }
    if (good) samples.push({ at: r2(q), per_input });
  }

  const verdict: VerdictDelta[] = members.map((m) => ({
    input: m.input,
...
export function compareVerb(input: CompareVerbInput): VerbOutcome {
  const parsed = parseZeroFileFlags(input.argv);
  if (!parsed.ok) return errOutcome(parsed.error);
  const lock: Lock = parsed.value.lock ?? "station";
  ...
  if (input.loadedTexts.length < 2) {
    return errOutcome(
      schemaErr("compare", `compare needs at least two inputs to compare (got ${input.loadedTexts.length}) — \`compare <A> <B> […]\``, "nothing_to_compare", {
  ...
  const recomputed: RecomputedInput[] = [];
  for (let i = 0; i < input.loadedTexts.length; i++) {
    const fr = recomputeInput(input.loadedTexts[i]!, input.engineSemver);
    if (!fr.ok) return errOutcome({ ...fr.error, at: `inputs[${i}]${fr.error.at ? `.${fr.error.at}` : ""}` });
  ...
  for (let i = 1; i < hashes.length; i++) {
    if (hashes[i] !== base) {
      const segA = (recomputed[0]!.road as ComposedRoad).segments;
      const segB = (recomputed[i]!.road as ComposedRoad).segments;
      const at = firstDifferingSegment(segA, segB);
      return errOutcome(
        schemaErr("inputs", `compare requires one shared road; inputs[0] road ${base} ≠ inputs[${i}] road ${hashes[i]}`, "road_mismatch", {
          road_hash_a: base,
  ...
  const perInput = recomputed.map((fr) => {
    const m = new Map<string, LineResult>();
    for (const l of fr.lines) if (!isLineRefusal(l)) m.set(l.line_id, l);
    return m;
  });
  const allIds = [...new Set(perInput.flatMap((m) => [...m.keys()]))].sort();

  const pairs: ComparePair[] = [];
  const unpairedA: string[] = [];
  const unpairedB: string[] = [];
  for (const id of allIds) {
    const members: Member[] = [];
    for (let i = 0; i < perInput.length; i++) {
      const line = perInput[i]!.get(id);
      if (line !== undefined) members.push({ input: i, line, road: recomputed[i]!.road });
    }
    if (members.length >= 2) {
  ...
  if (pairs.length === 0) {
    return errOutcome(
      schemaErr("compare", "nothing to compare — no line_id is shared by two or more inputs (pair lines by giving them the same line_id)", "no_paired_lines", {
  ...
  if (parsed.value.out !== undefined) {
    const mergedLines: LineResult[] = [];
    recomputed.forEach((fr, i) => {
      for (const l of fr.lines) if (!isLineRefusal(l)) mergedLines.push(relabelLine(l, `in${i}_${l.line_id}`));
    });
    ...
    const rendered = renderViews({ road: recomputed[0]!.road as ComposedRoad, lines: mergedLines });
    if (rendered.ok) {
  ...
  const result: CompareResult = {
    ...
    inputs: recomputed.map((fr, i) => ({
      ...
      lines: fr.lines.filter((l): l is LineResult => !isLineRefusal(l)).map((l) => l.line_id)
    })),
