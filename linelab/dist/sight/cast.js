...
function rayBlocked(eye, target, footprints) {
    for (const fp of footprints) {
        ...
        for (let i = 0; i < n; i++) {
            const a = poly[i];
            const b = poly[(i + 1) % n];
            if (segmentsCross(eye, target, a, b))
                return true;
        }
    }
    return false;
 ...
 */
export function castSight(road, eye, footprints) {
    ...
    for (const st of road.stations) {
        if (st.s <= sEye + EPS_PARAM)
            continue; // targets strictly forward of the eye's station
        const target = road.worldAt(st.s, dCentre);
        if (rayBlocked(eye, target, footprints))
            break; // first blocked — stop; re-emergence never counts
        lastVisible = st.s;
    }
    return {
 ...
 */
export function sightFrom(road, eye, occluders) {
