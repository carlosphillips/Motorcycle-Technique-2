...
function aLongAvailLocal(a_lat, mu) {
    const cap = mu * G;
 ...
 */
function aNoReturnLocal(phi_rad, roll_rate_rad_s) {
 ...
 */
export function ssd(v_ms, phi_rad, model, profile, mu) {
    ...
    const a_lean = Math.min(a_ssd, aLongAvailLocal(G * Math.tan(absPhi), mu), aNoReturnLocal(phi_rad, roll_rate));
    let standup_m;
    let brake_m;
    if (v_ms <= a_lean * t_su) {
