...
export const QUALITIES = ["good", "caution", "failing"];
/**
 ...
 */
export function clean(outcome, doctrine) {
 ...
 */
export function criticalFailed(doctrine, pack) {
    for (const r of doctrine.checks) {
        if (r.verdict !== "fail")
            continue;
        const row = pack.checks.find((c) => c.id === r.id);
        if (row !== undefined && row.severity === "critical")
            return true;
    }
    return false;
}
 ...
 */
export function quality(outcome, doctrine, pack) {
    if (outcome === "crash" || outcome === "runoff" || outcome === "wide")
        return "failing";
    if (criticalFailed(doctrine, pack))
        return "failing";
    if (clean(outcome, doctrine))
        return "good";
    return "caution";
}
