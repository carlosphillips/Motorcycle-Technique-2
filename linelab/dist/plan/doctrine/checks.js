...
import { apexArgmin, apexPct, blindAtTurnIn, chainExtent, chainStructure, committedLeanDeg, cornerWindow, ellipseMax, exitSampleIndex, flowLegs, holdWideLegs, inputCount, leanMax, linkLegs, oioFractions, sightDeficit, steerShare, taperProfile, throttleLegs, trackerOverdrive } from "./metrics.js";
// ---------------------------------------------------------------------------
 ...
 */
export const CHECK_SCOPE = Object.freeze({
    late_apex: "corner",
    out_in_out: "corner",
    single_input: "corner",
    quick_steer: "corner",
 ...
 */
export const CHECK_BANDS = Object.freeze({
...
function thresholdValue(row, name) {
...
function naInstance(cornerId, pair, reason, message) {
    return {
        corner_id: cornerId,
        pair,
        band: null,
...
function cornerInstances(record, ctx, row, gradeOne) {
    const out = [];
    for (let i = 0; i < record.corners.length; i++) {
        ...
        if (trend && !trend.includes(c.type)) {
            out.push(naInstance(c.id, null, `${c.type}_radius`, `not applicable on ${c.type}-radius corner`));
            continue;
        }
        ...
        if (row.applicability.requires_blind) {
            ...
            if (blind !== true) {
                out.push(naInstance(c.id, null, "not_blind", "corner is not blind at this line's turn-in (blind(c) false)"));
                continue;
            }
        }
...
const evaluators = {
    // 1 ------------------------------------------------------------------------
    late_apex: (record, row, ctx) => cornerInstances(record, ctx, row, (w, c) => {
        ...
        if (m.graded_pct === null) {
            ...
            return naInstance(c.id, null, "sweep_unmeasurable", "no recorded apex and the corner's total sweep is not measurable from this record");
        }
        const pct = m.graded_pct;
        const bar = c.type === "decreasing"
    ...
    // 2 ------------------------------------------------------------------------
    out_in_out: (record, row, ctx) => cornerInstances(record, ctx, row, (w, c) => {
    ...
    // 3 ------------------------------------------------------------------------
    single_input: (record, row, ctx) => cornerInstances(record, ctx, row, (w, c) => {
    ...
    // 4 ------------------------------------------------------------------------
    quick_steer: (record, row, ctx) => cornerInstances(record, ctx, row, (w, c) => {
        ...
        const band = m.steer_share > shareFail
            ? "eats_corner"
    ...
    // 5 ------------------------------------------------------------------------
    throttle_rule: (record, row, ctx) => cornerInstances(record, ctx, row, (w, c) => {
    ...
    // 6 ------------------------------------------------------------------------
    trail_brake_taper: (record, row, ctx) => cornerInstances(record, ctx, row, (w, c) => {
        ...
        if (m.baseline) {
            return naInstance(c.id, null, "brake_complete_baseline", "entry braking completes at least brake_gap before turn-in");
        }
        if (m.forced_standup_at_s !== null || m.redeepened_at_s !== null) {
            ...
            return {
                ...
                pair: null,
                band: "forced_standup",
                message: m.forced_standup_at_s !== null
    ...
    // 7 ------------------------------------------------------------------------
    traction_ceiling: (record, row, ctx) => cornerInstances(record, ctx, row, (w, c) => {
    ...
    // 8 ------------------------------------------------------------------------
    lean_ceiling: (record, row, ctx) => cornerInstances(record, ctx, row, (w, c) => {
        ...
        const band = m.phi_max_deg <= reserve
            ? "within_reserve"
    ...
    exit_containment: (record, row, ctx) => cornerInstances(record, ctx, row, (w, c) => {
        ...
        const f = record.samples[exIdx].f;
    ...
    stop_within_sight: (record, row) => {
        ...
        const band = m.max_deficit_m > 0 ? "deficit" : m.min_margin_m < warnM ? "thin_margin" : "margin_ok";
        return [
            {
                corner_id: null,
                pair: null,
                band,
                message: band === "deficit"
    ...
    hold_wide_for_sight: (record, row, ctx) => {
        if (record.vertically_blind === true) {
            return record.corners.map((c) => naInstance(c.id, null, "vertical_sight_geometry_not_modelled", "vertical sight geometry not modelled"));
        }
        return cornerInstances(record, ctx, row, (w, c) => {
            ...
            const committedClosing = m.release_s === null || m.turn_in_s < m.release_s - releaseTol;
            if (committedClosing) {
                return {
                    corner_id: c.id,
                    pair: null,
    ...
    rideability: (record, row) => {
        ...
        const overdriven = m.max_excess_dps > rateTol || m.max_dkappa > kappaStep || m.phi_jump_deg > phiJump;
        const why = [
    ...
    wrong_strategy_for_corner: (record, row, ctx) => {
        ...
        for (let i = 0; i < record.corners.length; i++) {
            ...
            const isDr = c.r1 !== undefined && c.r2 !== undefined && c.r2 > 0 && c.r1 / c.r2 >= drRatioMin;
            if (!trendOk || !isDr) {
            ...
            const tiSample = tiIdx !== null ? record.samples[tiIdx] : null;
            const blindAtCommit = tiSample !== null && w.turn_in !== null
                ? tiSample.sight_m < c.s1 - w.turn_in.s
                : false;
            const vReserve = (c.r2 ?? 0) > 0
                ? Math.sqrt(G * Math.tan(degToRad(record.physics.phi_reserve_deg)) * c.r2)
                : 0;
            ...
            out.push({
                ...
                pair: null,
                band,
                message: band === "double_blind_slow"
                ...
                metrics: {
                    ...
                    book_note: "In the case of blind corners, if you don’t know if it is a decreasing radius, you can take either path shown but at a significantly lower speed."
                }
            });
        }
        return out;
...
function chainSpanWindows(record, ctx) {
    if (ctx.chainModeCornerIds.size === 0)
        return null;
    ...
    const sTo = exIdx !== null ? record.samples[exIdx].s : last.s_hi;
 ...
 * corner/pair order.
 */
export function runChecks(record, pack) {
    ...
    for (const row of pack.checks) {
        ...
        if (!evaluator)
            continue; // loader guarantees; skip defensively, never invent
        ...
        for (const inst of instances) {
            ...
            if (inst.band === null) {
                ...
                if (row.severity === "advisory" && verdict === "fail")
                    verdict = "warn";
            }
            results.push({
                id: row.id,
