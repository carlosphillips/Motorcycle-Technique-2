...
export const FIGURE_ROLES = ["ideal", "alternative", "mistake", "reference"];
export const LABEL_FEATURES = [
    "turn_point", "apex", "exit", "release", "correction", "run_wide_detect", "end", "sight_ray"
];
export const MARK_CLASSES = ["turn_point", "apex", "exit", "release"];
export const SOLVE_STYLES = ["single", "double_apex", "geometric"];
export const VIS_MODES = ["none", "cautious"];
...
function validateRoadSpecShape(raw, at) {
    if (!isObject(raw))
    ...
    const tail = {
        ...(useFullWidthRaw !== undefined ? { use_full_width: useFullWidthRaw } : {}),
        ...(bikeMarginRaw !== undefined ? { bike_margin_m: bikeMarginRaw } : {})
    };
    if (hasPreset) {
        ...
        if (raw["hand"] !== undefined) {
            if (!isHand(raw["hand"]))
                return err(schemaErr(`${at}.hand`, 'hand must be "L" or "R"', "hand_malformed"));
            hand = raw["hand"];
        }
    ...
    for (let i = 0; i < segmentsRaw.length; i++) {
        ...
        if (type === "straight") {
            ...
            if (!isHand(seg["hand"]))
                return err(schemaErr(`${segAt}.hand`, 'hand must be "L" or "R"', "hand_malformed"));
            const angle_deg = seg["angle_deg"];
            if (typeof angle_deg !== "number")
                return err(schemaErr(`${segAt}.angle_deg`, "angle_deg must be a number", "type_mismatch"));
            if (type === "arc") {
                const r_m = seg["r_m"];
                if (typeof r_m !== "number")
                    return err(schemaErr(`${segAt}.r_m`, "r_m must be a number", "type_mismatch"));
...
function validateAnchorShape(raw, at) {
    if (!isObject(raw))
        return err(schemaErr(at, "an anchor must be a JSON object", "type_mismatch"));
    ...
    if (typeof ref !== "string")
        return err(schemaErr(`${at}.ref`, "ref must be a string", "type_mismatch"));
    let offset_m;
...
const OCCLUDER_KINDS = ["hedge", "wall", "bank", "vehicle"];
const OCCLUDER_SIDES = ["inside", "outside", "left", "right"];
const VEHICLE_LANES = ["own", "oncoming"];
function validateOccluderShape(raw, at) {
    if (!isObject(raw))
        return err(schemaErr(at, "an occluder must be a JSON object", "type_mismatch"));
    const kind = raw["kind"];
    if (typeof kind !== "string" || !OCCLUDER_KINDS.includes(kind)) {
    ...
    if (!atAnchor.ok)
        return atAnchor;
    if (raw["side"] !== undefined && !OCCLUDER_SIDES.includes(raw["side"])) {
        return err(schemaErr(`${at}.side`, `side must be one of ${OCCLUDER_SIDES.join(", ")}`, "type_mismatch"));
    }
    if (raw["lane"] !== undefined && !VEHICLE_LANES.includes(raw["lane"])) {
        return err(schemaErr(`${at}.lane`, `lane must be one of ${VEHICLE_LANES.join(", ")}`, "type_mismatch"));
    }
    for (const numField of ["span_m", "margin_m", "depth_m", "len_m", "width_m", "f", "speed_kmh", "height_m"]) {
...
function validateHazardShape(raw, at) {
    if (!isObject(raw))
        return err(schemaErr(at, "a hazard must be a JSON object", "type_mismatch"));
    if (raw["kind"] !== "gravel") {
        return err(schemaErr(`${at}.kind`, 'hazard kind must be "gravel"', "type_mismatch"));
    }
    const HAZARD_SIDES = ["inside", "outside", "left", "right", "center"];
    if (typeof raw["side"] !== "string" || !HAZARD_SIDES.includes(raw["side"])) {
    ...
    for (const numField of ["width_m", "mu"]) {
        if (raw[numField] !== undefined && typeof raw[numField] !== "number") {
            return err(schemaErr(`${at}.${numField}`, `${numField} must be a number`, "type_mismatch"));
        }
    }
...
function validateConstraintShape(raw, at) {
    if (!isObject(raw))
        return err(schemaErr(at, "a constraint must be a JSON object", "type_mismatch"));
    const id = raw["id"];
    if (typeof id !== "string" || id.length === 0) {
        return err(schemaErr(`${at}.id`, "constraint id must be a non-empty string", "type_mismatch"));
    }
    const bound = raw["bound"];
    if (typeof bound !== "string" || !CONSTRAINT_BOUNDS.includes(bound)) {
        return err(schemaErr(`${at}.bound`, `bound must be one of ${CONSTRAINT_BOUNDS.join(", ")}`, "type_mismatch"));
    }
    const value = raw["value"];
    if (typeof value !== "number")
        return err(schemaErr(`${at}.value`, "value must be a number", "type_mismatch"));
    const span = raw["span"];
    if (!isObject(span))
        return err(schemaErr(`${at}.span`, "span must be a JSON object", "type_mismatch"));
    ...
    return ok({
        ...
        bound: bound,
        value,
        span: hasAt ? { at: span["at"] } : { from: span["from"], to: span["to"] }
    });
}
// ---------------------------------------------------------------------------
...
function validateSolveSpecShape(raw, at) {
    ...
    for (const strField of ["corner"]) {
        if (raw[strField] !== undefined) {
            if (typeof raw[strField] !== "string")
                return err(schemaErr(`${at}.${strField}`, `${strField} must be a string`, "type_mismatch"));
            out[strField] = raw[strField];
        }
    }
    for (const numField of ["mu", "vis_hold_f", "vis_margin", "start_f", "roll_rate_cap_dps"]) {
        if (raw[numField] !== undefined) {
            if (typeof raw[numField] !== "number")
                return err(schemaErr(`${at}.${numField}`, `${numField} must be a number`, "type_mismatch"));
            out[numField] = raw[numField];
        }
    }
    ...
    if (raw["occluders"] !== undefined) {
        if (!Array.isArray(raw["occluders"]))
            return err(schemaErr(`${at}.occluders`, "occluders must be an array", "type_mismatch"));
        const occs = [];
    ...
    if (raw["hazards"] !== undefined) {
        if (!Array.isArray(raw["hazards"]))
            return err(schemaErr(`${at}.hazards`, "hazards must be an array", "type_mismatch"));
        const haz = [];
    ...
    if (raw["constraints"] !== undefined) {
        if (!Array.isArray(raw["constraints"]))
            return err(schemaErr(`${at}.constraints`, "constraints must be an array", "type_mismatch"));
        const cs = [];
...
function validateMistakeSpecShape(raw, at) {
    if (!isObject(raw))
        return err(schemaErr(at, "a mistake spec must be a JSON object", "type_mismatch"));
    const kind = raw["kind"];
    if (typeof kind !== "string" || !MISTAKE_KINDS.includes(kind)) {
    ...
    if (raw["params"] !== undefined) {
        if (!isObject(raw["params"]))
            return err(schemaErr(`${at}.params`, "params must be a JSON object", "type_mismatch"));
    ...
    if (raw["scope"] !== undefined) {
        ...
        if (s === "all_corners")
            ...
            return err(schemaErr(`${at}.scope`, 'scope must be "all_corners" or an array of corner ids', "type_mismatch"));
    }
    return ok({ kind, ...(params !== undefined ? { params } : {}), ...(scope !== undefined ? { scope } : {}) });
}
function validateScenarioShape(raw, at) {
    ...
    if (!road.ok)
        return road;
    if (!isObject(raw["rider"]))
        return err(schemaErr(`${at}.rider`, "rider must be a JSON object", "type_mismatch"));
    return ok(raw);
}
/** Structural discriminator over `SolveSpec | MistakeSpec | Scenario` (design/03 §8). */
function validateLineSpecShape(raw, at) {
    if (!isObject(raw))
        return err(schemaErr(at, "a line spec must be a JSON object", "type_mismatch"));
    if (raw["spec"] === "linelab/1")
        return validateScenarioShape(raw, at);
    if ("entry_kmh" in raw)
        return validateSolveSpecShape(raw, at);
    if ("kind" in raw)
        return validateMistakeSpecShape(raw, at);
    return err(schemaErr(at, 'a line spec must be a SolveSpec ({entry_kmh,…}), a MistakeSpec ({kind,…}), or an explicit Scenario ({spec:"linelab/1",…})', "line_spec_shape_unrecognized"));
}
// ---------------------------------------------------------------------------
// FigureLabel shape (design/03 §8)
function validateLabelShape(raw, at) {
    if (!isObject(raw))
        return err(schemaErr(at, "a label must be a JSON object", "type_mismatch"));
    const feature = raw["feature"];
    if (typeof feature !== "string" || !LABEL_FEATURES.includes(feature)) {
    ...
    return ok({
        feature: feature,
        line,
        ...(raw["corner"] !== undefined ? { corner: raw["corner"] } : {}),
        ...(raw["n"] !== undefined ? { n: raw["n"] } : {}),
        ...(raw["offset_m"] !== undefined ? { offset_m: raw["offset_m"] } : {}),
        ...(raw["text"] !== undefined ? { text: raw["text"] } : {})
    });
}
// ---------------------------------------------------------------------------
// MarkSpec shape (design/03 §8) — `auto | all | none | <class-list>`
function validateMarkSpecShape(raw, at) {
    if (raw === "auto" || raw === "all" || raw === "none")
        return ok(raw);
    if (Array.isArray(raw) && raw.length > 0 && raw.every((c) => MARK_CLASSES.includes(c))) {
        return ok(raw);
    }
    return err(schemaErr(at, `marks must be "auto"|"all"|"none" or a non-empty array of ${MARK_CLASSES.join("|")}`, "marks_malformed"));
}
// ---------------------------------------------------------------------------
 ...
 */
export function validateFigureSpec(json) {
    if (!isObject(json))
        return err(schemaErr("", "a FigureSpec must be a JSON object", "figure_not_object"));
    ...
    for (let i = 0; i < linesRaw.length; i++) {
        ...
        const at = `lines[${i}]`;
        if (!isObject(raw))
            return err(schemaErr(at, "a figure line must be a JSON object", "type_mismatch"));
        const name = raw["name"];
        ...
        seenNames.add(name);
        const role = raw["role"];
        if (typeof role !== "string" || !FIGURE_ROLES.includes(role)) {
    ...
    if (json["occluders"] !== undefined) {
        if (!Array.isArray(json["occluders"]))
    ...
    if (json["labels"] !== undefined) {
        if (!Array.isArray(json["labels"]))
            return err(schemaErr("labels", "labels must be an array", "type_mismatch"));
        const ls = [];
 ...
 */
export function lineMarksOf(fig) {
    return new Map(fig.lines.filter((l) => l.marks !== undefined).map((l) => [l.name, l.marks]));
}
// ---------------------------------------------------------------------------
