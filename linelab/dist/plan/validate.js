...
import { RIDER_PROFILES, G, SLEW_MIN, SLEW_MAX, A_SLEW_DEFAULT, FREEZE_MAX_S, K_REACH, a_lat_pos_max } from "../core/constants.js";
...
import { resolveOccluder, resolveHazard, parseOccluderOrHazardToken } from "./placements.js";
import { CONFIG_MU_DEFAULT, CONFIG_DS_M_DEFAULT, CONFIG_SSD_MODEL_DEFAULT, CONFIG_RUBRIC_DEFAULT, CONFIG_CHECKS_VERSION_DEFAULT, START_F_DEFAULT } from "./constants.js";
// ---------------------------------------------------------------------------
...
function parseHandToken(value, at) {
    ...
    if (typeof value === "string") {
        ...
        if (lower === "left" || lower === "right") {
            ...
            return err(schemaErr(at, `hand must be spelled "L"|"R" — write "${rewrite}" instead of "${value}"`, "hand_full_word", {
                rewrite
            }));
        }
    }
...
function parseRoadSpecJson(raw, at) {
    if (!isObject(raw))
        return err(schemaErr(at, "road must be a JSON object", "road_not_object"));
    if ("traffic" in raw) {
        return err(schemaErr(`${at}.traffic`, "the traffic field is reserved for a future left-hand-traffic mode", "traffic_reserved"));
    }
    ...
    const useFullWidth = useFullWidthRaw;
    const bikeMargin = bikeMarginRaw;
    if (hasPreset) {
        const presetName = raw["preset"];
        if (typeof presetName !== "string")
            return err(schemaErr(`${at}.preset`, "preset must be a string", "type_mismatch"));
        let hand;
    ...
    if (hasDsl) {
        ...
        if (typeof dsl !== "string")
            return err(schemaErr(`${at}.dsl`, "dsl must be a string", "type_mismatch"));
        return ok({
            dsl,
            ...(useFullWidth !== undefined ? { use_full_width: useFullWidth } : {}),
            ...(bikeMargin !== undefined ? { bike_margin_m: bikeMargin } : {})
    ...
    for (let i = 0; i < segmentsRaw.length; i++) {
        ...
        if (!isObject(seg))
            return err(schemaErr(segAt, "segment must be a JSON object", "type_mismatch"));
        const type = seg["type"];
        if (type === "straight") {
            const len_m = seg["len_m"];
            if (typeof len_m !== "number")
                return err(schemaErr(`${segAt}.len_m`, "len_m must be a number", "type_mismatch"));
...
function mintIds(rawList, prefix, at) {
    const seen = new Set();
    const ids = [];
...
// ---------------------------------------------------------------------------
// Rider
function parseRider(riderRaw, corners) {
    if (!isObject(riderRaw))
        return err(schemaErr("rider", "rider must be a JSON object", "type_mismatch"));
    let profile = "street";
    if (riderRaw["profile"] !== undefined) {
        const p = riderRaw["profile"];
        if (typeof p !== "string" || !RIDER_PROFILE_NAMES.includes(p)) {
            return err(schemaErr("rider.profile", `rider.profile must be one of ${RIDER_PROFILE_NAMES.join(", ")} (got ${JSON.stringify(p)})`, "type_mismatch"));
        }
        profile = p;
    }
    const profileTable = RIDER_PROFILES[profile];
    let roll_rate_cap_dps;
    if (riderRaw["roll_rate_cap_dps"] !== undefined) {
        ...
        if (cap >= profileTable.roll_rate_dps) {
            return err(ineffectual("rider.roll_rate_cap_dps", `roll_rate_cap_dps (${cap}) does not bind below the ${profile} profile rate (${profileTable.roll_rate_dps})`, "roll_rate_cap_not_binding"));
        }
        roll_rate_cap_dps = cap;
    }
    const rollRateEff = roll_rate_cap_dps ?? profileTable.roll_rate_dps;
    const startRaw = riderRaw["start"];
    if (!isObject(startRaw))
        return err(schemaErr("rider.start", "rider.start must be a JSON object", "type_mismatch"));
    ...
    if (typeof speedKmhRaw !== "number" || !(speedKmhRaw > 0)) {
        return err(schemaErr("rider.start.speed_kmh", "rider.start.speed_kmh is required and must be a positive number", "speed_kmh_required"));
    }
    ...
    if (hasD) {
        const d = startRaw["d"];
        if (typeof d !== "number")
            return err(schemaErr("rider.start.d", "d must be a number", "type_mismatch"));
        start_d = d;
    }
    else {
        const f = hasF ? startRaw["f"] : START_F_DEFAULT;
        if (typeof f !== "number")
            return err(schemaErr("rider.start.f", "f must be a number", "type_mismatch"));
        if (!(f >= 0 && f <= 1)) {
    ...
    if (!Array.isArray(planRaw))
        return err(schemaErr("rider.plan", "rider.plan must be an array", "type_mismatch"));
    void corners;
    return ok({
        profile,
        roll_rate_cap_dps,
...
function resolveActionStation(raw, corners, at) {
    ...
    if (hasAtS) {
        const at_s = raw["at_s"];
        if (typeof at_s !== "number")
            return err(schemaErr(`${at}.at_s`, "at_s must be a number", "type_mismatch"));
        return ok(at_s);
    }
    const atObj = raw["at"];
    if (!isObject(atObj))
        return err(schemaErr(`${at}.at`, "at must be a JSON object", "type_mismatch"));
    const ref = atObj["ref"];
    if (typeof ref !== "string")
        return err(schemaErr(`${at}.at.ref`, "at.ref must be a string", "type_mismatch"));
    let offset_m;
    if (atObj["offset_m"] !== undefined) {
        const o = atObj["offset_m"];
        if (typeof o !== "number")
            return err(schemaErr(`${at}.at.offset_m`, "offset_m must be a number", "type_mismatch"));
        offset_m = o;
    }
    return resolveAnchor(offset_m !== undefined ? { ref, offset_m } : { ref }, corners, `${at}.at`);
}
function parseSlew(value, at) {
    if (value === undefined)
        return ok(A_SLEW_DEFAULT);
    if (typeof value !== "number")
        return err(schemaErr(at, "slew_mss must be a number", "type_mismatch"));
    if (value < SLEW_MIN || value > SLEW_MAX) {
        return err(badRange(at, `slew_mss must be in [${SLEW_MIN}, ${SLEW_MAX}]`, "slew_out_of_range"));
    }
    return ok(value);
}
function parseBrake(raw, id, at_s, at) {
    ...
    if (raw["taper_to_s"] !== undefined) {
        ...
        if (typeof t !== "number")
            return err(schemaErr(`${at}.taper_to_s`, "taper_to_s must be a number", "type_mismatch"));
        taper_to_s = t;
    }
...
function parseThrottle(raw, id, at_s, at) {
    ...
    if (raw["freeze_steer_s"] !== undefined) {
        ...
        if (typeof f !== "number")
            return err(schemaErr(`${at}.freeze_steer_s`, "freeze_steer_s must be a number", "type_mismatch"));
...
function parseTurnInTarget(raw, at) {
    if (raw === "tangent_inside")
        return ok("tangent_inside");
...
function parseTurnInStageA(raw, id, at_s, at) {
...
function parsePositionStageA(raw, id, at_s, at) {
    if (raw["slew_mss"] !== undefined) {
    ...
    if (hasF) {
        const fv = raw["f"];
        if (typeof fv !== "number")
            return err(schemaErr(`${at}.f`, "f must be a number", "type_mismatch"));
        f = fv;
    }
    else {
        const dv = raw["d"];
        if (typeof dv !== "number")
            return err(schemaErr(`${at}.d`, "d must be a number", "type_mismatch"));
        d = dv;
    }
    let overRaw;
    if (raw["over_m"] !== undefined) {
        ...
        if (o === "auto")
            ...
            return err(schemaErr(`${at}.over_m`, 'over_m must be a number or "auto"', "type_mismatch"));
    }
    return ok({ id, at_s, f, d, overRaw, at });
}
function resolveGoverningCorner(at_s, explicitHand, corners, at) {
    ...
    if (found === undefined) {
        return err(badRange(at, explicitHand === undefined
            ? "no corner lies downstream of this turn_in"
            : `no ${explicitHand}-hand corner lies downstream of this turn_in`, "no_governing_corner"));
    }
    return ok(found);
}
function buildAccelSegments(brakes, throttles, horizon) {
    const longs = [
    ...
    if (cursor < horizon)
        segs.push({ s0: cursor, s1: horizon, aStart: cursorA, aEnd: cursorA });
    return segs;
}
const VCMD_GRID_N = 32;
function vSqAt(segs, v0sq, sTarget) {
    let vsq = v0sq;
...
function tCmd(segs, v0, s0, s1) {
    if (s1 <= s0)
        return 0;
    const atEnd = vSqAt(segs, v0 * v0, s1);
    if (atEnd.parkedAt !== null && atEnd.parkedAt <= s1)
        return Infinity;
    let t = 0;
...
export function validate(json) {
    if (!isObject(json))
        return err(schemaErr("", "a scenario must be a JSON object", "scenario_not_object"));
    ...
    const presetOccluderTokens = isPresetSpec(roadSpec.value)
        ? PRESETS[roadSpec.value.preset]?.occluders ?? []
        : [];
    const tokenOccluders = [];
    for (let i = 0; i < presetOccluderTokens.length; i++) {
        ...
        if (parsed.value.occluder !== undefined)
            tokenOccluders.push(parsed.value.occluder);
    }
    const occludersInput = [...tokenOccluders, ...(occludersRaw ?? [])];
    ...
    for (let i = 0; i < occludersInput.length; i++) {
        ...
        if (!isObject(raw))
            return err(schemaErr(`occluders[${i}]`, "occluder must be a JSON object", "type_mismatch"));
    ...
    for (let i = 0; i < hazardsInput.length; i++) {
        ...
        if (!isObject(raw))
            return err(schemaErr(`hazards[${i}]`, "hazard must be a JSON object", "type_mismatch"));
    ...
    const finalActions = new Array(planRaw.length).fill(undefined);
    const stagedTurnIns = [];
    ...
    for (let i = 0; i < planRaw.length; i++) {
        ...
        if (!isObject(raw))
            return err(schemaErr(at, "plan action must be a JSON object", "type_mismatch"));
    ...
    for (const { index, staged } of stagedTurnIns) {
        ...
        for (const th of throttleActions) {
            if (th.freeze_steer_s === undefined)
                continue;
            const winStart = th.at_s;
            const winEnd = th.at_s + th.freeze_steer_s;
            if (staged.at_s >= winStart && staged.at_s < winEnd) {
                return err(ineffectual(staged.at, "turn_in falls inside a throttle steering-freeze window", "turn_in_during_freeze", {
                    throttle_id: th.id
                }));
            }
        }
    ...
    for (const { staged } of stagedPositions) {
        const f_tgt = staged.f !== undefined ? staged.f : road.fOf(staged.d, staged.at_s);
    ...
    function fFrom(at_s) {
        ...
        for (const e of lateralEvents) {
            if (e.at_s < at_s && (best === undefined || e.at_s > best.at_s))
                best = e;
        }
        if (best === undefined) {
            return start_f !== undefined ? start_f : road.fOf(start_d, at_s);
        }
    ...
    for (const { index, staged } of stagedPositions) {
        const f_tgt = staged.f !== undefined ? staged.f : road.fOf(staged.d, staged.at_s);
        if (!(f_tgt >= 0 && f_tgt <= 1)) {
            return err(badRange(staged.at, `position target f=${f_tgt.toFixed(3)} is outside the corridor [0,1]`, "position_target_outside_corridor"));
        }
        let over_m;
        ...
        if (!(windowStart >= 0 && windowEnd <= roadEnd)) {
            return err(badRange(staged.at, `position window [${windowStart}, ${windowEnd}] must lie within [0, ${roadEnd}]`, "position_window_outside_road"));
        }
        for (const t of resolvedTurnIns) {
            if (windowStart < t.commitmentEnd && t.commitmentStart < windowEnd) {
                return err(ineffectual(staged.at, `position window overlaps turn_in "${t.action.id}"'s commitment window`, "position_overlaps_turn_in", { turn_in_id: t.action.id }));
            }
        }
    ...
    for (let i = 0; i < resolvedPositions.length; i++) {
        for (let j = i + 1; j < resolvedPositions.length; j++) {
            ...
            if (a.windowStart < b.windowEnd && b.windowStart < a.windowEnd) {
                return err(ineffectual(a.staged.at, `position windows of "${a.staged.id}" and "${b.staged.id}" overlap`, "position_overlaps_position", {
    ...
    for (const rp of resolvedPositions) {
        ...
        if (from !== undefined) {
            ...
            if (requested > achievableDd) {
                ...
                return err(ineffectual(rp.staged.at, `position target unreachable in ${rp.over_m.toFixed(1)} m (need ≈ ${requiredOverM.toFixed(1)} m)`, "position_target_unreachable", {
        ...
        const action = {
            ...
            id: rp.staged.id,
            at_s: rp.staged.at_s,
            ...(rp.staged.f !== undefined ? { f: rp.staged.f } : { d: rp.staged.d }),
            over_m: rp.over_m
        };
        finalActions[rp.index] = action;
    }
    const plan = finalActions.map((a) => a);
    // -- config -----------------------------------------------------------------
    const configRaw = json["config"] ?? {};
    if (!isObject(configRaw))
        return err(schemaErr("config", "config must be a JSON object", "type_mismatch"));
    let mu = CONFIG_MU_DEFAULT;
    if (configRaw["mu"] !== undefined) {
        const m = configRaw["mu"];
        if (typeof m !== "number")
            return err(schemaErr("config.mu", "mu must be a number", "type_mismatch"));
        if (!(m > 0))
            return err(badRange("config.mu", "mu must be > 0", "config_mu_nonpositive"));
        mu = m;
    }
    ...
    if (configRaw["ds_m"] !== undefined) {
        ...
        if (typeof d !== "number" || !(d > 0))
            return err(schemaErr("config.ds_m", "ds_m must be a positive number", "type_mismatch"));
        ds_m = d;
    }
    let ssd_model = CONFIG_SSD_MODEL_DEFAULT;
    if (configRaw["ssd_model"] !== undefined) {
        const s = configRaw["ssd_model"];
        if (typeof s !== "string" || !SSD_MODELS.includes(s)) {
    ...
    if (configRaw["rubric"] !== undefined) {
        ...
        if (typeof r !== "string")
            return err(schemaErr("config.rubric", "rubric must be a string", "type_mismatch"));
    ...
    if (configRaw["checks_version"] !== undefined) {
        ...
        if (cv !== CONFIG_CHECKS_VERSION_DEFAULT) {
            return err(schemaErr("config.checks_version", `checks_version must be ${CONFIG_CHECKS_VERSION_DEFAULT} (got ${JSON.stringify(cv)})`, "checks_version_unsupported"));
        }
    }
    const config = { mu, ds_m, ssd_model, rubric, checks_version: CONFIG_CHECKS_VERSION_DEFAULT };
    // -- expect_fail / meta -----------------------------------------------------
    let expect_fail;
    ...
    if (json["meta"] !== undefined) {
        const m = json["meta"];
        if (!isObject(m))
            return err(schemaErr("meta", "meta must be a JSON object", "type_mismatch"));
        meta = m;
    }
    return ok({
        spec: "linelab/1",
        ...
        rider: {
            ...
            start: { speed_kmh, ...(start_f !== undefined ? { f: start_f } : { d: start_d }) },
            plan
        },
        config,
