...
import { FIGURE_ROLES, LABEL_FEATURES, MARK_CLASSES, SOLVE_STYLES, VIS_MODES, ACCEPT_POLICIES, CONSTRAINT_BOUNDS } from "./figure.js";
// ---------------------------------------------------------------------------
...
function stripComment(line) {
    ...
    for (let i = 0; i < line.length; i++) {
        ...
        if (c === '"')
            inQuotes = !inQuotes;
        else if (c === "#" && !inQuotes && (i === 0 || /\s/.test(line[i - 1])))
...
function hasUnterminatedQuote(s) {
    let count = 0;
    for (const c of s)
        if (c === '"')
            count++;
...
function dequote(tok) {
    if (tok.length >= 2 && tok.startsWith('"') && tok.endsWith('"'))
        return tok.slice(1, -1);
    return undefined;
}
function parseNumberToken(value, at) {
    if (value.length === 0)
        return err(schemaErr(at, "expected a number, got an empty token", "scene_number_malformed"));
    const n = Number(value);
    if (!Number.isFinite(n))
        return err(schemaErr(at, `expected a number, got "${value}"`, "scene_number_malformed"));
    return ok(n);
}
// ---------------------------------------------------------------------------
...
function parseRoadRefToken(tokens, at) {
    ...
    if (first === "preset") {
        ...
        for (const tok of tokens.slice(2)) {
            ...
            if (m === null)
                return err(schemaErr(at, `unrecognized road option "${tok}"`, "road_ref_option_malformed"));
            const key = m[1];
            const val = m[2];
            if (seen.has(key))
                return err(schemaErr(at, `duplicate road option "${key}"`, "road_ref_option_duplicate"));
            seen.add(key);
            if (key === "hand") {
                if (val !== "L" && val !== "R")
                    return err(schemaErr(at, `hand must be "L" or "R" (got "${val}")`, "hand_malformed"));
                hand = val;
            }
            else if (key === "fullWidth") {
                if (val !== "true" && val !== "false")
                    return err(schemaErr(at, `fullWidth must be "true" or "false" (got "${val}")`, "type_mismatch"));
...
function parseConstraintTokens(value, lineName, at) {
    const parts = value.split(";").map((p) => p.trim()).filter((p) => p.length > 0);
    if (parts.length === 0)
        return err(schemaErr(at, "constraints=\"...\" carries no tokens", "constraints_empty"));
    const out = [];
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        if (field === "f" && op === ">=")
            ...
            return err(schemaErr(at, `"${field}${op}" has no bound in the closed set — the bound vocabulary is closed`, "constraint_bound_unknown"));
        }
        ...
        out.push({ id: `${lineName}_c${i + 1}`, span, bound: bound, value: n.value });
    }
    return ok(out);
}
const RIDE_KEYS = [
...
function parseRideArgs(argsText, lineName, at) {
    ...
    for (const tok of splitRespectingQuotes(argsText)) {
        ...
        if (eq <= 0)
            return err(schemaErr(at, `malformed ride argument "${tok}" (expected key=value)`, "ride_arg_malformed"));
        ...
        switch (key) {
            ...
            case "style": {
                if (!SOLVE_STYLES.includes(value)) {
            ...
            case "accept": {
                if (!ACCEPT_POLICIES.includes(value)) {
            ...
            case "role": {
                if (!FIGURE_ROLES.includes(value)) {
...
// silently misparsed.
const LABEL_FEATURE_RE = new RegExp(`^(${LABEL_FEATURES.join("|")})(?::([A-Za-z0-9_]+))?(?:#(\\d+))?(?:@([A-Za-z0-9_]+))?$`);
function parseLabelAnchor(tok, at) {
    if (/^(entry|mid):/.test(tok)) {
        return err(schemaErr(at, `"${tok}": road anchors (entry:/mid:) are not representable as a line-feature label in this build`, "label_road_anchor_unsupported"));
    }
...
function parseLabelEntry(text, at) {
    ...
    if (tokens.length === 3) {
        ...
        if (!n.ok)
            return err(schemaErr(at, `malformed label offset "${offTok}"`, "scene_label_malformed"));
        offset_m = n.value;
    }
...
function parseMarkSpec(value, at) {
    const v = value.trim();
    if (v === "auto" || v === "all" || v === "none")
        return ok(v);
    const classes = v.split(",").map((c) => c.trim()).filter((c) => c.length > 0);
    if (classes.length === 0)
        return err(schemaErr(at, "marks: carries no value", "marks_malformed"));
    for (const c of classes) {
        if (!MARK_CLASSES.includes(c)) {
...
function parseViewSpec(value, at) {
    const out = {};
    for (const tok of value.trim().split(/\s+/).filter((t) => t.length > 0)) {
        const eq = tok.indexOf("=");
        if (eq <= 0)
            return err(schemaErr(at, `malformed view token "${tok}" (expected key=value)`, "view_token_malformed"));
        const key = tok.slice(0, eq);
        if (key in out)
            return err(schemaErr(at, `duplicate view key "${key}"`, "view_duplicate_key"));
        out[key] = tok.slice(eq + 1);
    }
    if (Object.keys(out).length === 0)
        return err(schemaErr(at, "view: carries no key=value pairs", "view_malformed"));
    return ok(out);
}
// ---------------------------------------------------------------------------
// Top-level scanning: column-0 keys, `lines:`/`labels:` children indented.
export const TOP_LEVEL_KEYS = ["road", "lines", "occluders", "hazards", "marks", "labels", "view", "note", "placards"];
function scanTopLevel(sceneText) {
    ...
    for (let idx = 0; idx < rawLines.length; idx++) {
        ...
        if (!isIndented) {
            ...
            entries.push({ key: key, inlineValue: m[2].trim(), children: [], lineNo });
        }
        else {
            ...
            last.children.push({ text: stripped.trim(), lineNo });
        }
    }
    return ok(entries);
}
function parseLinesBlock(children) {
    const out = [];
 ...
 */
export function lowerScene(sceneText) {
    ...
    const SINGLETON_KEYS = ["road", "lines", "marks", "labels", "view", "note", "placards"];
    ...
    for (const key of ["road", "marks", "view", "note", "occluders", "hazards"]) {
    ...
    if (!rawLines.value.some((l) => l.kind === "ride")) {
        return err(schemaErr("lines", "no reference line; a mistake needs a first \"ride\" entry to compile against", "scene_no_reference_line"));
    }
    const figureLines = [];
    let sawRide = false;
    for (const raw of rawLines.value) {
        ...
        if (raw.kind === "ride") {
            ...
            if (parsed.value.line_id !== undefined) {
                return err(schemaErr(at, `a scene mistake entry supplies its own line id ("${raw.name}") — drop the "${parsed.value.line_id}=" prefix`, "mistake_lineid_not_allowed_in_scene"));
            }
            const spec = {
            ...
            return err(schemaErr(at, `line kind "${raw.kind}" is design vocabulary (design/04 §7) not reachable through the v0.1 solve/ build (ARCHITECTURE §5) — ` +
                (raw.kind === "plan"
                    ? "an explicit-plan line requires file IO, which a pure/total lowerScene cannot perform"
                    : "the naive baseline solver is not part of the v0.1 exported surface"), "scene_line_kind_unsupported_v0_1", { kind: raw.kind }));
        }
        else {
