...
export const CONTROLS_NEUTRAL_INKS = Object.freeze([
    INK_PRIMARY,
    INK_SECONDARY,
    INK_AXIS,
    INK_GRID,
 ...
 */
export function lastCornerIdOf(segments) {
    const ids = cornerIdsOf(segments);
 ...
 * a span, not an event.
 */
export function phaseBandsOf(line) {
...
function seriesFor(panel, samples) {
    switch (panel) {
        case "v":
...
// ---------------------------------------------------------------------------
// Drawing
function polylineRuns(samples, values, gate, x, y) {
    const runs = [];
    let current = [];
...
function panelSvg(panel, samples, x, y0, plotWidth) {
    ...
    const key = series.secondary !== undefined
...
function phaseBandsSvg(bands, x, top) {
...
function windowBandSvg(win, sMin, sMax, x, top, height) {
    const from = Math.max(Math.min(win.from, win.to), sMin);
    const to = Math.min(Math.max(win.from, win.to), sMax);
    if (!(to > from))
        return "";
    const x0 = x(from);
    return (open("g", { class: "window-band", "data-window-from-s": n3(from), "data-window-to-s": n3(to) }) +
...
function stationAxisSvg(sMin, sMax, x, y, plotWidth) {
    ...
    for (let s = Math.ceil(sMin / step) * step; s <= sMax + 1e-9; s += step) {
        ...
        out += textEl({ x: px, y: y + 15, "font-family": "sans-serif", "font-size": 8, fill: INK_AXIS, "text-anchor": "middle" }, `${n3(s)}`);
    }
    out += textEl({
...
function cursorSample(line, s) {
    const rs = line.resolved_scenario.road;
    const composed = compose(rs.dsl.length > 0
    ...
    if (composed.ok) {
        ...
        if (at.ok)
            return { sample: at.value.sample, source: "stateAt" };
    }
    return { sample: nearestSample(line.trajectory.samples, s), source: "nearest" };
}
function chipText(panel, p) {
    switch (panel) {
        case "v":
...
function cursorSvg(cursorS, line, x, panelTop, bodyBottom) {
    ...
    PANEL_IDS.forEach((panel, i) => {
        ...
        out += textEl({ x: n3(px + 6), y: y + 10, "font-family": "sans-serif", "font-size": 8, fill: INK_CURSOR }, label);
    });
    out += "</g>";
    return out;
}
 ...
 *                   per frame)
 */
export function renderControls(lineResult, window, cursor) {
    try {
        const samples = lineResult.trajectory.samples;
        if (samples.length < 2)
            return fallbackSvg("controls: the line has no plottable sample series");
