...
function textEl(a, content) {
    return `${open("text", a)}${esc(content)}${close("text")}`;
}
const QUALITIES = ["good", "caution", "failing"];
function points(pts) {
...
function stageLaneMarkings(scene, pxScale) {
    const w = pxScale * 1.0;
...
function stageUsableCorridor(scene, pxScale) {
    ...
    return (open("g", { "data-stage": "3b-usable-corridor" }) +
...
const OCCLUDER_FILL = { hedge: "#4c7a4c", wall: "#8a8a8a", bank: "#a8875a", vehicle: "#6b6b8a" };
/**
 ...
 */
function occluderGlyphSvg(o, pxScale) {
    ...
    if (o.kind === "vehicle") {
        ...
        if (frontL === undefined || frontR === undefined || rearR === undefined || rearL === undefined)
            return base;
...
function stageOccluders(scene, pxScale) {
    let s = open("g", { "data-stage": "5-occluders" });
    for (const o of scene.occluders)
        s += occluderGlyphSvg(o, pxScale);
    s += close("g");
    return s;
}
/**
 ...
 * here.
 */
function stageGravel(scene, pxScale) {
 ...
 */
function stageOcclusion(scene) {
    if (scene.occlusionWash === null)
        return "";
    return leaf("polygon", {
...
function stageSightRays(scene, pxScale) {
    let s = open("g", { "data-stage": "7-sight-rays" });
    for (const line of scene.lines) {
        if (line.sightRay === null)
            continue;
        s += leaf("line", {
...
function terminalGlyphSvg(line, pxScale) {
    const t = line.terminal;
    const r = pxScale * TERMINAL_R_PX;
    const common = { "data-terminal-reason": t.reason, "data-line-id": line.line_id };
    if (t.glyph === "burst") {
        return (leaf("line", { x1: t.at.x - r, y1: t.at.y - r, x2: t.at.x + r, y2: t.at.y + r, stroke: line.colour, "stroke-width": pxScale * 1.5, ...common }) +
...
function stageLines(scene, pxScale) {
...
function outcomeWord(line) {
    if (line.terminal.reason === "off_road")
        return line.outcome === "wide" ? "ran wide" : "ran off";
    if (line.terminal.reason === "crash")
        return "crashed";
    if (line.terminal.reason === "stopped")
        return "stopped";
...
function chromeText(x, y, pxScale, size, colour, anchor, content, extra = {}) {
    return textEl({
        x,
        y,
...
function indexAtStation(line, s) {
    for (let i = 0; i < line.stations.length; i++)
        if (line.stations[i] >= s)
            return i;
    return null;
}
function headingAt(line, i) {
...
function stageLineChrome(scene, pxScale) {
    let s = open("g", { "data-stage": "8b-line-chrome" });
    for (const line of scene.lines) {
        if (line.polyline.length < 2)
            continue;
        ...
        s += chromeText(p0.x - h0.ux * pxScale * 10 - h0.uy * pxScale * 26 * side, p0.y - h0.uy * pxScale * 10 + h0.ux * pxScale * 26 * side, pxScale, CHROME_FONT_PX, line.colour, "middle", `${Math.round(line.entry_kmh)} km/h`, { "data-entry-label": line.line_id });
 ...
 */
export function hourglassPoints(cx, cy, r) {
...
function markerGlyphSvg(m, pxScale) {
    ...
    switch (m.cls) {
        ...
        case "release":
            return (leaf("line", { x1: m.at.x - r, y1: m.at.y - r, x2: m.at.x, y2: m.at.y, stroke: m.colour, "stroke-width": pxScale * 1.8, ...common }) +
...
function stageMarkers(scene, pxScale) {
 ...
 */
function stageLabels(scene, pxScale, vbX, vbY, vbW, vbH) {
...
function stageChrome(scene, pxScale, vbX, vbY, vbW, vbH, bandTop) {
    ...
    if (scene.legend.visible) {
        ...
        for (const row of scene.legend.rows) {
            ...
            s += textEl({ x: x + pxScale * 22, y, "font-size": pxScale * 11, "font-family": "sans-serif", fill: "#222222", "data-legend-row": row.line_id }, text);
            y += lineH;
        }
        s += close("g");
    }
 ...
 */
const SCALE_NICE_M = [5, 10, 20, 25, 50, 100];
const M_PER_FT = 0.3048;
function scaleBarSvg(scene, pxScale, vbX, vbY, vbW, vbH) {
    ...
    return (open("g", { "data-scale-bar": metres, "data-lane-width-m": scene.road.lane_width_m }) +
...
// ---------------------------------------------------------------------------
// renderTopdown
function renderInner(scene, style) {
 ...
 */
export function renderTopdown(drawnScene, style) {
    try {
