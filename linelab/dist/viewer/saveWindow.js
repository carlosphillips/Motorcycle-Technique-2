...
import { SAVE_WINDOW_PLACARD, SAVE_WINDOW_STATUS_SENTENCES, horizonDisplay, saveAt, saveWindow } from "../solve/saveWindow.js";
 ...
 */
export function saveWindowOverlay(line, scene, opts) {
    ...
    for (const w of all.value) {
        if (!drawsScalars(w) || w.tau_close_s === undefined || w.s_close_m === undefined)
            continue;
        ...
        if (!probe.ok)
            continue; // a probe that cannot be re-launched simply draws nothing
        const samples = probe.value.shadow.samples;
        const path = samples.map((p) => {
            if (scene === undefined)
                return { x: p.x, y: p.y };
 ...
 * of the above.
 */
export function saveWindowHudRows(overlay, cursorT) {
    const rows = [];
    for (const w of overlay.windows) {
        if (w.status === "not_applicable")
            continue; // the toggle is inert for that corner
...
export function saveWindowTicks(overlay) {
 ...
 */
export function saveWindowOverlaySvg(overlay) {
    if (overlay.probes.length === 0)
        return "";
