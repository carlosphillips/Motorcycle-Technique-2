...
export function domainOf(line, axis) {
 ...
 */
export function scenarioDomain(lines, axis) {
    ...
    if (!Number.isFinite(min) || !Number.isFinite(max))
        return Object.freeze({ axis, min: 0, max: 0 });
    return Object.freeze({ axis, min, max });
}
export function clampTo(value, domain) {
...
export function withSpeed(state, speed) {
    return PLAYBACK_SPEEDS.includes(speed) ? Object.freeze({ ...state, speed }) : state;
}
/**
 ...
 */
export function advance(state, wallDeltaS, domain, line) {
    if (!state.playing)
        return state;
    const runDelta = state.speed * wallDeltaS;
    const next = state.axis === "t"
        ? state.value + runDelta
        : line === undefined
            ? state.value + runDelta
            : stationForTime(line, timeForStation(line, state.value) + runDelta);
    if (next >= domain.max)
        return Object.freeze({ ...state, value: domain.max, playing: false });
    return Object.freeze({ ...state, value: clampTo(next, domain) });
}
/** design/07 §3.1 — the frame-step button: one HUD refresh, ±0.1 s. */
export function stepFrame(state, direction, domain, line) {
    if (state.axis === "t")
        return scrubTo(state, state.value + direction * FRAME_STEP_S, domain);
    if (line === undefined)
        return state;
 ...
 */
export function stepSample(state, direction, line) {
    const samples = line.trajectory.samples;
    if (samples.length === 0)
        return state;
    const key = (i) => (state.axis === "s" ? samples[i].s : samples[i].t);
 ...
 */
export function jumpTo(state, bookmark, domain) {
 ...
 */
export function toggleAxis(state, line) {
    const axis = state.axis === "t" ? "s" : "t";
 ...
 */
function lastIndexAtOrBefore(length, key, value) {
    let lo = 0;
    let hi = length - 1;
    while (lo < hi) {
        const mid = (lo + hi + 1) >> 1;
...
function convert(line, value, from) {
    const samples = line.trajectory.samples;
    const first = samples[0];
    const last = samples[samples.length - 1];
...
export function stationForTime(line, t) {
    return convert(line, t, "t");
}
/** The run time at which this line reaches station `s`. */
export function timeForStation(line, s) {
