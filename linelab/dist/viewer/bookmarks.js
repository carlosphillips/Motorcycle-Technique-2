 ...
 */
function labelFor(kind, ordinal) {
    return ordinal <= 1 ? kind : `${kind}#${ordinal}`;
}
function bookmarkOf(e, index, ordinal) {
 ...
 */
export function bookmarksOf(line) {
    const seen = new Map();
    return Object.freeze(line.trajectory.events.map((e, index) => {
...
const KIND_BY_NAME = new Map(EVENT_KINDS.map((k) => [k, k]));
/** The round-trip's forward half: `Bookmark → token`. Total, never fails. */
export function printBookmark(b) {
    return b.label;
}
/**
 ...
 */
export function parseBookmark(token, line) {
