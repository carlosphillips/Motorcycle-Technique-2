...
function findReservedToken(tokens, at) {
    for (const t of tokens) {
        if (WIDTH_SUFFIX_RE.test(t)) {
            return {
                code: "SCHEMA",
                at,
                message: `per-segment width ("${t}") is reserved grammar space for a future version — ` +
 ...
 */
export function parseRoadDSL(str) {
    ...
    if (laneTokens[0] !== "lane" || laneTokens.length !== 2) {
        return err(schemaErr("dsl[0]", `road DSL must begin "lane <w>" (got "${first}")`, "dsl_lane_exactly_once_first"));
    }
    ...
    for (let i = 1; i < parts.length; i++) {
        ...
        if (head === "lane") {
            return err(schemaErr(at, `"lane" must appear exactly once and first`, "dsl_lane_exactly_once_first"));
        }
        ...
        if (head === "L" || head === "R") {
            const hand = head;
            if (tokens.length !== 3) {
                return err(schemaErr(at, `arc/taper is "${head} <r>[><r2>] ^<deg>" (got "${raw}")`, "dsl_bad_segment"));
            }
            const radiusTok = tokens[1] ?? "";
            const sweepTok = tokens[2] ?? "";
            if (!sweepTok.startsWith("^")) {
                return err(schemaErr(at, `sweep must be spelled "^<deg>" (got "${sweepTok}")`, "dsl_bad_segment"));
            }
            ...
            if (radiusTok.includes(">")) {
                ...
                if (radii.length !== 2) {
                    return err(schemaErr(at, `taper radii are "<r1>><r2>" (got "${radiusTok}")`, "dsl_bad_segment"));
                }
        ...
        return err(schemaErr(at, `unknown segment head "${head}" (expected S, L, or R)`, "dsl_bad_segment"));
    }
...
function fmtNum(n) {
    ...
    if (pointAt <= 0)
        return "0." + "0".repeat(-pointAt) + digits;
...
export function printRoadDSL(spec) {
    const parts = [`lane ${fmtNum(spec.lane_width_m)}`];
