...
import { G, K_REACH, MIN_POS_DD_M, RIDER_PROFILES, a_lat_pos_max, v_floor_ms } from "../core/constants.js";
...
import { COARSE_DS_M, VIS_HOLD_F_DEFAULT, VIS_MARGIN_DEFAULT, VIS_MAX_ITERATIONS } from "./constants.js";
import { constraintUnmet, constraintsSatisfied, evalConstraints, noSolution, solve } from "./solve.js";
import { buildChainContext, chainedSolve, executeSolvedPlan, patchAndReseal, wirePlanFromResolved } from "./chained.js";
// ---------------------------------------------------------------------------
...
function visMeasure(ctx, plan, entryKmh, coarse) {
    const scenario = {
        ...ctx.base,
        rider: {
            ...ctx.base.rider,
...
function ssdAt(env, v_ms, phi_deg) {
...
function evalV1(traj, margin, env) {
    ...
    for (const sm of traj.samples) {
        ...
        if (need > have + 1e-9) {
            ...
            const okAt = (u) => margin * ssd(u * sm.v, Math.atan(Math.tan(phiRad) * u * u), env.model, env.profile, env.mu).ssd_m <= have + 1e-9;
            let lo = 0.02;
            let hi = 1;
            if (!okAt(lo))
                lo = 0.001;
...
function tRoll(profile) {
    return Math.atan(a_lat_pos_max / G) / degToRad(profile.roll_rate_dps);
}
function generateHold(ctx, corner, windowStart, turnInS, fFrom, holdF, vGovMs, profile) {
 ...
 */
function releaseStation(traj, from_s, margin, env) {
    ...
    for (let i = 0; i < samples.length; i++) {
        ...
        if (sightOk && sightTrendAt(samples, i) === "opening")
            return sm.s;
    }
    return null;
}
...
function buildCandidate(chain, blind, vGovKmh, holdF, profile) {
    ...
    for (let k = 0; k < chain.ctxs.length; k++) {
        ...
        for (let p = 0; p < LEAN_FIX_PROBES; p++) {
            ...
            const probePlan = [...plan, tiProbe].sort((a, b) => a.at_s - b.at_s);
            ...
            for (const sm of m.traj.samples) {
                if (sm.s < corner.s0 - 1e-9 || sm.s > corner.s1 + 1e-9)
                    continue;
                if (sm.f > maxF)
                    maxF = sm.f;
                if (sm.f < minF)
 ...
 */
export function solveCautiousDetailed(spec) {
    ...
    const profile = cap !== undefined && cap < baseProfile.roll_rate_dps ? { ...baseProfile, roll_rate_dps: cap } : baseProfile;
    const env = { model: ctx0.base.config.ssd_model, profile, mu: ctx0.base.config.mu };
    ...
    for (let iter = 1; iter <= VIS_MAX_ITERATIONS; iter++) {
        ...
        if (kmhToMs(vGov) < v_floor_ms) {
            return err(noSolution("vis_speed_below_model_floor", "solveCautious", "the V1 governor caps speed below the model floor — the corner cannot be ridden within sight at this margin", { governed_kmh: vGov, floor_kmh: msToKmh(v_floor_ms), worst_s: v1.worst_s, vis_margin: margin }));
        }
        ...
        if (lastRelease !== null) {
            ...
            if (roS < chain.road.total_len_m - 1.5) {
                const ro = { do: "throttle", id: "ro_vis", at_s: roS, accel: 2.2, slew_mss: 12 };
        ...
        for (const gen of candidate.holds) {
            if (gen.action === null)
                continue; // no wire action → no hold clause
            ...
            let bandOk = true;
            for (const sm of traj.samples) {
                if (sm.s < gen.window[1] - 1e-9 || sm.s > release + 1e-9)
                    continue;
                if (sm.f < gen.target_eff - VIS_HOLD_F_TOL - 1e-9)
                    bandOk = false;
            }
            const relSample = traj.samples.reduce((best, sm) => (best === null || Math.abs(sm.s - release) < Math.abs(best.s - release) ? sm : best), null);
    ...
    return err(noSolution("vis_unsatisfiable_within_bound", "solveCautious", `no iterate passed the self-check within ${VIS_MAX_ITERATIONS} solve passes`, {
