 ...
 */
function interpolateSample(a, b, alpha, key, q, road) {
 ...
 */
function ssdStationM(samples, startIdx, from) {
    ...
    for (let k = startIdx; k < samples.length; k++) {
        ...
        if (seg > 0 && remaining <= seg)
            return ps + (n.s - ps) * (remaining / seg);
        remaining -= seg;
        px = n.x;
        py = n.y;
        ps = n.s;
...
function deriveAt(input, sample, i0, i1, alpha) {
    ...
    const action = sample.action_id === null ? null : (input.plan.find((a) => a.id === sample.action_id) ?? null);
...
function locate(samples, key, value) {
    if (samples.length === 0) {
        return err({
            code: "INTERNAL",
 ...
 */
export function dualAt(trajectory, query) {
    ...
    if (!located.ok)
        return located;
    const dual = parsed.value.key === "s" ? "t" : "s";
 ...
 */
export function stateAt(input, query) {
    const parsed = parseQuery(query);
    if (!parsed.ok)
        return parsed;
    const samples = input.trajectory.samples;
...
//# sourceMappingURL=stateAt.js.map