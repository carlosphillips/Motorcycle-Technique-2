...
function withSight(p, road, sight) {
...
function interpolate(raw, i, g, orClipped, orBelow, road) {
    const a = raw[i];
    const b = raw[i + 1];
    const span = b.s - a.s;
 ...
 */
export function resample(raw, road, sight, ds) {
    if (raw.length === 0)
        return [];
    const out = [];
    ...
    for (let gi = 0;; gi++) {
        ...
        while (i + 1 < raw.length && raw[i + 1].s <= g)
            i++;
        let point;
        let consumedTo;
        if (i + 1 >= raw.length) {
