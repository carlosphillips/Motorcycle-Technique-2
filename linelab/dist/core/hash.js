...
function build(v, path) {
    ...
    switch (typeof v) {
        ...
        case "number": {
            if (!Number.isFinite(v))
                canonFail(path, "non_finite_number", String(v));
            if (Object.is(v, -0))
                return "0"; // -0 normalized (JSON.stringify(-0) is "0" too; explicit for clarity)
    ...
    if (isOmitted(v))
        canonFail(path, "unserializable_value", typeof v);
    if (Array.isArray(v)) {
    ...
    for (const key of keys) {
        const value = obj[key];
        if (isOmitted(value))
            continue; // undefined-valued keys omitted
