...
import { errOutcome, okOutcome, parseJson, recomposeEnvelopeRoad, schemaErr, isObject } from "./shared.js";
function straightLenM(road) {
    return road.segments.filter((s) => s.type === "straight").reduce((sum, s) => sum + s.len_m, 0);
}
const MARK_CLASSES = ["turn_point", "apex", "exit", "release"];
function parseMarkSpec(v) {
    if (v === "auto" || v === "all" || v === "none")
        return v;
    const classes = v.split(",").map((c) => c.trim()).filter((c) => MARK_CLASSES.includes(c));
    return classes.length > 0 ? classes : "auto";
}
export function renderVerb(input) {
    ...
    const view = { ...(parsed.value.draft.view ?? {}) };
    if (parsed.value.mode !== undefined)
        view["mode"] = parsed.value.mode;
    ...
    const report = { figure_id: envelope.figure_id };
    if (wantTopdown) {
