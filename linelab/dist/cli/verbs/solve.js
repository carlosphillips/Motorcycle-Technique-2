...
export function solveVerb(input) {
    const parsed = parseZeroFileFlags(input.argv);
    if (!parsed.ok)
        return errOutcome(parsed.error);
    if (parsed.value.draft.mistakes.length > 0) {
        return errOutcome(schemaErr("--mistake", "solve returns one authored line — compile a mistake onto it with the `mistake` verb instead", "solve_mistake_use_mistake_verb"));
    }
    let loaded;
    ...
    // renamed by rebuild.
    const { line_id: lineId, ...composedSpec } = mergeDraftOverLoaded(loaded, parsed.value.draft);
    ...
    const line = lineId !== undefined ? relabelLine(result.value, lineId) : result.value;
    const roadR = composeSpecRoad(composed.road);
    if (!roadR.ok)
        return errOutcome(roadR.error);
