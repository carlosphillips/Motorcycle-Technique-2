...
function ineffectualStanding() {
    return {
        code: "INEFFECTUAL",
        at: "--standing",
        message: "check --standing grades finished lines, and this input carries no verdicts — pass a result envelope (the output of run/solve/figure)",
...
function sniffEnvelopeLines(lines) {
    for (let i = 0; i < lines.length; i++) {
        ...
        if (typeof verdict["outcome"] !== "string" ||
            ...
            return schemaErr(`${at}.verdict`, "envelope verdict must carry outcome, quality, rubric, checks_version, and a doctrine block", "envelope_line_malformed");
        }
    }
    return null;
}
function standingOutcome(envelope) {
    const lines = envelope["lines"];
    const sniffed = sniffEnvelopeLines(lines);
    if (sniffed !== null)
        return errOutcome(sniffed);
    const rows = standingAttachment(lines);
    if (!rows.ok)
        return errOutcome(rows.error);
    return okOutcome({
        valid: true,
...
// ---------------------------------------------------------------------------
export function checkVerb(input) {
    ...
    if (!looksLikeJson(input.loadedText)) {
        if (wantStanding)
            return errOutcome(ineffectualStanding());
    ...
    if (!validated.ok)
        return errOutcome(validated.error);
