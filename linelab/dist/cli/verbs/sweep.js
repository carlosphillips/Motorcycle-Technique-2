...
export const SWEEP_DEFAULT_METRICS = ["outcome", "apex_pct", "grip_min"];
/** design/08 §4.3 — the closed root set of the sweep-path grammar (D34). */
export const SWEEP_ROOTS = ["plan", "scenario", "config", "ride", "mistake", "constraint", "believe"];
/** `scenario.` addresses exactly the two rider.start scalars. */
const SCENARIO_FIELDS = ["entry_kmh", "start_f"];
/** `config.` addresses exactly the one numeric config field. */
const CONFIG_FIELDS = ["mu"];
/** `ride.` addresses the solve-spec intent scalars. */
const RIDE_FIELDS = ["vis_margin", "vis_hold_f", "turn_in_s"];
/** `mistake.` params (design/08 §4.3's inline list). */
const MISTAKE_PARAMS = ["early_by_m", "roll_rate_factor", "facets", "offset_m", "freeze_s", "by_kmh"];
/** `believe.` belief params (04 §4.6 / 03 §7.4's underread|overread sugar). */
const BELIEVE_FIELDS = ["r_believed", "sweep_believed_deg"];
function rootUnknown(at, root) {
    return schemaErr(at, `unknown sweep root "${root}" — the closed set is ${SWEEP_ROOTS.map((r) => `${r}.`).join(" | ")}`, "sweep_root_unknown", { root, roots: SWEEP_ROOTS });
}
function fieldNotNumeric(at, path, detail) {
...
export function parseSweepPath(path, at) {
    const parts = path.split(".");
    const root = parts[0];
    if (root === undefined || root.length === 0)
        return err(rootUnknown(at, path));
    if (!SWEEP_ROOTS.includes(root))
        return err(rootUnknown(at, root));
    switch (root) {
        case "plan": {
...
export function gridValues(range) {
    const out = [];
    const n = Math.floor((range.to - range.from) / range.step + 1e-9);
    for (let i = 0; i <= n; i++)
        out.push(Number((range.from + i * range.step).toFixed(9)));
    return out;
}
/**
 ...
 * engine runs on it.
 */
export function gridCells(v1, v2, max = SWEEP_MAX_CELLS) {
    const at = [];
 ...
 */
export function metricsOf(line, metrics) {
 ...
 */
function refusedRow(metrics) {
    const row = {};
    for (const m of metrics)
        row[m] = m === "outcome" ? "no_solution" : null;
    return row;
}
function baseKindOf(base) {
    if ("lines" in base)
        return "figure";
    if ("rider" in base)
        return "scenario";
...
function isSolveSpecLike(spec) {
    return "entry_kmh" in spec;
}
function isMistakeSpecLike(spec) {
    return "kind" in spec && !("entry_kmh" in spec) && !("rider" in spec);
}
/** Every SolveSpec-shaped line spec of a FigureSpec base, with its name. */
function figureRideSpecs(base) {
    const lines = base["lines"];
    if (!Array.isArray(lines))
        return [];
    const out = [];
    for (const l of lines) {
        if (!isObject(l) || typeof l["name"] !== "string" || !isObject(l["spec"]))
            continue;
        if (isSolveSpecLike(l["spec"]))
            out.push({ name: l["name"], spec: l["spec"] });
    }
    return out;
}
function lineSelectorRequired(at, ids) {
    return schemaErr(at, `this base carries several solver lines — pass --line (available: ${ids.join(", ")})`, "line_selector_required", { available: ids });
}
/**
 ...
 */
function selectRideSpec(base, requested, at) {
    if (baseKindOf(base) !== "figure") {
        ...
        if (requested !== undefined) {
            const id = typeof base["line_id"] === "string" ? base["line_id"] : "solved";
            if (requested !== id) {
                return err(unknownId(at, `unknown line "${requested}" (the composed base's solver line is "${id}")`, "unknown_line_id", { available: [id] }));
            }
        }
        return ok({ name: typeof base["line_id"] === "string" ? base["line_id"] : "solved", spec: base });
    }
    const rides = figureRideSpecs(base);
    if (rides.length === 0)
        return err(unknownId(at, "this figure carries no ride line to address", "sweep_no_solver_line"));
    if (requested !== undefined) {
    ...
    if (rides.length > 1)
        return err(lineSelectorRequired(at, rides.map((r) => r.name)));
    return ok(rides[0]);
}
// ---------------------------------------------------------------------------
// Per-root mutation (the "per-cell work" column of §4.3's table)
function setScenarioScalar(base, field, value) {
    const kind = baseKindOf(base);
    if (kind === "scenario") {
        const rider = isObject(base["rider"]) ? base["rider"] : {};
        const start = isObject(rider["start"]) ? rider["start"] : {};
    ...
    const lines = base["lines"];
    for (const l of lines) {
        if (!isObject(l) || !isObject(l["spec"]))
            continue;
        const spec = l["spec"];
        if (isSolveSpecLike(spec))
            spec[field] = value;
        else if (isObject(spec["rider"])) {
            const rider = spec["rider"];
            const start = isObject(rider["start"]) ? rider["start"] : {};
            start[field === "entry_kmh" ? "speed_kmh" : "f"] = value;
            rider["start"] = start;
        }
...
function setConfigMu(base, value) {
    const kind = baseKindOf(base);
    if (kind === "scenario") {
        const config = isObject(base["config"]) ? base["config"] : {};
        config["mu"] = value;
        base["config"] = config;
        return;
    }
    ...
    for (const l of base["lines"]) {
        ...
        if (isSolveSpecLike(spec))
            spec["mu"] = value;
        else if (isObject(spec["config"]))
            spec["config"]["mu"] = value;
        else if ("rider" in spec)
            spec["config"] = { mu: value };
    }
}
/** `mistake.<lineId>.<param>` — the addressed mistake line's params. */
function mistakeParamsOf(base, lineId, at) {
    if (baseKindOf(base) === "figure") {
        for (const l of base["lines"]) {
            if (!isObject(l) || l["name"] !== lineId || !isObject(l["spec"]))
                continue;
            const spec = l["spec"];
            if (!isMistakeSpecLike(spec))
                break;
    ...
    if (mistake["kind"] !== lineId) {
        return err(unknownId(at, `unknown mistake line "${lineId}" (this base's mistake line is "${String(mistake["kind"])}")`, "unknown_line_id", {
            available: [String(mistake["kind"])]
        }));
    }
    const params = isObject(mistake["params"]) ? mistake["params"] : {};
    mistake["params"] = params;
    return ok(params);
}
/** `believe.<field>` — the belief params of the addressed misjudge (underread|overread) line. */
function beliefParamsOf(base, at) {
    const isMisjudgeKind = (k) => k === "underread" || k === "overread";
    if (baseKindOf(base) === "figure") {
        for (const l of base["lines"]) {
            if (!isObject(l) || !isObject(l["spec"]))
                continue;
 ...
 * consumers.
 */
function runnableScenario(rs, lineId) {
    return clone({
        ...rs,
        spec: "linelab/1",
        id: lineId,
...
function firstLineResult(env, requested) {
    const drawn = env.lines.filter((l) => !isLineRefusal(l));
    if (requested !== undefined)
        return drawn.find((l) => l.line_id === requested) ?? null;
    return drawn[0] ?? null;
}
export function sweepVerb(input) {
    ...
    // -- metric columns -------------------------------------------------------
    let metrics = SWEEP_DEFAULT_METRICS;
    if (flags.metric !== undefined) {
        const named = flags.metric.split(",").map((m) => m.trim()).filter((m) => m.length > 0);
        const bad = named.find((m) => !SWEEP_METRICS.includes(m));
        if (bad !== undefined) {
            return errOutcome(schemaErr("--metric", `unknown sweep metric "${bad}" — the closed set is ${SWEEP_METRICS.join(", ")}`, "sweep_metric_unknown", {
    ...
    for (const at of grid.at) {
        const assign = [[p1.value, at[0]]];
        if (p2 !== undefined && at[1] !== undefined)
            assign.push([p2, at[1]]);
        const cell = runCell(prep.value, assign, flags.line, metrics, runOpts, expected);
        if (!cell.ok)
            return errOutcome(cell.error);
        for (const id of Object.keys(cell.value))
    ...
    const writes = flags.format === "tsv" && flags.out !== undefined
...
// ---------------------------------------------------------------------------
// prepare / runCell
function prepare(base, path1, path2, line, runOpts) {
    const paths = path2 === undefined ? [path1] : [path1, path2];
    const planPath = paths.find((p) => p.root === "plan");
    ...
    if (paths.some((p) => p.root !== "plan")) {
        ...
        return err(schemaErr("--param2", "a `plan.` sweep bypasses the solver, so it cannot be crossed with a solver-layer root in one grid", "sweep_plan_root_not_crossable"));
    }
    ...
    for (const p of paths) {
        ...
        if (typeof action[p.field] !== "number") {
 ...
 */
function expectedLineIds(base, prep) {
    ...
    switch (baseKindOf(base)) {
        case "figure": {
            ...
            if (!Array.isArray(lines))
                return [];
            return lines.filter((l) => isObject(l) && typeof l["name"] === "string").map((l) => l.name);
        }
        case "scenario":
            return [typeof base["id"] === "string" ? base["id"] : "scenario"];
        case "composed": {
            const primary = typeof base["line_id"] === "string" ? base["line_id"] : "solved";
            const mistake = base["mistake"];
            return isObject(mistake) && typeof mistake["kind"] === "string"
                ? [primary, mistake["kind"]]
                : [primary];
        }
    }
}
function runCell(prep, assign, line, metrics, runOpts, expected) {
    let input;
    let keyOverride;
    if (prep.kind === "resolved") {
        const scenario = runnableScenario(prep.scenario, prep.lineId);
        const rider = scenario["rider"];
        ...
        for (const [p, value] of assign) {
            switch (p.root) {
                ...
                case "believe": {
                    ...
                    for (const f of BELIEVE_FIELDS)
                        delete params.value[f];
    ...
    if (!result.ok) {
        ...
        if (result.error.code !== "NO_SOLUTION")
            return result;
        const row = refusedRow(metrics);
        const out = {};
        for (const id of expected) {
            if (line !== undefined && id !== line)
                continue;
            out[id] = row;
        }
        return ok(out);
...
function perLineRows(lines, metrics, filter, keyOverride) {
    const out = {};
    for (const entry of lines) {
        const key = keyOverride !== undefined && lines.length === 1 ? keyOverride : entry.line_id;
        if (filter !== undefined && key !== filter && entry.line_id !== filter)
            continue;
        out[key] = isLineRefusal(entry) ? refusedRow(metrics) : metricsOf(entry, metrics);
    }
    return out;
}
// ---------------------------------------------------------------------------
// TSV (ARCHITECTURE §10 pin #23: one file at --out; stdout stays the JSON doc)
function toTsv(paths, lineIds, metrics, cells) {
    const cols = [...paths];
    ...
    for (const cell of cells) {
        const row = cell.at.map((v) => String(v));
        for (const id of lineIds) {
