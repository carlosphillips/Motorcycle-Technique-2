...
function isEnvelope(v) {
    return typeof v === "object" && v !== null && Array.isArray(v["lines"]);
}
export function mistakeVerb(input) {
    ...
    if (!parsedTok.ok)
        return errOutcome(parsedTok.error);
    ...
    const candidates = envelope.lines.filter((l) => !isLineRefusal(l));
    let base;
    if (parsed.value.line !== undefined) {
        base = candidates.find((l) => l.line_id === parsed.value.line);
        if (base === undefined) {
            return errOutcome(schemaErr("--line", `no line "${parsed.value.line}" (available: ${candidates.map((l) => l.line_id).join(", ")})`, "line_selector_required"));
        }
    }
        ...
        if (ideals.length === 1)
            ...
            return errOutcome(schemaErr("--line", `ambiguous base line — pass --line (available: ${candidates.map((l) => l.line_id).join(", ")})`, "line_selector_required"));
        }
    }
    const baseLine = base;
    if (baseLine.source.kind !== "solve") {
        return errOutcome(schemaErr("--on", `line "${baseLine.line_id}" was not produced by the solver — a mistake needs a solved base line`, "mistake_base_not_solved"));
    }
    const spec = baseLine.source.solveSpec;
    ...
    if (!compiled.ok)
        return errOutcome(compiled.error);
