 ...
 */
export function controlsView(input) {
    ...
    const road = recomposed.value;
    const drawn = envelope.lines.filter((l) => !isLineRefusal(l));
    if (drawn.length === 0) {
        return errOutcome(schemaErr("input", "no drawable line in this envelope — every line is a refusal (05 §7)", "controls_no_drawn_line"));
    }
    ...
    if (!projected.ok)
        return errOutcome(projected.error);
    const strip = { from: projected.value.window.from_s, to: projected.value.window.to_s };
