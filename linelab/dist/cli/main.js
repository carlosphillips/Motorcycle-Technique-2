...
function writeOutputs(outcome, pretty) {
    ...
    if (outcome.stderr !== undefined && outcome.stderr.length > 0)
    ...
    const finalExit = writeFailed && outcome.exit === EXIT.OK ? EXIT.WRITE_FAILED : outcome.exit;
...
function main() {
    ...
    if (!preParsed.ok)
        fail(EXIT.BAD_INPUT, preParsed.error);
    ...
    switch (verb) {
        ...
        case "check": {
            let loadedText;
            if (positional0 === "-")
                loadedText = readStdin();
            else if (positional0 !== undefined) {
                const r = tryReadFile(positional0);
                if (!r.ok)
                    fail(EXIT.BAD_INPUT, { code: "SCHEMA", at: verb, message: `cannot read "${positional0}": ${r.message}`, detail: { reason: "file_not_readable" } });
                loadedText = r.text;
            }
            ...
            outcome =
                verb === "run"
                    ...
                    : verb === "solve"
                        ...
                        : checkVerb({ loadedText: loadedText, argv: rest });
            break;
        }
        case "figure": {
        ...
        case "sweep": {
            ...
            if (positional0 === "-")
                ...
                loadedText = loadFileOrFail("sweep", positional0);
        ...
        case "mistake": {
            ...
            if (onPath === "-")
                loadedText = readStdin();
            else if (onPath !== undefined)
                loadedText = loadFileOrFail("mistake", onPath);
        ...
        case "explain": {
            ...
            if (positional0 === "-")
                loadedText = readStdin();
            else if (positional0 !== undefined && existsSync(positional0))
                loadedText = loadFileOrFail("explain", positional0);
            else
                target = positional0;
...
function loadFileOrFail(at, path) {
    const r = tryReadFile(path);
    if (!r.ok)
        fail(EXIT.BAD_INPUT, { code: "SCHEMA", at, message: `cannot read "${path}": ${r.message}`, detail: { reason: "file_not_readable" } });
    return r.text;
}
// ---------------------------------------------------------------------------
...
function resolveModuleFile(urlPath, moduleRoot) {
    if (!urlPath.startsWith(moduleRoot + "/"))
        return null;
    let decoded;
    try {
...
function startViewerServer(plan, outcome, pretty) {
    ...
    const server = createServer((req, res) => {
        ...
        res.end(JSON.stringify({ ok: false, error: { code: "UNKNOWN_ID", at: path, message: `no route "${path}"`, detail: { reason: "no_route" } } }));
    });
    server.on("error", (e) => {
